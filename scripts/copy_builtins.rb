...
$options = {:insieme_builtins_dir => nil, :command => nil, :std_include_dir => nil, :force => false, :quiet => false}
...
parser = OptionParser.new do|opts|
	...
	opts.on('-c', '--command command', 'Compiler or preprocessor command to run and parse for getting include directory path') do |cmd|
		$options[:command] = cmd;
	end
...
raise OptionParser::MissingArgument if $options[:insieme_builtins_dir] == nil || ($options[:std_include_dir] == nil && $options[:command] == nil)
...
if($options[:std_include_dir] == nil)
	preprocessor_output = `#{$options[:command]}`
	match = preprocessor_output.match(/#include <\.\.\.> search starts here:\n\s+([^\n]+)/)
	if(match)
		$options[:std_include_dir] = match[1]
	else
		puts "Unable to parse preprocessor output" if !$options[:quiet]
		exit 1
	end
end
