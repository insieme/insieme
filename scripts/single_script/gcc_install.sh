...
CFLAGS="-m64 -O3" CXXFLAGS="-m64 -O3" LDFLAGS="-m64 -O3" ../gcc-$VERSION/configure --prefix=$PREFIX/gcc-$VERSION --enable-languages=c,c++ --with-gmp=$PREFIX/gmp-latest --with-mpfr=$PREFIX/mpfr-latest --with-mpc=$PREFIX/mpc-latest --with-ppl=$PREFIX/ppl-latest --with-cloog=$PREFIX/cloog-gcc-latest --enable-cloog-backend=isl --disable-multilib  --enable-lto
make -j $SLOTS
