...
// global variables
cl_context cxGPUContext = 0;
cl_kernel multiplicationKernel[MAX_GPU_COUNT] = {0};
cl_command_queue commandQueue[MAX_GPU_COUNT] = {0};
...
void matrixMulGPU(cl_uint ciDeviceCount, cl_mem h_A, float* h_B_data, unsigned int mem_size_B, unsigned int size_B, float* h_C )
{
    ...
    for(unsigned int i=0; i < ciDeviceCount; ++i) 
    {
        ...
        d_A[i] = clCreateBuffer(cxGPUContext, CL_MEM_READ_ONLY, workSize[i] * sizeof(float) * uiWA, NULL,0);
        ...
        d_B[i] = clCreateBuffer(cxGPUContext, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR,
                                sizeof(float) * size_B, h_B_data, 0);

        // Output buffer
        d_C[i] = clCreateBuffer(cxGPUContext, CL_MEM_WRITE_ONLY,  workSize[i] * uiWC * sizeof(float), NULL,0);
...
{ 
    ...
    cl_mem h_A = clCreateBuffer(cxGPUContext, CL_MEM_READ_ONLY | CL_MEM_USE_HOST_PTR,
				    sizeof(float) * size_A, h_A_data, &ciErrNum);
    if (ciErrNum != CL_SUCCESS)
    {
    ...
    for(unsigned int k = 1; k < ciDeviceCount+1; ++k) 
    {
        matrixMulGPU(k, h_A, h_B_data, mem_size_B, size_B, h_C);
    }
    ...
    if( ciErrNum != CL_SUCCESS) 
        shrLog("Error: Failed to release context: %d\n", ciErrNum);
