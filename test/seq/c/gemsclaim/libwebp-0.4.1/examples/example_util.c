...
int ExUtilReadFromStdin(const uint8_t** data, size_t* data_size) {
  ...
  *data_size = 0;
  ...
  while (!feof(stdin)) {
    ...
    uint8_t* const new_data = (uint8_t*)realloc(input, max_size + extra_size);  // insieme: pointer type specialized
...
static char* const kStatusMessages[VP8_STATUS_NOT_ENOUGH_DATA + 1] = {  // insieme: dropped const
