...
__kernel void median_filter(__global uint* inputImage, __global uint* outputImage, int num_elements, uint width) {
	...
	if( tx >= 1 && tx < (width-1) && ty >= 1 && ty < num_elements/width - 1) {
		...
		uiRGBA[0] = inputImage[gid - 1 - width];
                uiRGBA[1] = inputImage[gid - width];
                uiRGBA[2] = inputImage[gid + 1 - width];
                uiRGBA[3] = inputImage[gid - 1];
                uiRGBA[4] = inputImage[gid];
                uiRGBA[5] = inputImage[gid + 1];
                uiRGBA[6] = inputImage[gid - 1 + width];
                uiRGBA[7] = inputImage[gid + width];
                uiRGBA[8] = inputImage[gid + 1 + width];
