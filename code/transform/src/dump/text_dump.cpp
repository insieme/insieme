...
struct ValuePrinter : public boost::static_visitor<std::ostream&> {
	...
	ValuePrinter(std::ostream& out, const Indent& indent)
...
class TextLoader {
	...
	struct ValueParser {
		...
		ValueParser(token_iterator& next, const token_iterator& end, const TextLoader& loader)
			: next(next), end(end), loader(loader) {}
		...
		parameter::Value operator()(const parameter::ListParameterPtr& type) {
			...
			if(*(next++) != "(") {
				assert_fail() << "No opening bracket!";
			}
		...
		parameter::Value operator()(const parameter::TupleParameterPtr& type) {
			...
			if(*(next++) != ")") {
				assert_fail() << "No closing bracket!";
			}
			return parameter::makeValue(res);
		}
...
private:
	...
	TransformationPtr resolve(token_iterator& cur, const token_iterator& end) const {
		...
		if(paramInfos->isAtomic()) {
			...
			if(*(cur++) != "(") {
				/* TODO: throw exception */ assert_fail();
			};
