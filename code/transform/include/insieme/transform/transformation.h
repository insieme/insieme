...
namespace transform {
	...
	class Transformation : public utils::Printable {
	  ...
	  public:
		 ...
		 */
		Transformation(const TransformationType& type, const parameter::Value& params) : type(type), subTransformations(), parameters(params) {
	...
	struct AbstractTransformationType : public TransformationType, public utils::StaticConstant<Derived> {
		 ...
		 */
		AbstractTransformationType(const string& name, const string& desc, bool connector,
		                           const parameter::ParameterPtr& paramInfo = parameter::no_parameters())
