...
namespace c {
	 ...
	 * the actual name.
	 */
	struct DeclarationTag : public core::value_annotation::copy_on_migration {
		bool operator==(const DeclarationTag& other) const {
			return true;
		}
	};
...
bool isDeclaration(const insieme::core::GenericTypePtr& genTy) {
	return genTy->hasAttachedValue<DeclarationTag>();
}

void markDeclaration(const insieme::core::GenericTypePtr& genTy, bool value) {
	if(value) {
		genTy->attachValue(DeclarationTag());
	} else {
		genTy->detachValue<DeclarationTag>();
	}
}
