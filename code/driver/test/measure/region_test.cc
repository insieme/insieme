...
namespace measure {
	...
	TEST(Region, MPIExample) {
		...
		EXPECT_EQ("0-0-2-0-1-2-8-1-3-0-0-1-0", toString(mpiList[0].getBegin()));
		EXPECT_EQ("0-0-2-0-1-2-8-1-3-0-0-1-2", toString(mpiList[0].getEnd()));
		EXPECT_EQ("0-0-2-0-1-2-8-1-3-0-0-1-3", toString(mpiList[1].getBegin()));
