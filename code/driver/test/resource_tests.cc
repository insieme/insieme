...
TEST(SpeedTest, VisitAllPtr) {
	...
	// visit all pointer
	int counterPtr = 0;
	double timePtr = TIME(visitDepthFirst(root, [&](const NodePtr& cur) {
		counterPtr++;
	}, true, true));
	
	// visit all addresses
	int counterAdr = 0;
	double timeAdr = TIME(visitDepthFirst(NodeAddress(root), [&](const NodeAddress& cur) {
		counterAdr++;
	}, true, true));
...
TEST(SpeedTest, VisitOncePtr) {
	...
	// visit all pointer
	int counterPtr = 0;
	double timePtr = TIME(visitDepthFirstOnce(root, [&](const NodePtr& cur) {
		counterPtr++;
	}, true, true));
	
	// visit all addresses
	int counterAdr = 0;
	double timeAdr = TIME(visitDepthFirstOnce(NodeAddress(root), [&](const NodeAddress& cur) {
		counterAdr++;
	}, true, true));
