...
Unit::Terms getTerms(known_unit unit) {
	switch(unit) {
	case m:
		return toVector(Unit::Term("m",1));
	case s:
		return toVector(Unit::Term("s",1));
	case kg:
		return toVector(Unit::Term("kg",1));
	case byte:
		return toVector(Unit::Term("byte",1));
	case celsius:
		return toVector(Unit::Term("celsius",1));
	case percent:
		return Unit::Terms();	// empty!
	}
...
Unit::Terms merge(const Unit::Terms& a, const Unit::Terms& b) {
	...
	while(it1 != end1 && it2 != end2) {
		...
		if(a.first == b.first) {
			...
			if(newExp != 0) {
				res.push_back(Unit::Term(a.first, newExp));
			}
			++it1;
			++it2;
		}
	...
	std::transform(it2, end2, out, [](const Unit::Term& cur)->Unit::Term {
		connector_op op; return Unit::Term(cur.first, op(0, cur.second));
	});
...
Unit Unit::operator^(int exp) const {
	...
	for_each(res.terms, [&](Unit::Term& cur) {
		cur.second *= exp;
	});
	return res;
}
...
bool Unit::operator<(const Unit& other) const {
	...
	if(lexicographical_compare(terms, other.terms)) {
		return true;
	}
