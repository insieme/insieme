 ...
 */
template<typename T> first_impl<T> first(const T& list) {
 ...
 * parameters, this function is introducing the necessary automated type deduction.
 */
template<typename T> var_impl<T> var(const T& list) {
 ...
 * parameters, this function is introducing the necessary automated type deduction.
 */
template<typename T> min_impl<T> min(const T& list) {
 ...
 * parameters, this function is introducing the necessary automated type deduction.
 */
template<typename T> max_impl<T> max(const T& list) {
 ...
 * parameters, this function is introducing the necessary automated type deduction.
 */
template<typename T> sum_impl<T> sum(const T& list) {
 ...
 * parameters, this function is introducing the necessary automated type deduction.
 */
template<typename T> avg_impl<T> avg(const T& list) {
...
//			}
	...
	for(const core::StatementAddress& point : exitPoints) {
		...
		if(point->getNodeType() == core::NT_BreakStmt || point->getNodeType() == core::NT_ContinueStmt) {
			...
			instrumented = core::transform::insert(manager, point.switchRoot(instrumented).getParentAddress().as<core::CompoundStmtAddress>(), region_inst_end_call,
			                                       point.getIndex()).as<core::StatementPtr>();
			continue;
		}
...
vector<vector<MetricPtr>> partitionPapiCounter(const vector<MetricPtr>& metric) {
	...
	for_each(getDependencyClosureLeafs(metric), [&](const MetricPtr& cur) {
		if(boost::algorithm::starts_with(cur->getName(), "PAPI")) {
			dep.insert(cur);
		}
	});
