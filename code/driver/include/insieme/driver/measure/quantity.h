...
namespace measure {
	...
	class Prefix : public utils::Printable,
		public boost::less_than_comparable<Prefix, Prefix> {
	...
	private:
		 ...
		 */
		Unit(Prefix prefix, const Terms& components)
			: scale(prefix), terms(components) {}
	...
	public:
		...
		Unit(const string& unit)
			: scale(1), terms(toVector(Term(unit, 1))) {}
		 ...
		 */
		const Prefix& getPrefix() const { return scale; }

		/**
		 * Obtains a reference to the unit terms of this unit.
		 */
		const Terms& getUnitTerm() const { return terms; }
	...
	T scaleTo(const T& value, const Unit& from, const Unit& to) {
		...
		if (from.getUnitTerm() != to.getUnitTerm()) {
			throw UnitException(from, to);
		}
	...
	class Quantity : public utils::Printable,
		...
		public boost::addable<Quantity>, public boost::subtractable<Quantity>,
		public boost::multipliable<Quantity>, public boost::dividable<Quantity> {
		 ...
		 */
		Quantity(const UnitPtr& unit)
			: value(0), valid(false), unit(unit) {}
	...
	public:
		 ...
		 */
		Quantity(double value, Unit unit = Unit())
			: value(value), valid(true), unit(std::make_shared<Unit>(unit)) {}
		 ...
		 * @param unit the associated unit
		 */
		Quantity(double value, const UnitPtr& unit)
			: value(value), valid(true), unit(unit) {}
