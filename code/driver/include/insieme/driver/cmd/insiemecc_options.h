...
namespace cmd {
	...
	enum BackendEnum { Runtime = 0, Sequential, OpenCL, Pthreads };
	...
	#define OPTION(_name__, _id__, _type__, _default_value__, _description__) _type__ _id__;
	#include "insieme/driver/cmd/insiemecc_options.def"
	};
	...
	struct Options {
	  ...
	  private:
		 ...
		 */
		Options() : valid(true), gracefulExit(false), settings(Settings{}), job() {}
	};
