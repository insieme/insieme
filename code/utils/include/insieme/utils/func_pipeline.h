...
namespace impl {
	...
	struct check_compability<First, Second, Fs...> {
		enum {
			value = (std::is_same<type_list<typename lambda_traits<First>::result_type>, typename lambda_traits<Second>::argument_types>::value
			         || std::is_same<type_list<const typename lambda_traits<First>::result_type&>, typename lambda_traits<Second>::argument_types>::value)
			        && check_compability<Second, Fs...>::value
		};
	};
	...
	struct check_compability<SecondLast, Last> {
		enum {
			value = std::is_same<type_list<typename lambda_traits<SecondLast>::result_type>, typename lambda_traits<Last>::argument_types>::value
			        || std::is_same<type_list<const typename lambda_traits<SecondLast>::result_type&>, typename lambda_traits<Last>::argument_types>::value
		};
	};
	...
	struct apply_func {
		...
		static typename lambda_traits<Func>::result_type applyTuple(const Func& f, const std::tuple<ArgsT...>& t, Args... args) {
	...
	typename lambda_traits<Func>::result_type applyTuple(const Func& f, const std::tuple<ArgsT...>& t) {
		return apply_func<Start, size_of<typename lambda_traits<Func>::argument_types>::value>::applyTuple(f, t);
	}
...
struct Pipeline<
	...
	template <class... Args>
	typename std::enable_if<std::is_same<argument_types, type_list<Args...>>::value || std::is_same<argument_types, type_list<const Args&...>>::value,
	                        result_type>::type
	...
	template <class... Args>
	typename std::enable_if<std::is_same<argument_types, type_list<Args&...>>::value, result_type>::type operator()(Args&... args) const {
		return impl::invoker<std::tuple<Fs...>, sizeof...(Fs)>()(tup, args...);
	}
...
struct Reduction<Op, F1, F2, typename std::enable_if<std::is_same<type_list<typename lambda_traits<F1>::result_type, typename lambda_traits<F2>::result_type>,
                                                                  typename lambda_traits<Op>::argument_types>::value
                                                     || std::is_same<type_list<const typename lambda_traits<F1>::result_type&,
                                                                               const typename lambda_traits<F2>::result_type&>,
                                                                     typename lambda_traits<Op>::argument_types>::value>::type> {
...
auto makeReduction(const Op& op, const F1& f1, const F2& f2, const F3& f3, const Fs&... fs) -> Reduction<Op, F1, decltype(makeReduction(op, f2, f3, fs...))> {
