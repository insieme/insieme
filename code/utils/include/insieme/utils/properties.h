 ...
 */
template<typename ... T> struct value_helper;
...
public:
	 ...
	 * @param description the description to be attached to the resulting property
	 */
	AtomicProperty(const string& typeName, const string& description)
		: Property<Value>(true, description), typeName(typeName) {}
...
public:
	...
	virtual bool isValid(const Value& value) const {
		...
		if(!isTypeOf<vector<Value>, Value>(value)) {
			return false;
		}
		...
		return all(paired.first, paired.second, [](const std::pair<Value, typename Property<Value>::ptr>& cur) {
...
template<> const char* getName<bool>() {
	return "bool";
};
template<> const char* getName<int>() {
	return "int";
};
template<> const char* getName<unsigned>() {
	return "unsigned";
};
template<> const char* getName<float>() {
	return "float";
};
template<> const char* getName<double>() {
	return "double";
};

template<> const char* getName<string>() {
	return "string";
};

template<> const char* getName<uint64_t>() {
	return "uint64";
};
template<> const char* getName<int64_t>() {
	return "int64";
};
}
...
private:
	...
	void printFormated(std::ostream& out, const PropertyPtr& ptr, unsigned indent) const {
		...
		if(TuplePropertyPtr tuple = std::dynamic_pointer_cast<TupleProperty<Value>>(ptr)) {
			out << times(" ", indent) << "( \n";
			for_each(ptr->getComponents(), [&](const PropertyPtr& cur) {
				this->printFormated(out, cur, indent + 4);
			});
