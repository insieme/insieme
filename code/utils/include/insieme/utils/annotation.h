...
namespace utils {
	 ...
	 */
	template <class SubAnnTy, class BaseAnnotation = Annotation, typename boost::enable_if<boost::is_base_of<Annotation, BaseAnnotation>, int>::type = 0>
	...
	namespace detail {
		 ...
		 */
		template <typename V, typename AnnotationType, typename KeyType, typename Derived>
		class ValueAnnotationBase : public AnnotationType {
		  ...
		  public:
			...
			ValueAnnotationBase(const Args&... args)
			    : value(args...) {}
		  ...
		  private:
			...
			template <typename T>
			typename std::enable_if<is_printable<V>::value, T&>::type print(T& out) const {
				return out << value;
			}
		...
		class ValueAnnotation : public ValueAnnotationBase<V, AnnotationType, KeyType, ValueAnnotation<V, AnnotationType, KeyType>> {
		  public:
			template <typename... Args>
			ValueAnnotation(const Args&... args)
			    : ValueAnnotationBase<V, AnnotationType, KeyType, ValueAnnotation<V, AnnotationType, KeyType>>(args...) {}
		};
	}
	 ...
	 */
	template <typename AnnotationType = Annotation, typename KeyType = AnnotationKey,
	...
	class Annotatable {
	  ...
	  public:
		 ...
		 */
		Annotatable(const Annotatable<AnnotationType, KeyType>& other) : map((other.map) ? new annotation_map_type(*other.map) : 0) {}
		...
		const annotation_ptr_type& getAnnotation(const AnnotationKeyPtr& key) const {
			...
			if(!hasAnnotations()) { return none; }

			// search for entry
			auto pos = map->find(key);
			if(pos == map->end()) { return none; }
		...
		typename std::shared_ptr<typename Key::annotation_type> getAnnotation(const Key* key) const {
			...
			// check type
			assert_true(std::dynamic_pointer_cast<typename Key::annotation_type>(res)) << "Annotation Type of Key does not match actual annotation!";
	...
	class SimpleKey : public AnnotationKey {
	  ...
	  public:
		...
		bool equals(const AnnotationKey& other) const {
			...
			if(typeid(other) != typeid(SimpleKey<T, Comparator>)) { return false; }
	...
	bool hasSameAnnotations(const Annotatable<Annotation, Key>& annotatableA, const Annotatable<Annotation, Key>& annotatableB) {
		...
		if(!annotatableA.hasAnnotations() && !annotatableB.hasAnnotations()) { return true; }

		// check in case both have annotations ...
		if(!(annotatableA.hasAnnotations() && annotatableB.hasAnnotations())) { return false; }
