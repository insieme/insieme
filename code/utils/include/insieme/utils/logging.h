...
#define LOG_FILTER_ENV "INSIEME_LOG_FILTER"
	...
	class Logger {
		...
		Logger(std::ostream& out, const Level& level, unsigned short verbosity, boost::regex filter)
		    : m_null_logger(io::null_sink()), out(out), m_level(level), m_verbosity(verbosity), filter(filter) {}
...
#define LOG(LEVEL)                                                                                                                                             \
	if(Logger::get().level() > LEVEL || !Logger::get().isIncludedInFilter(__PRETTY_FUNCTION__))                                                                \
		;                                                                                                                                                      \
	else                                                                                                                                                       \
	(Logger::get().getActiveStream<MAKE_FORMAT(LEVEL)>(MAKE_CONTEXT).logStream << "] ")

#define VLOG(VerbLevel)                                                                                                                                        \
	if(VerbLevel > Logger::get().verbosity() || !Logger::get().isIncludedInFilter(__PRETTY_FUNCTION__))                                                        \
		;                                                                                                                                                      \
	else                                                                                                                                                       \
	(Logger::get().getActiveStream<MAKE_FORMAT(DEBUG)>(MAKE_CONTEXT).logStream << "] ")
