 ...
 */
template<
    typename Pattern = char,
    typename Matcher = std::equal_to<Pattern>
    >
...
public:
	 ...
	 */
	~Automata() {
		for_each(allStates, [](const state_type cur) {
			delete cur;
		});
	}
	...
	getOutgoingTransitions(state_type state) const {
...
private:
	...
	void addReachableStates(state_type state, std::set<state_type>& res) const {
		...
		// resolve successors
		for_each(state->transitions, [&](const transition_type& cur) {
			this->addReachableStates(cur.second, res);
		});
	}
	...
	void buildEpsilonClosure(state_type state, std::set<state_type>& res) const {
		...
		// resolve successors
		for_range(state->transitions.equal_range(epsilon), [&](const transition_type& cur) {
			this->buildEpsilonClosure(cur.second, res);
		});
	}
		...
		for_each(begin, end, [&](const CharType& character) {
			...
			for_each(current, [&](const state_type& state) {
				for_range(this->getOutgoingTransitions(state), [&](const transition_type& cur) {
					...
					if(matcher(cur.first, character)) {
						set::insertAll(newStates, this->getEpsilonClosure(cur.second));
					}
				});
			});
...
NFA<P,M> toNFA(const Automata<P,M>& automata) {
	...
	// add final states
	for_each(automata.getFinalStates(), [&](State cur) {
		res.addFinalState(ids[cur]);
	});
 ...
 */
template<
    template<typename P, typename M> class A,
    typename P, typename M, typename Iterator
    >
...
template<
    template<typename P, typename M> class A,
    typename P, typename M, typename Container
    >
...
struct GraphPrinter {
	...
	void print(std::stringstream& out, State cur) {
		...
		if(resolved.find(cur) != resolved.end()) {
			return;
		}
		resolved.insert(cur);
		...
		if(automata.isFinalState(cur)) {
			out << "    " << from << " [shape=doublecircle]\n";
		}
		...
		for_each(next, [&](State successor) {
			this->print(out, successor);
		});
	}
