...
namespace constraint {
	...
	public:
		...
		Constraint(const vector<Variable>& in, const vector<Variable>& out,
		...
		const vector<Variable>& getInputs() const { return inputs; };
		const vector<Variable>& getOutputs() const { return outputs; };
	...
	namespace detail {
		...
		struct BinaryOpFilter : public Filter<> {
			F f; TypedVariable<A> a; TypedVariable<B> b;
			BinaryOpFilter(const F& f, const TypedVariable<A>& a, const TypedVariable<B>& b)
				: f(f), a(a), b(b) {}
			bool operator()(const Assignment& ass) const {
		...
		struct TrinaryOpFilter : public Filter<> {
			F f; TypedVariable<A> a; TypedVariable<B> b; TypedVariable<C> c;
			TrinaryOpFilter(const F& f, const TypedVariable<A>& a, const TypedVariable<B>& b, const TypedVariable<C>& c)
				: f(f), a(a), b(b), c(c) {}
			bool operator()(const Assignment& ass) const {
		...
		struct BiggerThanFilter : public Filter<>{
			...
			std::size_t s;
			BiggerThanFilter(const TypedVariable<SetLattice<T>>& a, std::size_t s)
				: a(a), s(s) {}
			bool operator()(const Assignment& ass) const {
				return ass[a].size() > s;
			}
		...
		struct BiggerThanReducedFilter : public Filter<>{
			...
			T e;
			std::size_t s;
			BiggerThanReducedFilter(const TypedVariable<SetLattice<T>>& a, const T& e, std::size_t s)
				: a(a), e(e), s(s) {}
			bool operator()(const Assignment& ass) const {
				auto& set = ass[a];
		...
		class ElementOf : public Executor {
			...
			TypedVariable<L> a;
		public:
			ElementOf(const E& e, const TypedVariable<L>& a)
				: e(e), a(a) {}
			const Variables& getInputs() const {
				static const Variables empty;
				return empty;
		...
		public:
			SubsetUnary(const TypedVariable<A>& a, const TypedVariable<R>& r, const fun_type& f)
				: a(a), r(r), f(f) {}
			Variables getInputs() const {
				return toVector<Variable>(a);
			}
		...
		public:
			SubsetBinary(const TypedVariable<A>& a, const TypedVariable<B>& b, const TypedVariable<R>& r, const fun_type& f)
				: a(a), b(b), r(r), f(f) {}
			Variables getInputs() const {
				return toVector<Variable>(a,b);
			}
	...
	template<typename A, typename L>
	typename std::enable_if<!std::is_base_of<TypedVariable<L>,A>::value, ConstraintPtr>::type
	subset(const A& a, const TypedVariable<L>& b) {
		return combine(detail::TrueFilter(), e_in(a,b));
	}
	...
	ConstraintPtr subsetIf(const A& a, const TypedVariable<L1>& as, const B& b, const TypedVariable<L2>& bs, const TypedVariable<C>& in, const TypedVariable<C>& out) {
	...
	ConstraintPtr subsetIfReducedBigger(const TypedVariable<SetLattice<E>>& a, const E& e, std::size_t s, const TypedVariable<A>& b, const TypedVariable<A>& c) {
