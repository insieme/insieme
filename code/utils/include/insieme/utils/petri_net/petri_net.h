...
template<typename Place, typename Transition> class PetriNet;
template<typename Place, typename Transition> class Marking;
...
template<
    ...
    >
class Marking :
	public boost::equality_comparable<Marking<Place,Transition>>,
...
public:
	...
	Marking& fireTransition(const T& t) {
		...
		for_each(net->getPrePlaces(t),  [&](place_idx p) {
			--marking[p];
		});
		for_each(net->getPostPlaces(t), [&](place_idx p) {
			++marking[p];
		});
		return *this;
	}
