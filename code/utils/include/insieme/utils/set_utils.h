...
namespace set {
	...
	class PointerSet : public boost::unordered_set<Ptr, hash_target<Ptr>, equal_target<Ptr>> {
	  ...
	  public:
		 ...
		 */
		PointerSet(std::initializer_list<Ptr> list) : boost::unordered_set<Ptr, hash_target<Ptr>, equal_target<Ptr>>(list.begin(), list.end()) {}
	...
	Set intersect(const Set& setA, const Set& setB) {
		...
		std::for_each(setA.cbegin(), setA.cend(), [&res, &setB](const Element& cur) {
			if(setB.find(cur) != setB.cend()) { res.insert(cur); }
		});
		return res;
	}
	...
	bool equal(const SetA& setA, const SetB& setB) {
		...
		return all(setA.cbegin(), setA.cend(), [&setB](const Element& cur) { return setB.find(cur) != setB.cend(); });
	}
	...
	std::size_t computeHash(const Set& set, Hasher hasher) {
		...
		std::size_t seed = 0;
		std::for_each(set.cbegin(), set.cend(), [&seed, &hasher](const Element& cur) { seed += hasher(cur); });
		return seed;
	}
