...
namespace utils {
	...
	struct Functional : std::function<const RetTy&(void)> {
		...
		Functional(const FuncTy& f)
		    : std::function<const RetTy&(void)>(f) {}
	};
	/**
	...
	struct Action : public LazyFuncType {
		...
		Action(Input& inBuf, Output& outBuf,
		       const std::function<typename std::tuple_element<RetPos, Output>::type(const typename std::tuple_element<ArgsPos, Input>::type&...)>& functor)
		    : LazyFuncType([&inBuf, &outBuf, functor](void) -> void { std::get<RetPos>(outBuf) = functor(std::get<ArgsPos>(inBuf)...); }) {}
	};
	...
	struct Stage : public boost::noncopyable, utils::Printable {
		...
		Stage(const Units&... units)
		    : inBuf(std::make_shared<in_buff>()), outBuf(std::make_shared<out_buff>()), functors({std::bind(units)...}) {}

		Stage(const in_buff_ptr& in, const out_buff_ptr& out)
		    : inBuf(in ? in : std::make_shared<in_buff>()), outBuf(out ? out : std::make_shared<out_buff>()) {}

		template <class FuncTy, unsigned RetPos, unsigned... ArgPos>
		void add(const InOut<RetPos, ArgPos...>& pos, const FuncTy& f) {
			std::function<typename std::tuple_element<RetPos, out_buff>::type(const typename std::tuple_element<ArgPos, in_buff>::type&...)> func = f;
		...
		template <class... Args, typename std::enable_if<std::is_same<std::tuple<Args...>, InTuple>::value, bool>::type = 0>
		...
		const out_buff& out_buffer() const {
			return *outBuf;
		}

		out_buff_ptr& out_buffer_ptr() {
			return outBuf;
		}
		const out_buff_ptr& out_buffer_ptr() const {
			return outBuf;
		}
		...
		const in_buff& in_buffer() const {
			return *inBuf;
		}

		in_buff_ptr& in_buffer_ptr() {
			return inBuf;
		}
		const in_buff_ptr& in_buffer_ptr() const {
			return inBuf;
		}

		std::ostream& printTo(std::ostream& out) const {
			return out << "Pipeline.Stage[" << std::tuple_size<InTuple>::value << ":" << std::tuple_size<OutTuple>::value << "] - F(" << functors.size() << ")";
		}
	...
	struct FunctorPtr : public std::shared_ptr<Functor<InTuple, OutTuple>>, utils::Printable {
		...
		FunctorPtr(const std::shared_ptr<value_type>& other) : std::shared_ptr<value_type>(other) {}
	...
	struct Pipeline : public Functional<OutTuple>, utils::Printable, boost::noncopyable {
		...
		template <class... Stages, typename std::enable_if<std::is_same<out_buff, typename details::last<Stages...>::value::value_type::out_buff>::value
		...
		const in_buff& in_buffer_ptr() const {
			return *inBuf;
		}
		...
		std::ostream& printTo(std::ostream& out) const {
			return out << "Pipeline[" << std::tuple_size<InTuple>::value << ":" << std::tuple_size<OutTuple>::value << "]";
		}
	...
	template <class InTuple, class InnerTuple, class OutTuple, template <class, class> class Unit1, template <class, class> class Unit2>
	FunctorPtr<InTuple, OutTuple, Pipeline> operator>>(const FunctorPtr<InTuple, InnerTuple, Unit1>& s1, const FunctorPtr<InnerTuple, OutTuple, Unit2>& s2) {
		// Bind the pipelines

		FunctorPtr<InnerTuple, InnerTuple, Stage> conn(std::make_shared<Stage<InnerTuple, InnerTuple>>(s1->out_buffer_ptr(), s2->in_buffer_ptr()));

		add_in_connector<InnerTuple, InnerTuple, 0, 0, std::tuple_size<InnerTuple>::value - 1>()(conn);

		auto&& s = FunctorPtr<InTuple, OutTuple, Pipeline>(std::make_shared<Pipeline<InTuple, OutTuple>>(s1, conn, s2));

		return s;
	}

	template <class... InTuple1, class... OutTuple1, class... InTuple2, class... OutTuple2, template <class, class> class Unit1,
	          template <class, class> class Unit2>
...
#define OPERATOR(SYM, FUNC)                                                                                                                                    \
	template <class InTuple1, class InTuple2, class OutTuple, template <class, class> class Unit1, template <class, class> class Unit2>                        \
	insieme::utils::FunctorPtr<std::tuple<InTuple1, InTuple2>, std::tuple<OutTuple>, insieme::utils::Pipeline> operator SYM(                                   \
