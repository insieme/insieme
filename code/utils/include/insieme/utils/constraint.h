...
inline ConstraintType getLogicNegation(const ConstraintType& c) {
	...
	case ConstraintType::GE:
		return ConstraintType::LT;
		
	default:
		assert_fail() << "Constraint type not supported";
	}
...
struct Constraint :
	...
	inline bool isTrue() const {
		if(!isEvaluable()) {
			throw std::logic_error("ERROR: not evaluatable!"); /* FIXME: introduce an exception for this */
		}
...
inline CombinerPtr<FuncTy> toConstraint(const std::vector<std::vector<CombinerPtr<FuncTy>>>& disjunctions) {
	...
	for_each(disjunctions, [&](const std::vector<CombinerPtr<FuncTy>>& cur) {
		CombinerPtr<FuncTy> sub;
		for_each(cur, [&](const CombinerPtr<FuncTy>& cur) {
