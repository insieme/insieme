...
bool LazySolver::hasUnresolvedInput(const Constraint& cur) {
	...
	// check dependencies
	bool hasUnresolvedInputs = any(cur.getInputs(), [&](const Variable& cur) {
		return resolved.find(cur) == resolved.end();
	});
	if(!hasUnresolvedInputs) {
		resolvedConstraints.insert(&cur);
	}
