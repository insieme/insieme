...
namespace {
	...
	void rewriterFile(Rewriter::CodeModificationList::const_iterator& modIt, const Rewriter::CodeModificationList::const_iterator& end) {
		...
		while(modIt != end && modIt->getFileName() == currFile) {
			...
			case Rewriter::CodeModification::REMOVE: columns = skipLines(modIt->getEndLoc().getLine()); break;
			case Rewriter::CodeModification::INSERT: insertCode(modIt->getCode()); break;
			...
			if(columns) {
				size_t endLoc = 0;
				if(modIt->getEndLoc().getColumn() > 0) { endLoc = std::min(modIt->getEndLoc().getColumn() - 1, columns); }
...
namespace utils {
	...
	//							CodeModification
	//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
	Rewriter::CodeModification::CodeModification(const SourceLocation& locStart, const SourceLocation& locEnd, const std::string& code,
	...
	Rewriter::CodeModification::CodeModification(const SourceLocation& loc, const std::string& code)
	    : fileName(loc.getFileName()), locStart(loc), locEnd(loc), code(code), type(CodeModification::INSERT) {}
