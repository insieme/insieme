...
polyhedron. */
class Element: public utils::Printable, public boost::equality_comparable<Element> {
public:
...
private:
	...
	mutable int freshVarCounter;		// counter for fresh variable IDs

	template <class T> boost::optional<unsigned int> getIdxFrom(const T& elem, const std::vector<T>& vec) const;
	template <class T> unsigned int addTo(const T& elem, std::vector<T>& vec);

public:
	unsigned int add(const Element& e);
	boost::optional<unsigned int> getIdx(const Element& elem) const;
	boost::optional<unsigned int> getIdx(const core::ExpressionPtr& var) const;
	...
	/// Returns an iterator over the iterators of this iteration vector: (iter0, iter1, ... iterN)
	inline iter_iterator iter_begin() const { return iters.begin(); }
	inline iter_iterator iter_end() const { return iters.end(); }

	/// Returns an iterator over the parameters of this iteration vector: (param0, param1, ... paramM)
...
/// Merges two iteration vectors (a and b) to create a new iteration vector which contains both the elements of a and b.
