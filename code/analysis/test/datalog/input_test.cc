...
namespace analysis {
	...
	class ActualTest {
		...
		bool isTrue(const core::ExpressionAddress& x) {
			return insieme::analysis::isTrue<Backend>(ctxt,x);
		}

		bool isFalse(const core::ExpressionAddress& x) {
			return insieme::analysis::isFalse<Backend>(ctxt,x);
		}

		bool mayBeTrue(const core::ExpressionAddress& x) {
			return insieme::analysis::mayBeTrue<Backend>(ctxt,x);
		}

		bool mayBeFalse(const core::ExpressionAddress& x) {
			return insieme::analysis::mayBeFalse<Backend>(ctxt,x);
		}
		...
		ArithmeticSet getValue(const core::ExpressionAddress& x) {
			...
			const IntegerSet& is = insieme::analysis::getIntegerValues<Backend>(x);
			return insieme::analysis::IntegerToArithmeticSet(is);
		}
