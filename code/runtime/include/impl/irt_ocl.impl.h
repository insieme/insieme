...
void irt_ocl_init_devices() {
	...
	if(universe_size == 1) {
		IRT_INFO("No room for new workers");
	}
...
void irt_ocl_print_device_infos(irt_ocl_device* device) {
	...
	if(node == 0) {
		...
		for(cl_uint i = 0; i < ldev->max_work_item_dimensions; i++) {
			...
			if(i < ldev->max_work_item_dimensions - 1) {
				IRT_INFO(", ");
			}
		}
		IRT_INFO("]\n");
		...
		if(ldev->single_fp_config & CL_FP_DENORM) {
			IRT_INFO(" denorm");
		}
		if(ldev->single_fp_config & CL_FP_INF_NAN) {
			IRT_INFO(" inf_nan");
		}
		if(ldev->single_fp_config & CL_FP_ROUND_TO_NEAREST) {
			IRT_INFO(" round_to_nearest");
		}
		if(ldev->single_fp_config & CL_FP_ROUND_TO_ZERO) {
			IRT_INFO(" round_to_zero");
		}
		if(ldev->single_fp_config & CL_FP_ROUND_TO_INF) {
			IRT_INFO(" round_to_inf");
		}
		if(ldev->single_fp_config & CL_FP_FMA) {
			IRT_INFO(" fma");
		}
		IRT_INFO("\n");
...
inline static cl_uint _irt_cl_get_num_platforms() {
	...
	if(clGetPlatformIDs(0, NULL, &cl_num_platforms) != CL_SUCCESS) {
		return 0;
	}
...
static cl_uint _irt_cl_get_num_devices(cl_platform_id* platform, cl_device_type device_type) {
	...
	if(clGetDeviceIDs(*platform, device_type, 0, NULL, &cl_num_devices) != CL_SUCCESS) {
		return 0;
	}
