...
// static double _irt_cl_profile_events(cl_event event_one, cl_profiling_info event_one_command, cl_event event_two, cl_profiling_info event_two_command,
...
void irt_ocl_init_devices() {
	...
	if(local_num_devices == 0) { irt_throw_string_error(IRT_ERR_OCL, "No OCL devices present on system."); }
...
irt_ocl_buffer* irt_ocl_create_buffer(irt_ocl_device* device, irt_ocl_mem_flag flags, size_t size) {
	...
	if(node == 0) {
		...
		#endif
		if(size > ldev->mem_available) { IRT_ASSERT(false, IRT_ERR_OCL, "Error creating buffer: \"Not enough memory available\""); }
...
void irt_ocl_create_kernel(irt_ocl_device* device, irt_ocl_kernel* kernel, const char* file_name, const char* kernel_name, const char* build_options,
...
void irt_ocl_rt_create_all_kernels(irt_context* context, irt_ocl_kernel_code* g_kernel_code_table, uint32_t num_kernels) {
	...
	for(uint32_t i = 0; i < num_devices; ++i) {
		...
		for(uint32_t j = 0; j < num_kernels; ++j) {
			irt_ocl_create_kernel(dev, &(context->kernel_binary_table[i][j]), g_kernel_code_table[j].code, g_kernel_code_table[j].kernel_name, "",
			                      IRT_OCL_STRING);
		}
	}
...
void irt_ocl_rt_run_kernel(uint32_t kernel_id, uint32_t work_dim, size_t* global_work_offset, size_t* global_work_size, size_t* local_work_size,
                           uint32_t num_args, ...) {
	int worker_id = irt_worker_get_current()->id.thread % irt_ocl_get_num_devices();
	...
	if(node == 0) {
		...
		if(lkernel->type == IRT_OCL_NDRANGE) {
		#ifdef IRT_OCL_INSTR
			irt_ocl_local_event* rt_event = irt_ocl_get_new_event();
			err_code = clEnqueueNDRangeKernel((lkernel->local_dev)->queue, lkernel->cl_ker, lkernel->work_dim, lkernel->global_work_offset,
			                                  lkernel->global_work_size, lkernel->local_work_size, 0, NULL, &(rt_event->cl_event));
			#else
			err_code = clEnqueueNDRangeKernel((lkernel->local_dev)->queue, lkernel->cl_ker, lkernel->work_dim, lkernel->global_work_offset,
			                                  lkernel->global_work_size, lkernel->local_work_size, 0, NULL, NULL);
			#endif
			...
			IRT_ASSERT(false, IRT_ERR_OCL, "Kernel Type Not Valid");
		}
...
irt_ocl_local_event* irt_ocl_get_new_event() {
	...
	if(table->num_events >= table->size) { _irt_cl_event_table_resize(table); }
...
static const char* _irt_error_string(cl_int errcode) {
	...
	case CL_EXEC_STATUS_ERROR_FOR_EVENTS_IN_WAIT_LIST: return "Status Error for events in wait list";
