...
inline static void irt_schedule_loop_fixed(irt_work_item* self, uint32 id, irt_work_item_range base_range,
	...
	if(id>0) {
		base_range.begin = sched_data->policy.param.boundaries[id-1];
	}
	if(id<sched_data->policy.participants-1) {
		base_range.end = sched_data->policy.param.boundaries[id];
	}
...
inline static void irt_schedule_loop_shares(irt_work_item* self, uint32 id, irt_work_item_range base_range,
	...
	if(id<sched_data->policy.participants-1) {
		base_range.end = start + pos*extent;
	}
...
inline static void irt_schedule_loop(
	...
	if(group->pfor_count < mem->pfor_count) {
		...
		case IRT_SHARES:
			break;
		case IRT_DYNAMIC:
			irt_schedule_loop_dynamic_prepare(sched_data, base_range);
			break;
		case IRT_DYNAMIC_CHUNKED:
		case IRT_DYNAMIC_CHUNKED_COUNTING:
			irt_schedule_loop_dynamic_chunked_prepare(sched_data, base_range);
			break;
		case IRT_GUIDED:
			irt_schedule_loop_guided_prepare(sched_data, base_range);
			break;
		case IRT_GUIDED_CHUNKED:
			irt_schedule_loop_guided_chunked_prepare(sched_data, base_range);
			break;
		default:
			IRT_ASSERT(false, IRT_ERR_INTERNAL, "Unknown scheduling policy");
		}
	...
	if(mem->num >= sched_data->policy.participants) {
		return;
	}
	...
	switch(sched_data->policy.type) {
	case IRT_STATIC:
		irt_schedule_loop_static(self, mem->num, base_range, impl, args, (const irt_loop_sched_policy*)&sched_data->policy);
		break;
	case IRT_STATIC_CHUNKED:
		irt_schedule_loop_static_chunked(self, mem->num, base_range, impl, args, (const irt_loop_sched_policy*)&sched_data->policy);
		break;
	case IRT_DYNAMIC:
	case IRT_DYNAMIC_CHUNKED:
		irt_schedule_loop_dynamic_chunked(self, mem->num, base_range, impl, args, sched_data);
		break;
	case IRT_DYNAMIC_CHUNKED_COUNTING:
		irt_schedule_loop_dynamic_chunked_counting(self, mem->num, base_range, impl, args, sched_data);
		break;
	case IRT_GUIDED:
	case IRT_GUIDED_CHUNKED:
		irt_schedule_loop_guided_chunked(self, mem->num, base_range, impl, args, sched_data);
		break;
	case IRT_FIXED:
		irt_schedule_loop_fixed(self, mem->num, base_range, impl, args, sched_data);
		break;
	case IRT_SHARES:
		irt_schedule_loop_shares(self, mem->num, base_range, impl, args, sched_data);
		break;
	default:
		IRT_ASSERT(false, IRT_ERR_INTERNAL, "Unknown scheduling policy");
	}
...
void irt_loop_sched_policy_init() {
	...
	irt_g_loop_sched_policy_single = (irt_loop_sched_policy) {
		IRT_DYNAMIC_CHUNKED, IRT_SANE_PARALLEL_MAX, { 1000 }
	};
}
