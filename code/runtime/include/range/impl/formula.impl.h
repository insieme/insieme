...
uint64 irt_range_formula_1d_cardinality(irt_range_formula_1d* a) {
	...
	tmp = (irt_range_formula_1d) {
		a->num_terms - 1, intersected
	};
...
static irt_int3_list* _irt_range_term_diff(
	...
	if(intersection.a == 0 && intersection.b == 0) {	// intersection is empty => return a
		...
		res->num_entries = 1;
		res->entries[0] = (irt_int3) {
			a_start, a_end, a_step
		};
		return res;
	}
	...
	if(a_end > b_start) {
		res->entries[counter++] = (irt_int3) {
			a_start, b_start, a_step
		};
	}
	else {
	...
	for(int i=0; i<a->num_terms; i++) {
		...
		for(int j=0; j<b->num_terms; j++) {
			...
			for(int k=0; k<diffList->num_entries; k++) {
				cur->terms[k] = (irt_range_term_1d) {
					diffList->entries[k].a, diffList->entries[k].b, diffList->entries[k].c
				};
			}
