...
uint64 countMembers(irt_range_formula_1d *f, int lb = 0, int ub = 30) {
	uint64 count = 0;
	for(int i=lb; i<ub; ++i) {
		if(irt_range_formula_1d_contains(f, irt_range_point_1d_create(i))) {
			count++;
		}
	}
	return count;
		...
		for(int j=0; j<30; j++) {
			for(int k=1; k<30; k++) {
				...
				for(int h=0; h<30; h++) {
					...
					ASSERT_EQ(
					    irt_range_formula_1d_contains(a,p) && !irt_range_formula_1d_contains(b,p),
					    irt_range_formula_1d_contains(c,p)
					);
					if(irt_range_formula_1d_contains(c,p)) {
						count++;
					}
				}
...
//					if (irt_range_formula_2d_contains(a,p) && !irt_range_formula_2d_contains(b,p) != irt_range_formula_2d_contains(c,p)) {
...
//					}
									...
									ASSERT_EQ(
									    irt_range_formula_2d_contains(a,p) && !irt_range_formula_2d_contains(b,p),
									    irt_range_formula_2d_contains(c,p)
									);
									if(irt_range_formula_2d_contains(c,p)) {
										count++;
									}
								}
							}
