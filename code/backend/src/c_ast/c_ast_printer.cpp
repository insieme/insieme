...
#define PRINT(name) std::ostream& print ## name (name ## Ptr node, std::ostream& out)
	...
	PRINT(ModifiedType) {
		if(node->hasMod(ModifiedType::VOLATILE)) {
			out << "volatile ";
		}
		if(node->hasMod(ModifiedType::CONST)) {
			out << "const ";
		}
	...
	PRINT(PrimitiveType) {
		...
		case PrimitiveType::UInt128 :
			return out << "uint128_t";
		case PrimitiveType::Float :
			return out << "float";
		case PrimitiveType::Double :
			return out << "double";
		case PrimitiveType::LongLong :
			return out << "long long";
	...
	PRINT(NamedType) {
		...
		if(!node->parameters.empty()) {
			out << "<" << join(", ", node->parameters, [&](std::ostream& out, const NodePtr& cur) {
				out << print(cur);
			}) << " >";
		}
		return out;
...
//					}
	...
	PRINT(Initializer) {
		if(!makeTypesImplicit && node->explicitType) {
			out << "(" << print(node->type) << ")";
		}
		return out << "{"
	...
	PRINT(UnaryOperation) {
		...
		case UnaryOperation::UnaryPlus:
			return out << "+" << print(node->operand);
		case UnaryOperation::UnaryMinus:
			return out << "-" << print(node->operand);
		case UnaryOperation::PrefixInc:
			return out << "++" << print(node->operand);
		case UnaryOperation::PrefixDec:
			return out << "--" << print(node->operand);
		case UnaryOperation::PostfixInc:
			return out << print(node->operand) << "++";
		case UnaryOperation::PostfixDec:
			return out << print(node->operand) << "--";
		case UnaryOperation::LogicNot:
			return out << "!" << print(node->operand);
		case UnaryOperation::BitwiseNot:
			return out << "~" << print(node->operand);
		case UnaryOperation::Indirection:
			return out << "*" << print(node->operand);
		case UnaryOperation::Reference:
			return out << "&" << print(node->operand);
		case UnaryOperation::SizeOf:
			return out << "sizeof(" << print(node->operand) << ")";
		case UnaryOperation::Typeid:
			return out << "typeid(" << print(node->operand) << ")";
		case UnaryOperation::New:
			return out << "new " << print(node->operand);
		case UnaryOperation::Delete:
			return out << "delete " << print(node->operand);
		case UnaryOperation::DeleteArray:
			return out << "delete[] " << print(node->operand);
		case UnaryOperation::ComplexReal:
			return out << "__real__ " << print(node->operand);
		case UnaryOperation::ComplexImag:
			return out << "__imag__ " << print(node->operand);
		}
	...
	PRINT(BinaryOperation) {
		...
		case BinaryOperation::BitwiseXOr:
			op = "^";
			break;
		case BinaryOperation::BitwiseLeftShift:
			op = "<<";
			break;
		case BinaryOperation::BitwiseRightShift:
			op = ">>";
			break;
		case BinaryOperation::AdditionAssign:
			op = "+=";
			break;
		case BinaryOperation::SubtractionAssign:
			op = "-=";
			break;
		case BinaryOperation::MultiplicationAssign:
			op = "*=";
			break;
		case BinaryOperation::DivisionAssign:
			op = "/=";
			break;
		...
		case BinaryOperation::BitwiseXOrAssign:
			op = "^=";
			break;
		case BinaryOperation::BitwiseLeftShiftAssign:
			op = "<<=";
			break;
		case BinaryOperation::BitwiseRightShiftAssign:
			op = ">>=";
			break;
		case BinaryOperation::MemberAccess:
			op = ".";
			break;
		case BinaryOperation::IndirectMemberAccess:
			op = "->";
			break;
		...
		case BinaryOperation::Subscript:
			return out << print(node->operandA) << "[" << print(node->operandB) << "]";
		case BinaryOperation::Cast:
			return out << "(" << print(node->operandA) << ")" << print(node->operandB);
			
		case BinaryOperation::StaticCast:
			return out << "static_cast<"  << print(node->operandA) << ">(" << print(node->operandB) << ")";
		case BinaryOperation::DynamicCast:
			return out << "dynamic_cast<" << print(node->operandA) << ">(" << print(node->operandB) << ")";
			
		case BinaryOperation::ScopeResolution:
			op = "::";
			break;
		case BinaryOperation::PointerToMember:
			op = "->*";
			break;
		}
	...
	PRINT(TypeDefinition) {
		...
		if(structType && !structType->parents.empty()) {
			out << " : public " << join(", public ", structType->parents, [&](std::ostream& out, const ParentPtr& cur) {
				out << print(cur);
			});
		}
	...
	PRINT(Constructor) {
		...
		if(!node->initialization.empty()) {
			out << ": " << join(", ", node->initialization, [&](std::ostream& out, const Constructor::InitializerListEntry& cur) {
				out << print(cur.first) << "(" << join(", ", cur.second, [&](std::ostream& out, const NodePtr& cur) {
					out << print(cur);
				}) << ")";
			}) << " ";
		}
