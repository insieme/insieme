...
public:
	 ...
	 */
	VariableCollector(const utils::set::PointerSet<core::VariablePtr>& excluded, vector<core::VariablePtr>& list) : core::IRVisitor<>(false), excluded(excluded),
		list(list) {}
 ...
 */
vector<core::VariablePtr> getVariablesToBeCaptured(const core::ExpressionPtr& code,
...
void collectSubMetaInfos(core::NodeManager& manager, core::NodePtr job) {
	...
	core::visitDepthFirstPrunable(job, [&](const core::NodePtr& npr) -> bool {
		...
		assert(::all(rootMetas, [&](const annotations::AnnotationMap::value_type& v) {
			return metas.count(v.first) == 0;
...
//				irt_work_item* irt_pfor(irt_work_item* self, irt_work_group* group, irt_work_item_range range, irt_wi_implementation_id impl_id, irt_lw_data_item* args);
	...
	core::StatementPtr convertVariant(const core::CallExprPtr& call) {
		...
		if(pickCall->getType()->getNodeType() != core::NT_FunctionType) {
			return call;
		}
		...
		if(pickCall->hasAttachedValue<PickImplementationHint>()) {
			...
			switch(implHint) {
			case PickImplementationHint::CALL:
				return convertVariantToCall(call);
			case PickImplementationHint::SWITCH:
				return convertVariantToSwitch(call);
			default:
				assert_fail() << "Invalid variant implementation hint";
				break;
			}
		}
		// default to switch
