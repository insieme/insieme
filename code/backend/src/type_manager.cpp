...
template<> struct info_trait<core::Type> {
	typedef TypeInfo type;
};
template<> struct info_trait<core::GenericType> {
	typedef TypeInfo type;
};
template<> struct info_trait<core::RefType> {
	typedef RefTypeInfo type;
};
template<> struct info_trait<core::StructType> {
	typedef StructTypeInfo type;
};
template<> struct info_trait<core::TupleType> {
	typedef StructTypeInfo type;
};
template<> struct info_trait<core::UnionType> {
	typedef UnionTypeInfo type;
};
template<> struct info_trait<core::ArrayType> {
	typedef ArrayTypeInfo type;
};
template<> struct info_trait<core::VectorType> {
	typedef VectorTypeInfo type;
};
template<> struct info_trait<core::ChannelType> {
	typedef ChannelTypeInfo type;
};
template<> struct info_trait<core::FunctionType> {
	typedef FunctionTypeInfo type;
};
template<> struct info_trait<core::RecType> {
	typedef TypeInfo type;
};
...
public:
	...
	const c_ast::CodeFragmentPtr getDefinitionOf(const c_ast::TypePtr& type) const {
		auto pos = std::find_if(allInfos.begin(), allInfos.end(), [&](const TypeInfo* cur) {
			return *cur->rValueType == *type;
		});
		if(pos != allInfos.end()) {
...
const TypeInfo* TypeInfoStore::resolveInternal(const core::TypePtr& type) {
	...
	if(type->getNodeType() == core::NT_GenericType && annotations::c::hasIncludeAttached(type) && !core::annotations::hasAttachedName(type)
	        && !gen.isIRBuiltin(type)) {
	...
	case core::NT_GenericType:
		info = resolveGenericType(static_pointer_cast<const core::GenericType>(type));
		break;
	case core::NT_StructType:
		info = resolveStructType(static_pointer_cast<const core::StructType>(type));
		break;
	case core::NT_UnionType:
		info = resolveUnionType(static_pointer_cast<const core::UnionType>(type));
		break;
	case core::NT_TupleType:
		info = resolveTupleType(static_pointer_cast<const core::TupleType>(type));
		break;
	case core::NT_FunctionType:
		info = resolveFunctionType(static_pointer_cast<const core::FunctionType>(type));
		break;
	case core::NT_VectorType:
		info = resolveVectorType(static_pointer_cast<const core::VectorType>(type));
		break;
	case core::NT_ArrayType:
		info = resolveArrayType(static_pointer_cast<const core::ArrayType>(type));
		break;
	case core::NT_RefType:
		info = resolveRefType(static_pointer_cast<const core::RefType>(type));
		break;
...
//			case core::NT_ChannelType:
	case core::NT_TypeVariable:
		info = resolveTypeVariable(static_pointer_cast<const core::TypeVariable>(type));
		break;
	default:
...
const RefTypeInfo* TypeInfoStore::resolveRefType(const core::RefTypePtr& ptr) {
	...
	if(elementNodeType == core::NT_VectorType) {
		res->externalize = [res,subType](const c_ast::SharedCNodeManager& manager, const c_ast::ExpressionPtr& node) {
			...
			if(node->getNodeType() == c_ast::NT_Literal && static_pointer_cast<const c_ast::Literal>(node)->value[0] == '\"') {
				return node;
			}
