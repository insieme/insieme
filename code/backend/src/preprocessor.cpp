...
namespace backend {
	...
	core::NodePtr InitGlobals::process(const Converter& converter, const core::NodePtr& code) {
		...
		for(const auto& cur : inits) {
			...
			if(cur.second[1].isa<core::VectorExprPtr>()) { continue; }
	...
	class VectorToArrayConverter : public core::transform::CachedNodeMapping {
	  ...
	  public:
		...
		const core::NodePtr resolveElement(const core::NodePtr& ptr) {
			...
			// handle calls
			if(ptr->getNodeType() == core::NT_CallExpr) { res = handleCallExpr(core::static_pointer_cast<const core::CallExpr>(res)); }
	  ...
	  private:
		...
		core::CallExprPtr handleCallExpr(core::CallExprPtr call) {
			...
			for(std::size_t i = 0; !conversionRequired && i < size; i++) {
				conversionRequired =
				    conversionRequired || (args[i]->getType()->getNodeType() == core::NT_VectorType && paramTypes[i]->getNodeType() != core::NT_VectorType);
			}
			...
			for(unsigned i = 0; i < size; i++) {
				...
				if(*newParamTypes[i] == *argTypes[i]) { continue; }
	...
	core::NodePtr RecursiveLambdaInstantiator::process(const Converter& converter, const core::NodePtr& code) {
		...
		visitDepthFirstOnce(code, [&](const core::TypePtr& t) {
			if(core::hasMetaInfo(t)) {
				...
				auto pre = metaEnc;
				LOG(DEBUG) << "META INFO PRE: elem? " << core::analysis::contains(metaEnc, elem) << "\n" << dumpColor(metaEnc) << "META INFO PRE TEXT:\n"
				           << dumpText(metaEnc) << "\n";
				metaEnc = core::transform::instantiateTypes(metaEnc, isLangOrOpBuiltin);
				LOG(DEBUG) << "META INFO POST: elem? " << core::analysis::contains(metaEnc, elem) << "\n" << dumpColor(metaEnc) << "META INFO POST TEXT:\n"
				           << dumpText(metaEnc) << "\n";
