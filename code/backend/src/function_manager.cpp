...
template<> struct info_trait<core::Literal> {
	typedef FunctionInfo type;
};
template<> struct info_trait<core::LambdaExpr> {
	typedef LambdaInfo type;
};
template<> struct info_trait<core::BindExpr> {
...
bool FunctionManager::isBuiltIn(const core::NodePtr& op) const {
	if(op->getNodeCategory() != core::NC_Expression) {
		return false;
	}
...
core::StructTypePtr getClassType(const core::FunctionTypePtr& funType) {
	...
	if(auto recType = type.isa<core::RecTypePtr>()) {
		type = recType->unroll();
	}
...
ElementInfo* FunctionInfoStore::resolveInternal(const core::ExpressionPtr& expr, bool isConst, bool isVirtual) {
	...
	case core::NT_Literal:
		info = resolveLiteral(static_pointer_cast<const core::Literal>(expression), isConst);
		break;
	case core::NT_LambdaExpr:
		info = resolveLambda(static_pointer_cast<const core::LambdaExpr>(expression), isConst, isVirtual);
		break;
	case core::NT_BindExpr:
		info = resolveBind(static_pointer_cast<const core::BindExpr>(expression));
		break;
	default:
...
ElementInfo* FunctionInfoStore::resolveLiteral(const core::LiteralPtr& literal, bool isConst) {
	...
	if(header) {
		...
		if(annotations::c::isExternC(literal)) {
			code = manager->create<c_ast::ExternC>(code);
		}
	...
	for_each(lambdas, [&](const std::pair<c_ast::IdentifierPtr,core::LambdaExprPtr>& pair) {
		...
		if(isMember) {
			...
			for(const auto& dep : codeInfo.prototypeDependencies) {
				if(dep.isa<c_ast::IncludeFragmentPtr>()) {
					dep->remDependency(info->prototype);
				}
			}
...
core::NodePtr getAccessedField(const core::VariablePtr& thisVar, const core::ExpressionPtr& candidate) {
	...
	if(candidate->getNodeType() != core::NT_CallExpr) {
		return NO_ACCESS;
	}
	...
	if(core::analysis::isCallOf(call, basic.getRefAssign())) {
		...
		if(core::analysis::isCallOf(target, basic.getCompositeRefElem())) {
			...
			if(deref->getArgument(0) != thisVar) {
				return NO_ACCESS;
			}
...
bool valuesDerivedFromParametersOnly(const core::VariablePtr& thisVar, const core::VariableList& params, const core::CallExprPtr& call) {
	...
	for(const auto& cur : values) {
		if(parametersOnly) {
			core::visitDepthFirstOncePrunable(cur, check);
		}
	}
...
struct FirstWriteCollector : public
	core::IRVisitor<void, core::Address, const core::VariablePtr&, const core::VariableList&, core::NodeSet&, std::vector<core::StatementAddress>&, bool> {
	...
	void visitCompoundStmt(const core::CompoundStmtAddress& cur, const core::VariablePtr& thisVar, const core::VariableList& params, core::NodeSet& touched,
	...
	void visitSwitchStmt(const core::SwitchStmtAddress& cur, const core::VariablePtr& thisVar, const core::VariableList& params, core::NodeSet& touched,
	                     std::vector<core::StatementAddress>& res, bool iterating) {
		// iterate through sub-statements
		visitAll(cur->getChildList(), thisVar, params, touched, res, iterating);
	}
	
	void visitSwitchCases(const core::SwitchCasesAddress& cur, const core::VariablePtr& thisVar, const core::VariableList& params, core::NodeSet& touched,
	                      std::vector<core::StatementAddress>& res, bool iterating) {
		// iterate through sub-statements
		visitAll(cur->getChildList(), thisVar, params, touched, res, iterating);
	}
	
	void visitSwitchCase(const core::SwitchCaseAddress& cur, const core::VariablePtr& thisVar, const core::VariableList& params, core::NodeSet& touched,
	                     std::vector<core::StatementAddress>& res, bool iterating) {
		// iterate through sub-statements
		visitAll(cur->getChildList(), thisVar, params, touched, res, iterating);
	}
	...
	void visitWhileStmt(const core::WhileStmtAddress& cur, const core::VariablePtr& thisVar, const core::VariableList& params, core::NodeSet& touched,
	                    std::vector<core::StatementAddress>& res, bool iterating) {
		// iterate through sub-statements
		visitAll(cur->getChildList(), thisVar, params, touched, res, true);
	}
	
	void visitDeclarationStmt(const core::DeclarationStmtAddress& cur, const core::VariablePtr& thisVar, const core::VariableList& params, core::NodeSet& touched,
	                          std::vector<core::StatementAddress>& res, bool iterating) {
		// we can stop here
	}
	
	void visitTryCatchStmt(const core::TryCatchStmtAddress& cur, const core::VariablePtr& thisVar, const core::VariableList& params, core::NodeSet& touched,
	                       std::vector<core::StatementAddress>& res, bool iterating) {
		// we can stop here
	}
	...
	void visitCallExpr(const core::CallExprAddress& cur, const core::VariablePtr& thisVar, const core::VariableList& params, core::NodeSet& touched,
		...
		if(!valuesDerivedFromParametersOnly(thisVar, params, cur)) {
			return;
		}
	...
	void visitStatement(const core::StatementAddress& cur, const core::VariablePtr& thisVar, const core::VariableList& params, core::NodeSet& touched,
	                    std::vector<core::StatementAddress>& res, bool iterating) {
...
std::pair<c_ast::Constructor::InitializationList,core::CompoundStmtPtr> extractInitializer(const Converter& converter, const core::LambdaPtr& ctor,
	...
	if(firstWriteOps.empty()) {
		return std::make_pair(initializer, oldBody);
	}
...
FunctionInfoStore::resolveLambdaWrapper(const c_ast::FunctionPtr& function, const core::FunctionTypePtr& funType, bool external) {
	...
	core::FunctionTypePtr closureType = core::FunctionType::get(funType->getNodeManager(), funType->getParameterTypes(), funType->getReturnType(),
	                                    core::FK_CLOSURE);
	TypeManager& typeManager = converter.getTypeManager();
