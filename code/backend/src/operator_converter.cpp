 ...
 * expression.
 */
c_ast::ExpressionPtr expand(ConversionContext& context, const c_ast::ExpressionPtr& src, const core::ExpressionPtr& dataPath, const core::TypePtr& resType,
...
#include "insieme/backend/operator_converter_begin.inc"
	...
	auto refNewLoc = OP_CONVERTER({
		...
		if(core::types::isVariableSized(resType->getElementType())) {
			...
			return c_ast::cast(CONVERT_TYPE(resType), c_ast::call(C_NODE_MANAGER->create("memcpy"), malloc, c_ast::ref(CONVERT_EXPR(initValue)),
			c_ast::sizeOf(c_struct_type)));
		}
