...
//	std::cout << stmt2 << std::endl;
	...
	EXPECT_EQ("[]", toString(check(stmt2, all)));
	EXPECT_PRED2(containsSubString, toString(printer::PrettyPrinter(stmt2, printer::PrettyPrinter::NO_LET_BINDINGS)),
	             "decl ref<struct<kernel:vector<ref<(ref<array<real<4>,1>>)>,2>>> v0 =\
 ( var(undefined(type<struct<kernel:vector<ref<(ref<array<real<4>,1>>)>,2>>>)));");
	...
	EXPECT_PRED2(containsSubString, toString(printer::PrettyPrinter(stmt2, printer::PrettyPrinter::NO_LET_BINDINGS)),
	             "fun(ref<(ref<array<real<4>,1>>)> v1, ref<array<real<4>,1>> v2)");
