...
namespace transform {
	...
	printer::PrettyPrinter printer0(core::analysis::normalize(inlined), printer::PrettyPrinter::PRINT_SINGLE_LINE);
		EXPECT_EQ("decl fun000 : (ref<bool,f,f,plain>) -> bool;def fun000 = function (v1 : ref<ref<bool,f,f,plain>,f,f,plain>) -> bool {return !**v1;};{{var ref<bool,f,f,plain> v1 = ref_var_init(false);{if(3<4) {{v0 = 3+2*6;v1 = true;};};if(!*v1) {var ref<int<4>,f,f,plain> v2 = ref_var_init(3);while(true && !*v1) {v2 = *v2+1;if(*v2>6) {{v0 = *v2-6;v1 = true;};};};};};};}",
			      toString(printer0));
		EXPECT_TRUE(check(inlined, checks::getFullCheck()).empty()) << check(inlined, checks::getFullCheck());
	}
