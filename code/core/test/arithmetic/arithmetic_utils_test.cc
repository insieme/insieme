...
TEST(ArithmeticTest, PiecewiseToIRAndBack) {
	...
	EXPECT_EQ("rec v0.{v0=fun(bool v1, (()=>'b) v2, (()=>'b) v3) {if(v1) {return v2();} else {return v3();};}}(int_le(int_add(v1, v2), 0), bind(){rec v0.{v0=fun(int<4> v1) {return int_add(int_mul(-1, v1), 7);}}(v1)}, rec v0.{v0=fun() {return 0;}})",
...
TEST(ArithmeticTest, NestedSelectToPiecewise) {
	...
	EXPECT_EQ("v1 -> if (!(-v1+v2 <= 0) and !(-v1+v3 <= 0)); v3 -> if (!(-v1+v2 <= 0) and -v1+v3 <= 0) or (-v1+v2 <= 0 and -v2+v3 <= 0); v2 -> if (-v1+v2 <= 0 and !(-v2+v3 <= 0))",
	          toString(toPiecewise(builder.select(builder.select(v1,v2,lt),v3,lt))));
	EXPECT_EQ("v1 -> if (!(-v2+v3 <= 0) and !(-v1+v2 <= 0)) or (-v2+v3 <= 0 and !(-v1+v3 <= 0)); v2 -> if (!(-v2+v3 <= 0) and -v1+v2 <= 0); v3 -> if (-v2+v3 <= 0 and -v1+v3 <= 0)",
