...
namespace datapath {
	...
	TEST(DataPathBuilder, Basic) {
		...
		EXPECT_EQ("dp_member(dp_root(type<struct<hello:bool>>), hello, type<bool>)", toString(*build("struct { hello: bool; }").member("hello").getPath()));
		...
		EXPECT_EQ("dp_parent(dp_root(type<struct B : [struct A <>] <>>), type<A>)", toString(*build("alias A = struct A {}; struct B : [ A ] {}").parent(typeA).getPath()));
