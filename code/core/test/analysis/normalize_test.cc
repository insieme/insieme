...
TEST(Normalizing, Basic) {
	...
	EXPECT_EQ("AP(rec v0.{v0=fun(int<4> v1, bool v2) {v1; v2;}}(v2, v3))", toString(normalize(transform::outline(manager, StatementPtr(builder.compoundStmt(a,
	          b))))));
	EXPECT_EQ("AP(rec v0.{v0=fun(int<4> v1, bool v2) {v1; {v2;};}}(v2, v3))", toString(normalize(transform::outline(manager, StatementPtr(builder.compoundStmt(a,
	          builder.compoundStmt(b)))))));
