...
// -----------------------------
	...
	struct Link {
		...
		Link(const FormulaPtr& sub, const vector<int>& mapping)
			: sub(sub), mapping(mapping) { }
...
//			return *sub == *other.sub && mapping == other.mapping;
		...
		bool operator<(const Link& other) const {
			...
			return lexicographical_compare(
					...
					other.mapping.begin(), other.mapping.end(),
					[](int a, int b) {
	...
	struct And : public Formula {
		...
		And(unsigned freeVars, const Link& left, const Link& right)
			: Formula(NT_And, 0, freeVars, left, right), left(getChild(0)), right(getChild(1)) {};
	};

	struct Exist : public Formula {
		FormulaPtr sub;

		Exist(unsigned freeVars, const Link& sub)
			: Formula(NT_Exist, 1, freeVars, sub), sub(sub.sub) {};
	};
	...
	string printFormula(const Formula* f, const VarNames& names) {
		...
		if (dynamic_cast<const Variable*>(f)) {
			return string(1, names[1]);
		}
