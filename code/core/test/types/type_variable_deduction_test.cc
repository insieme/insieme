...
TEST(TypeVariableConstraints, ConstraintCombinations) {
	...
	EXPECT_TRUE(res);
	if(res) {
		EXPECT_EQ(uint4, res->applyTo(var));
	}
}
...
TEST(TypeVariableDeduction, getTypeVariableInstantiation) {
	...
	EXPECT_TRUE(res);
	if(res) {
		EXPECT_EQ("{'b->int<4>}", toString(*res));
	}
	...
	EXPECT_TRUE(res);
	if(res) {
		EXPECT_EQ(int4, res->applyTo(varA));
	}
	if(res) {
		EXPECT_EQ(uint4, res->applyTo(varB));
	}
	...
	EXPECT_TRUE(res);
	if(res) {
		EXPECT_EQ(int8, res->applyTo(varA));
	}
	if(res) {
		EXPECT_EQ(uint4, res->applyTo(varB));
	}
...
TEST(TypeVariableDeduction, vectorsAndArrays) {
	...
	EXPECT_TRUE(res);
	if(res) {
		EXPECT_EQ("{'a->vector<int<4>,12>}", toString(*res));
	}
	...
	EXPECT_TRUE(res);
	if(res) {
		EXPECT_EQ("{'a->vector<'a,12>}", toString(*res));
	}
	...
	EXPECT_TRUE(res);
	if(res) {
		EXPECT_EQ("{'a->vector<'a,#l>}", toString(*res));
	}
	...
	EXPECT_TRUE(res);
	if(res) {
		EXPECT_EQ("{'a->array<int<4>,1>}", toString(*res));
	}
	...
	EXPECT_TRUE(res);
	if(res) {
		EXPECT_EQ(vectorInt12, res->applyTo(varA));
	}
	if(res) {
		EXPECT_EQ(vectorUInt14, res->applyTo(varB));
	}
	...
	EXPECT_TRUE(res);
	if(res) {
		EXPECT_EQ(array1Int, res->applyTo(varA));
	}
	if(res) {
		EXPECT_EQ(vectorUInt14, res->applyTo(varB));
	}
...
TEST(TypeVariableDeduction, FunctionTypes) {
	...
	EXPECT_TRUE(res);
	if(res) {
		EXPECT_EQ(funII, res->applyTo(varA));
	}
	...
	EXPECT_TRUE(res);
	if(res) {
		EXPECT_EQ(builder.functionType(toVector(basic.getUInt2()), basic.getInt8()), res->applyTo(varA));
	}
	...
	EXPECT_TRUE(res);
	if(res) {
		EXPECT_EQ(funRes, res->applyTo(varA));
	}
	...
	EXPECT_TRUE(res);
	if(res) {
		EXPECT_EQ(getBiggestCommonSubType(int4, uint4), res->applyTo(varA));
	}
	if(res) {
		EXPECT_EQ(getSmallestCommonSuperType(int4, uint4), res->applyTo(varB));
	}
...
TEST(TypeVariableDeduction, PureIntTypeVariableConstraintsBug) {
	...
	EXPECT_TRUE(res);
	if(res) {
		EXPECT_EQ(int4, res->applyTo(intA));
	}
...
TEST(TypeVariableDeduction, VectorMatchingBug) {
	...
	EXPECT_TRUE(match);
	if(match) {
		EXPECT_EQ("{'a->array<A,1>}", toString(*match));
	}
...
TEST(TypeVariableDeduction, PassingVectorToArrayBug) {
	...
	EXPECT_TRUE(unifier);
	if(unifier) {
		EXPECT_EQ(*uint8, *unifier->applyTo(manager, alpha));
	}
}
