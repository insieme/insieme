...
namespace core {
	...
	class is_printable {
		...
		static int consume(...);
		template <typename C>
		static yes test(decltype(consume(*(std::ostream*)(0) << *(T*)(0))));
		template <typename C>
		static no test(...);
	...
	TEST(Annotation, ValueAnnotationPrint) {
		...
		EXPECT_EQ("A", toString(core::printer::PrettyPrinter(ptr, core::printer::PrettyPrinter::PRINT_ANNOTATIONS)));
/*
...
*/	}
	...
	TEST(Annotation, CopyOnCloneTest) {
		struct AnnotationDefault {
			int x;
			AnnotationDefault(int x = 0) : x(x){};
			bool operator==(const AnnotationDefault& other) const {
				return x == other.x;
			}
		};
		...
		struct AnnotationDrop : public value_annotation::drop_on_clone {
			int x;
			AnnotationDrop(int x = 0) : x(x){};
			bool operator==(const AnnotationDrop& other) const {
				return x == other.x;
			}
		};
		...
		struct AnnotationMigrate : public value_annotation::cloneable {
			int x;
			AnnotationMigrate(int x = 0) : x(x){};
			bool operator==(const AnnotationMigrate& other) const {
				return x == other.x;
			}
	...
	TEST(Annotation, CopyOnMigrationTest) {
		...
		struct AnnotationClone : public value_annotation::copy_on_migration {
			int x;
			AnnotationClone(int x = 0) : x(x){};
			bool operator==(const AnnotationClone& other) const {
				return x == other.x;
			}
		};
		...
		struct AnnotationMigrate : public value_annotation::migratable {
			...
			bool migrate(const NodeAnnotationPtr& ptr, const NodePtr& before, const NodePtr& after) const {
				after->attachValue(AnnotationMigrate(x + 1));
				return true;
			}
		};
