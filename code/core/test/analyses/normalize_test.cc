...
namespace analysis {
	...
	TEST(Normalizing, Basic) {
		...
		NodePtr node = builder.parseStmt("{ int<4> a = 0; let f = (int<4> a, int<4> b)->int<4> { return a; } in f(a,a); }");
		...
		EXPECT_EQ(
				...
				toString(*normalize(builder.parseStmt("{ int<4> a = 1; { int<4> b = 2; } { int<4> c = 3; } }")))
		);

		EXPECT_EQ(
				"{int<4> v0 = 1; {v0; int<4> v1 = 2;}; {int<4> v2 = 3; v0;};}",
				toString(*normalize(builder.parseStmt("{ int<4> a = 1; { a; int<4> b = 2; } { int<4> c = 3; a; } }")))
		);

		EXPECT_EQ(
				"{int<4> v0 = 1; int<4> v1 = 2;}",
				toString(*normalize(builder.parseStmt("{ int<4> a = 1; int<4> b = 2; }")))
		);
	}
