...
TEST(IRPattern, ifStmt) {
	...
	auto ps = [&manager](const string& str) {
...
TEST(IRPattern, forStmt) {
	...
	TreePattern pattern4 = irp::forStmt(var("i"), var("x"), var("y"), var("z"), irp::compoundStmt(irp::forStmt(var("j"), var("x"), var("y"), var("z"),
	                                    irp::compoundStmt(*any)) << *any));
...
TEST(IRPattern, Addresses) {
	...
	auto ps = [&manager](const string& str) {
		return StatementAddress(IRBuilder(manager).parseStmt(str));
	};
	...
	TreePattern pattern1 = irp::forStmt(var("x"), any, at("5"), at("-5"), irp::declarationStmt(var("y"), at("3")));
	TreePattern pattern2 = irp::forStmt(var("i"), var("x"), var("y"), var("z"), irp::compoundStmt(irp::forStmt(var("j"), var("x"), var("y"), var("z"),
	                                    irp::compoundStmt(*var("b", any))) << *any));
...
//
//	TreePattern pattern1 = irp::compoundStmt(irp::declarationStmt(var("x"), at("30")) << irp::whileStmt(irp::callExpr(irp::literal("int.lt", any),
//								irp::callExpr(irp::literal("ref.deref", any), var("x") << *any) << *any), irp::compoundStmt(*any << irp::callExpr(any, var("x") << *any))));
//	TreePattern pattern2 = irp::whileStmt(irp::callExpr(irp::literal("int.lt", any), irp::callExpr(irp::literal("ref.deref", any), var("x") << *any) << *any),
