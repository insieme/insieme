...
bool isMatch(const TreePattern& pattern, const TreePtr& tree) {
	return pattern.matchTree(tree);
}

bool noMatch(const TreePattern& pattern, const TreePtr& tree) {
	return !isMatch(pattern, tree);
}

bool isMatchList(const ListPattern& pattern, const vector<TreePtr>& list) {
	return pattern.match(list);
}

bool noMatchList(const ListPattern& pattern, const vector<TreePtr>& list) {
...
TEST(TreePattern, Basic) {
	...
	TreePattern pattern = atom(treeA);
...
TEST(TreePattern, VariablesWithSubPattern) {
	...
	TreePattern inner = node('a', single(any));
...
TEST(ListPattern, Basic) {
	...
	ListPattern pattern = single(atom(treeA));
	TreePattern treePattern = node(pattern);
	EXPECT_EQ("a", toString(pattern));
...
TEST(ListPattern, Variable) {
	...
	ListPattern a = listVar("a");
	ListPattern b = listVar("b");
	TreePattern x = atom(makeTree('x'));
	
	TreePattern p = node(a << x << b);
...
TEST(Sequence, Basic) {
	...
	ListPattern pattern = single(treeA);
...
TEST(Match, All) {
	...
	EXPECT_TRUE(res);
	if(res) {
		EXPECT_EQ("Match({x=[a(b,c),null,null]})", toString(*res));
	}
	
	res = pattern.matchTree(makeTree('b', a, c));
	EXPECT_TRUE(res);
	if(res) {
		EXPECT_EQ("Match({x=[null,a,null]})", toString(*res));
	}
	
	res = pattern.matchTree(makeTree('b', makeTree('b',a,makeTree('a',b)), c));
	EXPECT_TRUE(res);
	if(res) {
		EXPECT_EQ("Match({x=[null,null,a,a(b),null,null]})", toString(*res));
	}
	
	res = pattern.matchTree(makeTree('a', a, a));
	EXPECT_TRUE(res);
	if(res) {
		EXPECT_EQ("Match({x=[a(a,a),a,a]})", toString(*res));
	}
	
	res = pattern.matchTree(makeTree('b', makeTree('a', a, a)));
	EXPECT_TRUE(res);
	if(res) {
		EXPECT_EQ("Match({x=[null,a(a,a),a,a]})", toString(*res));
	}
	
	res = pattern.matchTree(makeTree('b', makeTree('b', makeTree('b', a), a), a));
	EXPECT_TRUE(res);
	if(res) {
		EXPECT_EQ("Match({x=[null,null,null,a,a,a]})", toString(*res));
	}
}
...
TEST(Match, Outermost) {
	...
	EXPECT_TRUE(res);
	if(res) {
		EXPECT_EQ("Match({x=[a(b,c)]})", toString(*res));
	}
	...
	EXPECT_TRUE(res);
	if(res) {
		EXPECT_EQ("Match({x=[null,null,a,a(b),null]})", toString(*res));
	}
	
	res = pattern.matchTree(makeTree('a', a, a));
	EXPECT_TRUE(res);
	if(res) {
		EXPECT_EQ("Match({x=[a(a,a)]})", toString(*res));
	}
	
	res = pattern.matchTree(makeTree('b', makeTree('a', a, a)));
	EXPECT_TRUE(res);
	if(res) {
		EXPECT_EQ("Match({x=[null,a(a,a)]})", toString(*res));
	}
...
//		pattern = rT(((!step(aT(pa))) & pa) | node(*recurse));
	...
	EXPECT_TRUE(res);
	if(res) {
		EXPECT_EQ("Match({x=[null,a,a]})", toString(*res));
	}
	
	res = pattern.matchTree(makeTree('b', makeTree('a', a, a)));
	EXPECT_TRUE(res);
	if(res) {
		EXPECT_EQ("Match({x=[null,null,a,a]})", toString(*res));
	}
...
TEST(Match, SeperatedConjunction) {
	...
	EXPECT_TRUE(res);
	if(res) {
		EXPECT_EQ("Match({x=i})", toString(*res));
	}
...
TEST(Match, AnyTreeRecover) {
	...
	EXPECT_TRUE(res);
	if(res) {
		EXPECT_EQ("Match({x=n})", toString(*res));
	}
	...
	EXPECT_TRUE(res);
	if(res) {
		EXPECT_EQ("Match({x=m})", toString(*res));
	}
}
