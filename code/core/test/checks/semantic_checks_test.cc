...
namespace checks {
	...
	TEST(ValidMaterializingDeclarationCheck, Valid) {
		...
		// - implicit copy constructor, where move is deleted - Note: We have to explicitly add the other ctor, as it won't be generated by default
		structDef = "def struct A1 { ctor ( o : ref<A1,t,f,cpp_ref>) = default; ctor ( o : ref<A1,f,f,cpp_rref>) = delete; }; ";
		decl = mkDecl(structDef + "ref<A1,f,f,plain>", structDef + "lit(\"X\" : ref<A1,t,f,cpp_ref>)");
		EXPECT_TRUE(analysis::isMaterializingDecl(decl));
		EXPECT_TRUE(check(decl,declCheck).empty()) << "Declaration:\n" << dumpReadable(decl) << "\nErrors: " << check(decl,declCheck);

		// - implicit move constructor, where copy is deleted - Note: We have to explicitly add the other ctor, as it won't be generated by default
		structDef = "def struct A2 { ctor ( o : ref<A2,t,f,cpp_ref>) = delete; ctor ( o : ref<A2,f,f,cpp_rref>) = default; }; ";
		decl = mkDecl(structDef + "ref<A2,f,f,plain>", structDef + "lit(\"X\" : ref<A2,f,f,cpp_rref>)");
		EXPECT_TRUE(analysis::isMaterializingDecl(decl));
		EXPECT_TRUE(check(decl,declCheck).empty()) << "Declaration:\n" << dumpReadable(decl) << "\nErrors: " << check(decl,declCheck);
		...
		decl = mkDecl(structDef + "ref<A3,f,f,plain>", structDef + "lit(\"X\" : ref<A3,t,f,cpp_ref>)");
		EXPECT_TRUE(analysis::isMaterializingDecl(decl));
		EXPECT_EQ(1,check(decl,declCheck).size()) << "Declaration:\n" << dumpReadable(decl) << "\nErrors: " << check(decl,declCheck);

		structDef = "def struct A4 { ctor ( o : ref<A4,f,f,cpp_rref>) = delete; }; ";
		decl = mkDecl(structDef + "ref<A4,f,f,plain>", structDef + "lit(\"X\" : ref<A4,f,f,cpp_rref>)");
		EXPECT_TRUE(analysis::isMaterializingDecl(decl));
		EXPECT_EQ(1,check(decl,declCheck).size()) << "Declaration:\n" << dumpReadable(decl) << "\nErrors: " << check(decl,declCheck);
