...
static inline bool printChildListTypes(const NodeList& list) {
	std::cout << "\nFaulty Child Node List: " << ::transform(list, [](const NodePtr& cur) {
		return cur->getNodeType();
	}) << "\n";
	return false;
}
}
...
private:
	...
	static bool checkTypes(const Iterator& begin, const Iterator& end) {
		...
		return all(begin, end, [](const NodePtr& cur) {
			return (bool)dynamic_pointer_cast<Pointer<const ElementType>>(cur);
		});
	}
