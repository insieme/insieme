...
namespace core {
	...
	class Node : public utils::HashableImmutableData<Node>, // Nodes are immutable data objects!
	  ...
	  protected:
		...
		Node(const NodeType nodeType, const NodeCategory nodeCategory, const Pointer<const Nodes>&... children)
		    : HashableImmutableData(hashNodes(nodeType, children...)), nodeType(nodeType), children(toVector<NodePtr>(children...)), nodeCategory(nodeCategory),
		...
		Node(const NodeType nodeType, const NodeCategory nodeCategory, const NodeList& children)
		    : HashableImmutableData(hashNodes(nodeType, children)), nodeType(nodeType), children(children), nodeCategory(nodeCategory), manager(0),
		      equalityID(0) {}
	  ...
	  private:
		...
		static std::size_t hashNodes(NodeType type, const NodeList& nodes) {
			std::size_t seed = 0;
			boost::hash_combine(seed, type);
			for_each(nodes, [&](const NodePtr& cur) { utils::appendHash(seed, *cur); });
			return seed;
		}
	};
	...
	class NodeManager : public InstanceManager<Node, Pointer, move_annotation_on_clone>,
	  ...
	  public:
		 ...
		 */
		template <typename E, typename boost::enable_if<boost::is_base_of<lang::Extension, E>, int>::type = 0>
	...
	NodePtr Node::substituteInternal(NodeManager& manager, NodeMapping<Context>& mapper, Context& c) const {
		...
		if(isValueInternal()) { return (&manager != getNodeManagerPtr()) ? manager.get(*this) : NodePtr(this); }
	 ...
	 */
	template <typename B = Node, template <typename T> class Ptr, typename D,
	...
	class FixedSizeNodeHelper {
	  public:
		...
		static bool checkChildList(const NodeList& list) {
			...
			return list.size() == type_list<Children...>::length && checkTypes<NodeList::const_iterator, Children...>(list.begin(), list.end());
		}
	...
	class ListNodeHelper {
		...
		typedef typename type_at<type_list<First, Rest...>::length - 1, type_list<First, Rest...>>::type ListValueType;
	  ...
	  public:
		...
		ListNodeHelper(const NodeList& children)
		    : listElements(convertList<ListValueType>(children).begin() + (type_list<First, Rest...>::length - 1), convertList<ListValueType>(children).end()) {
			// verify the proper composition of the child node list
			assert_true(checkChildList(children) || printChildListTypes(children)) << "Invalid composition of Child-Nodes discovered!";
		}
		...
		static bool checkChildList(const NodeList& list) {
			// the list has to have the correct length and composition of types
			return list.size() >= type_list<First, Rest...>::length - 1 && checkTypes<NodeList::const_iterator, First, Rest...>(list.begin(), list.end());
		}
	...
	*/
	...
	class Support : public Node {
	  protected:
		 ...
		 */
		Support(const NodeType nodeType, const NodeList& children) : Node(nodeType, NC_Support, children) {}
	};
	...
	class Statement : public Node {
	  protected:
		 ...
		 * @param children the child nodes to be used to create the new node
		 */
		Statement(const NodeType nodeType, const NodeList& children) : Node(nodeType, NC_Statement, children) {}
		 ...
		 * @param children the child nodes to be used to create the new node
		 */
		Statement(const NodeType nodeType, const NodeCategory& category, const NodeList& children) : Node(nodeType, category, children) {}
	};
...
struct IRDump : public insieme::utils::Printable {
	...
	IRDump(const Printer& printer, std::ostream& out) : printed(false), out(out), printer(printer) {}

	IRDump(IRDump&& other) : printed(other.printed), out(other.out), printer(other.printer) {
		other.printed = true;
	}
...
IRDump dumpJson(const insieme::core::NodeAddress&, std::ostream& out = std::cout);
