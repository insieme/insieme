 ...
 */
#define NODE(NAME) \
		typedef Instance<const NAME> NAME ## Instance;
...
private:
	...
	bool isIterable() const {
		...
		if(!this->ptr.isa<CompoundStmtPtr>()) {
			return false;
		}
...
template<
    ...
    typename std::enable_if<std::is_base_of<T,B>::value,int>::type = 0
    >
Instance(const Instance<B>& from) : path(from.getPath()) {}
...
template<
    ...
    >
Instance(Instance<B>&& from) : path(std::move(from.getPath())) {}
...
typename std::enable_if<is_ir_address<R>::value, R>::type isa() const {
	// shortcut
	if(!getAddressedNode().template isa<Pointer<const typename R::element_type>>()) {
		return R();
	}
...
Address<const T> getAsAddress() const {
	if(!path) {
		return Address<const T>();
	}
	if(isRoot()) {
		return Address<const T>(getAddressedNode());
	}
...
static Path convertPath(const NodeAddress& addr) {
	...
	// handle root path
	if(addr.isRoot()) {
		return Path(addr.getAddressedNode());
	}
