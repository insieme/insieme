...
namespace arithmetic {
	...
	class Rational : public utils::Printable {
	  ...
	  private:
		 ...
		 */
		Rational(int64_t num, uint64_t den, bool dummy) : numerator(num), denominator(den) {
	...
	class Formula : public utils::Printable {
	  ...
	  public:
		 ...
		 */
		inline bool isOne() const {
			return terms.size() == static_cast<std::size_t>(1) && terms[0].first.isOne() && terms[0].second.isOne();
		}
	...
	inline Product operator^(const VariablePtr& a, int exp) { return Product(Value(a), exp); }

	inline Product
	operator^(const Value& a, int exp) { return Product(a, exp); }
	...
	class Piecewise : public utils::Printable {
	  ...
	  public:
		 ...
		 */
		Piecewise(const Formula& formula = 0) : pieces(toVector(Piece(Constraint::getTrue(), formula))) {}
	 ...
	 */
	inline Piecewise abs(const Piecewise& a) {
		return max(a, -a);
	}
...
namespace utils {
	...
	inline int asConstant(const insieme::core::arithmetic::Formula& f) {
		if(!f.isConstant()) {
			throw std::logic_error("Formula is not constant"); // fixme
		}
