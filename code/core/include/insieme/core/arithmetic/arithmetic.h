/**
 * Copyright (c) 2002-2013 Distributed and Parallel Systems Group,
 *                Institute of Computer Science,
 *               University of Innsbruck, Austria
 *
 * This file is part of the INSIEME Compiler and Runtime System.
 *
 * We provide the software of this file (below described as "INSIEME")
 * under GPL Version 3.0 on an AS IS basis, and do not warrant its
 * validity or performance.  We reserve the right to update, modify,
 * or discontinue this software at any time.  We shall have no
 * obligation to supply such updates or modifications or any other
 * form of support to you.
 *
 * If you require different license terms for your intended use of the
 * software, e.g. for proprietary commercial or industrial use, please
 * contact us at:
 *                   insieme@dps.uibk.ac.at
 *
 * We kindly ask you to acknowledge the use of this software in any
 * publication or other disclosure of results by referring to the
 * following citation:
 *
 * H. Jordan, P. Thoman, J. Durillo, S. Pellegrini, P. Gschwandtner,
 * T. Fahringer, H. Moritsch. A Multi-Objective Auto-Tuning Framework
 * for Parallel Codes, in Proc. of the Intl. Conference for High
 * Performance Computing, Networking, Storage and Analysis (SC 2012),
 * IEEE Computer Society Press, Nov. 2012, Salt Lake City, USA.
 *
 * All copyright notices must be kept intact.
 *
 * INSIEME depends on several third party software packages. Please 
 * refer to http://www.dps.uibk.ac.at/insieme/license.html for details 
 * regarding third party software licenses.
 */

#pragma once

#include <utility>
#include <vector>

#include "insieme/core/ast_node.h"
#include "insieme/core/expressions.h"

#include "insieme/utils/printable.h"
#include "insieme/utils/hash_utils.h"

namespace insieme {
namespace core {
namespace arithmetic {

	using std::vector;
	using std::pair;

	class Product : public utils::HashableImmutableData<Product>, public utils::Printable {

	public:

		typedef pair<VariablePtr, int> Factor;
		typedef vector<Factor> Factors;

	private:

		Factors variables;

		Product(const Factors&& factors);

	public:

		Product();

		Product(const core::VariablePtr& var, int exponent = 1);

		Product operator*(const Product& other) const;
		Product operator/(const Product& other) const;

		bool operator<(const Product& other) const;

		bool isOne() const { return variables.empty(); }

		virtual std::ostream& printTo(std::ostream& out) const;

	protected:

		virtual bool equals(const Product& other) const {
			return variables == other.variables;
		}

	};

	class Formula : public utils::HashableImmutableData<Formula>, public utils::Printable {

	public:

		typedef pair<Product, int> Term;
		typedef vector<Term> Terms;

	private:

		Terms terms;

		Formula(const Terms&& terms);

	public:

		Formula();

		Formula(int value);

		Formula(const Product& product, int coefficient = 1);

		Formula(const core::VariablePtr& var, int exponent = 1, int coefficient = 1);

		Formula operator+(const Formula& other) const;
		Formula operator-(const Formula& other) const;
		Formula operator*(const Formula& other) const;
//		Formula operator/(const Formula& other) const;

		virtual std::ostream& printTo(std::ostream& out) const;

	protected:

		virtual bool equals(const Formula& other) const {
			return terms == other.terms;
		}

	};


	Formula operator+(int a, const Product& b);
	Formula operator+(const Product& a, int b);
	Formula operator+(const Product& a, const Product& b);
	Formula operator-(const Product& a, const Product& b);

} // end namespace arithmetic
} // end namespace core
} // end namespace insieme
