...
namespace core {
	...
	class IRBuilderBaseModule : public IRBuilderModule {
	  ...
	  public:
		...
		template <typename... T>
		InitExprPtr initExprTemp(const TypePtr& type, const T&... initExprs) const {
		...
		CallExprPtr refTemp(const TypePtr& type) const;
		...
		ReturnStmtPtr returnStmt(const ExpressionPtr& expr, const TypePtr& type) const;
