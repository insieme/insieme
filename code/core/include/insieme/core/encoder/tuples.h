...
namespace encoder {
	...
	namespace detail {
		...
		namespace {
			...
			vector<TypePtr> toTypeList(NodeManager& manager) { return vector<TypePtr>(); }
...
//				return TupleType::get(manager, toVector(typename Converter<T...>::type_factory()(manager)));
	 ...
	 */
	template<typename ... T>
	struct TupleConverter : public Converter<tuple<T...>, detail::create_tuple_type<T...>, detail::encode_tuple<T...>, detail::decode_tuple<T...>, detail::is_tuple<T...>> {};
