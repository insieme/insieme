...
namespace encoder {
	...
	namespace detail {
		...
		struct create_pair_type {
			core::TypePtr operator()(NodeManager& manager) const {
				return GenericType::get(manager, PairExtension::PAIR_TYPE_NAME, toVector(typename CF::type_factory()(manager), typename CS::type_factory()(manager)));
			}
		};
		...
		struct is_pair {
			bool operator()(const core::ExpressionPtr& expr) const {
				...
				if (*fun == *ext.pair) {
					...
					return  typename CF::is_encoding_of()(call->getArgument(0)) &&
							typename CS::is_encoding_of()(call->getArgument(1));
				}
		...
		struct encode_pair {
			...
			core::ExpressionPtr operator()(NodeManager& manager, const pair<F,S>& pair) const {
				...
				return builder.callExpr(pairType, ext.pair,
						toIR<F,CF>(manager, pair.first), toIR<S,CS>(manager, pair.second));
			}
		};
		...
		struct decode_pair {
			...
			pair<F,S> operator()(const core::ExpressionPtr& pair) const {
				...
				return std::make_pair(
						toValue<F,CF>(call->getArgument(0)),
						toValue<S,CS>(call->getArgument(1))
				);
			}
		};
	 ...
	 */
	template<typename F, typename S, typename CF = Converter<F>, typename CS = Converter<S>>
	struct PairConverter : public Converter<pair<F,S>, detail::create_pair_type<F,S,CF,CS>, detail::encode_pair<F,S,CF,CS>, detail::decode_pair<F,S,CF,CS>, detail::is_pair<F,S,CF,CS>> {};
