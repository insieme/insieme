...
namespace encoder {
		...
		struct encode_class_info {
			core::ExpressionPtr operator()(NodeManager& manager, const ClassMetaInfo& info) const {
				...
				auto encodedMemberFuns = ::transform(info.getMemberFunctions(), [](const MemberFunction& cur) -> encoded_member_fun_type {
					return encoded_member_fun_type(cur.getName(), cur.getImplementation(), cur.isVirtual(), cur.isConst());
				});

				// encode class meta-info object
				return encoder::toIR(manager,
				                     encoded_class_info_type(info.getConstructors(), info.getDestructor(), info.isDestructorVirtual(), encodedMemberFuns));
			}
		};
	 ...
	 */
	struct ClassMetaInfoConverter
	    : public Converter<ClassMetaInfo, detail::create_class_info_type, detail::encode_class_info, detail::decode_class_info, detail::is_class_info> {};
