...
namespace lang {
	...
	class PointerExtension : public core::lang::Extension {
	  ...
	  public:
		 ...
		 */
		TYPE_ALIAS("ptr<'a,'c,'v>", "struct _ir_pointer { data : ref<array<'a>,'c,'v>; offset : int<8>; }");
		...
		LANG_EXT_DERIVED_WITH_NAME(PtrFromArray, "ptr_from_array",
				"  lambda (r : ref<array<'a,'s>,'c,'v>) -> ptr<'a,'c,'v> {                             "
		...
		LANG_EXT_DERIVED_WITH_NAME(PtrReinterpret, "ptr_reinterpret",
				"  lambda (p : ptr<'a,'c,'v>, t : type<'b>) -> ptr<'b,'c,'v> {               "
		...
		LANG_EXT_DERIVED_WITH_NAME(PtrCast, "ptr_cast",
				"  lambda (p : ptr<'a,'c,'v>, c : type<'new_const>, v : type<'new_volatile>) -> ptr<'a,'new_const,'new_volatile> {   "
		...
		LANG_EXT_DERIVED_WITH_NAME(PtrConstCast, "ptr_const_cast",
				"  lambda (p : ptr<'a,'c,'v>, c : type<'nc>) -> ptr<'a,'nc,'v> {                    "
		...
		LANG_EXT_DERIVED_WITH_NAME(PtrVolatileCast, "ptr_volatile_cast",
				"  lambda (p : ptr<'a,'c,'v>, v : type<'nv>) -> ptr<'a,'c,'nv> {                       "
		...
		LANG_EXT_DERIVED_WITH_NAME(PtrNarrow, "ptr_narrow",
				"  lambda (p : ptr<'a,'c,'v>, dp : datapath<'a,'b>) -> ptr<'b,'c,'v> {                 "
		...
		LANG_EXT_DERIVED_WITH_NAME(
		    ...
		    "lambda (r : ptr<array<'a,'s>,'c,'v>, i : int<8>) -> ptr<'a,'c,'v> { return ptr_narrow(r, dp_element(dp_root(type_lit(array<'a,'s>)),i)); }"
		)
		...
		LANG_EXT_DERIVED_WITH_NAME(
		    ...
		    "lambda (r : ptr<'a,'c,'v>, name : identifier, type : type<'b>) -> ptr<'b,'c,'v> { return ptr_narrow(r, dp_member(dp_root(type_lit('a)),name,type)); }"
		)
		...
		LANG_EXT_DERIVED_WITH_NAME(
		    ...
		    "lambda (r : ptr<'a,'c,'v>, pos : uint<8>, type : type<'b>) -> ptr<'b,'c,'v> { return ptr_narrow(r, dp_component(dp_root(type_lit('a)),pos,type)); }"
		)
		...
		LANG_EXT_DERIVED_WITH_NAME(
		    ...
		    "lambda (a : ptr<'a,'c,'v>) -> ptr<array<'a>,'c,'v> { return ptr_expand(a, dp_element(dp_root(type_lit(array<'a>)),0u)); }"
		)
		...
		LANG_EXT_DERIVED_WITH_NAME(
			...
			"lambda (p : ptr<'a,'c,'v>, i : int<8>) -> ref<'a,'c,'v> { return p.data[p.offset + i]; }"
		)
		...
		LANG_EXT_DERIVED_WITH_NAME(
			PtrDeref, "ptr_deref",
			"lambda (p : ptr<'a,'c,'v>) -> 'a { return ref_deref(ptr_to_ref(p)); }"
		)
		...
		LANG_EXT_DERIVED_WITH_NAME(
				PtrNull, "ptr_null",
				"lambda (a : type<'a>, c : type<'c>, v : type<'v>) -> ptr<'a,'c,'v> { return <ptr<'a,'c,'v>> { ref_null(type_lit(array<'a,inf>),c,v), 0 }; }"
		)
		...
		LANG_EXT_DERIVED_WITH_NAME(
			PtrEqual, "ptr_eq",
			"lambda (p1 : ptr<'a,'c1,'v1>, p2 : ptr<'a,'c2,'v2>) -> bool { return ref_eq(p1.data,p2.data) && p1.offset == p2.offset; }"
		)

		/**
		 * An operator to compare two references for inequality.
		 */
		LANG_EXT_DERIVED_WITH_NAME(PtrNotEqual, "ptr_ne", "lambda (a : ptr<'a,'c1,'v1>, b : ptr<'a,'c2,'v2>) -> bool { return !ptr_eq(a,b); }")
		...
		LANG_EXT_DERIVED_WITH_NAME(
			PtrAdd, "ptr_add",
			"lambda (p : ptr<'a,'c,'v>, i : int<8>) -> ptr<'a,'c,'v> { return <ptr<'a,'c,'v>> { p.data, p.offset + i }; }"
		)
		...
		LANG_EXT_DERIVED_WITH_NAME(PtrPostInc, "ptr_post_inc", "lambda (p : ref<ptr<'a,'c,'v>>) -> ptr<'a,'c,'v> { var ptr<'a,'c,'v> temp = *p; p = ptr_add(*p, 1l); return temp; }")

		LANG_EXT_DERIVED_WITH_NAME(PtrPostDec, "ptr_post_dec", "lambda (p : ref<ptr<'a,'c,'v>>) -> ptr<'a,'c,'v> { var ptr<'a,'c,'v> temp = *p; p = ptr_sub(*p, 1l); return temp; }")

		LANG_EXT_DERIVED_WITH_NAME(PtrPreInc, "ptr_pre_inc", "lambda (p : ref<ptr<'a,'c,'v>>) -> ptr<'a,'c,'v> { p = ptr_add(*p, 1l); return *p; }")

		LANG_EXT_DERIVED_WITH_NAME(PtrPreDec, "ptr_pre_dec", "lambda (p : ref<ptr<'a,'c,'v>>) -> ptr<'a,'c,'v> { p = ptr_sub(*p, 1l); return *p; }")
