...
namespace pattern {
	...
	class MatchValue : public utils::Printable {
	  ...
	  public:
		MatchValue(unsigned depth, IncrementID increment = 0) : tree(), depth(depth), increment(increment) {}

		MatchValue(const value_type& tree, IncrementID increment = 0) : tree(tree), depth(0), increment(increment) {}
	  ...
	  private:
		...
		void addValue(const MatchPath::iterator& begin, const MatchPath::iterator& end, const value_type& value, IncrementID version) {
			...
			auto index = *begin;
			if(index >= children.size()) { children.resize(index + 1, MatchValue(depth - 1, version)); }
		...
		void addListValue(const MatchPath::iterator& begin, const MatchPath::iterator& end, const list_iterator& left, const list_iterator& right,
			...
			// pick or create inner node
			auto index = *begin;
			if(index >= children.size()) { children.resize(index + 1, MatchValue(depth - 1)); }
	...
	struct MatchMapValue {
		...
		MatchMapValue(const MatchValue<T>& value, IncrementID increment) : value(value), creationIncrement(increment), lastUpdate(increment) {}
		...
		bool operator==(const MatchMapValue<T>& other) const {
			return value == other.value && creationIncrement == other.creationIncrement && lastUpdate == other.lastUpdate;
		}
	};
	...
	class Match : public utils::Printable {
	  ...
	  public:
		Match(const value_type& root = value_type()) : root(root), increment(1){};
		...
		void bindTreeVar(const MatchPath& path, const std::string& var, const value_type& match) {
			...
			auto pos = map.find(var);
			if(pos == map.end()) { pos = map.insert(std::make_pair(var, MatchMapValue<T>(MatchValue<T>(path.getDepth()), increment))).first; }
		...
		void bindListVar(const MatchPath& path, const std::string& var, const list_iterator& begin, const list_iterator& end) {
			...
			auto pos = map.find(var);
			if(pos == map.end()) { pos = map.insert(std::make_pair(var, MatchMapValue<T>(MatchValue<T>(path.getDepth() + 1), increment))).first; }
		...
		virtual std::ostream& printTo(std::ostream& out) const {
			return out << "Match({"
			           << join(", ", map, [](std::ostream& out, const typename ValueMap::value_type& cur) { out << cur.first << "=" << cur.second.value; })
			           << "})";
		}
	...
	size_t hash_value(const MatchValue<T>& value) {
		return (value.getDepth() == 0) ? hash_value(value.getValue()) : boost::hash_value(value.getValues());
	}
