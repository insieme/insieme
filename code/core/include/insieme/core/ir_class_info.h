...
namespace core {
	...
	class MemberFunction : public utils::Printable {
	  ...
	  public:
		...
		/**
		 * Obtains the type of the member function
		 */
		...
		bool operator==(const MemberFunction& other) const {
			return this == &other || (name == other.name && *impl == *other.impl && m_virtual == other.m_virtual && m_const == other.m_const);
		}
	 ...
	 */
	class ClassMetaInfo : public utils::Printable,
	  ...
	  public:
		...
		bool hasMemberFunction(const string& name, const FunctionTypePtr& type) const {
			for(const auto& cur : memberFunctions) {
				if((cur.getName() == name) && (cur.getType() == type)) { return true; }
			}
			return false;
		}
		 ...
		 */
		MemberFunctionPtr getMemberFunction(const string& name, const FunctionTypePtr& type, bool _const) const {
			for(const auto& cur : memberFunctions) {
				if((cur.getName() == name) && (cur.getType() == type) && (cur.isConst() == _const)) { return &cur; }
			}
			return nullptr;
		}
		 ...
		 * @return a pointer to the requested function or null if there is no such function
		 */
		MemberFunctionPtr getMemberFunction(const string& name, const FunctionTypePtr& type) const {
			for(const auto& cur : memberFunctions) {
				if((cur.getName() == name) && (cur.getType() == type)) { return &cur; }
			}
			return nullptr;
		}
		...
		vector<MemberFunctionPtr> getMemberFunctionOverloads(const string& name) const {
			...
			for(auto& cur : memberFunctions) {
				if(cur.getName() == name) { res.push_back(&cur); }
			}
			return res;
		}
		...
		bool operator==(const ClassMetaInfo& other) const {
			return this == &other || (virtualDestructor == other.virtualDestructor && equals(constructors, other.constructors, equal_target<ExpressionPtr>())
			                          && equalTarget(destructor, other.destructor) && memberFunctions == other.memberFunctions);
		}
