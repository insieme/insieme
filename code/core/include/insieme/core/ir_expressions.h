 ...
 */
virtual std::ostream& printTo(std::ostream& out) const {
	static auto paramPrinter = [](std::ostream& out, const VariablePtr& cur) {
		out << *cur->getType() << " " << *cur;
	};
...
LambdaExprPtr peel(NodeManager& manager, unsigned numTimes = 1) const {
	if(!isRecursive()) {
		return manager.get(ExpressionAccessor<Derived, Ptr>::getNode());
	}
...
bool isBoundExpression(const Ptr<const Expression>& expr) const {
	...
	if(expr->getNodeType() == NT_Variable) {
		const auto& parameters = this->getNode().getParameters()->getElements();
		if(contains(parameters, expr.template as<VariablePtr>())) {
			return false;
		}
	}
 ...
 */
virtual std::ostream& printTo(std::ostream& out) const {
	static auto entryPrinter = [](std::ostream& out, const NamedValuePtr& cur) {
		out << *cur->getName() << "=" << *cur->getValue();
	};
