...
namespace {
	...
	struct ErrorPlugin : public PrinterPlugin{
		...
		ErrorPlugin(const addr2msg& addresses)
		:addresses(addresses) {
			...
			visitDepthFirst(rootAdrr, [&](const NodeAddress& cur){
				...
				if(match != addresses.end()){
					annotations::attachError(cur.getAddressedNode(), match->second);
				}
			});
		}
	...
	IRDump dumpErrors(const MessageList& msgs, std::ostream& out){
		...
		return IRDump([&](std::ostream& out)->std::ostream& {
			...
			for (auto msg : msgs.getAll()){
				...
				switch (msg.getType()){
					case Message::WARNING:
						text.append(RED + "WARNING: ");
						break;
					case Message::ERROR:
						text.append(RED + "ERROR: ");
						break;
					default:
						{ assert_fail() << "what was that?"; }
				};
			...
			if (msgs.getAll().empty()){
				out << "the IR seems to be correct, use a pretty printer" <<std::endl;
			}
