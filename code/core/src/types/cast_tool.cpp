...
core::ExpressionPtr castLiteral(const core::LiteralPtr lit, const core::TypePtr targetTy) {
	...
	if(gen.isBool(targetTy)) {
		if(gen.isChar(lit->getType())) {
			throw std::exception();
		}
		if(boost::regex_match(old, zeroValueFilter)) {
			res.assign("false");
		}
		else {
			res.assign("true");
...
std::size_t getPrecission(const core::TypePtr& type, const core::lang::BasicGenerator& gen) {
	...
	if(gen.isReal(type)) {
		...
		else if(gen.isReal8(type)) {
			return 8;
		}
		else if(gen.isReal16(type)) {
			return 16;
		}
		else if(gen.isFloat(type)) {
			return 4;
		}
		else if(gen.isDouble(type)) {
			return 8;
		}
		else if(gen.isLongDouble(type)) {
			return 16;
		}
	}
		...
		else if(gen.isInt2(type)) {
			return 2;
		}
		else if(gen.isInt4(type)) {
			return 4;
		}
		else if(gen.isInt8(type)) {
			return 8;
		}
		else if(gen.isInt16(type)) {
			return 16;
		}
	}
		...
		else if(gen.isUInt2(type)) {
			return 2;
		}
		else if(gen.isUInt4(type)) {
			return 4;
		}
		else if(gen.isUInt8(type)) {
			return 8;
		}
		else if(gen.isUInt16(type)) {
			return 16;
		}
	}
	else if(gen.isWChar(type)) {
		if(gen.isWChar16(type)) {
			return 16;
		}
		else if(gen.isWChar32(type)) {
			return 32;
		}
	}
...
core::ExpressionPtr castScalar(const core::TypePtr& trgTy, core::ExpressionPtr expr) {
	...
	if(gen.isSignedInt(exprTy)) {
		code = 1;
	}
	if(gen.isUnsignedInt(exprTy)) {
		code = 2;
	}
	if(gen.isReal(exprTy)) {
		code = 3;
	}
	if(gen.isChar(exprTy)) {
		code = 4;
	}
	if(gen.isBool(exprTy)) {
		code = 5;
	}
	if(gen.isWChar(exprTy)) {
		code = 6;
	}
	if(mgr.getLangExtension<core::lang::EnumExtension>().isEnumType(exprTy)) {
		code = 7;
	}
	
	// identify target type
	if(gen.isSignedInt(targetTy)) {
		code += 10;
	}
	if(gen.isUnsignedInt(targetTy)) {
		code += 20;
	...
	if(gen.isWChar(targetTy)) {
		code += 60;
	}
	if(mgr.getLangExtension<core::lang::EnumExtension>().isEnumType(targetTy)) {
		code += 70;
	}
	...
	case 11:
		...
		if(bytes != getPrecission(exprTy, gen)) {
			resIr = doCast(gen.getIntPrecisionFix(), expr, bytes);
		}
		else {
			resIr = expr;
		}
	...
	case 17:
		...
		if(bytes != getPrecission(resIr->getType(), gen)) {
			resIr = doCast(gen.getIntPrecisionFix(), resIr, bytes);
		}
		break;
	case 22:
		if(bytes != getPrecission(exprTy, gen)) {
			resIr = doCast(gen.getUintPrecisionFix(), expr, bytes);
		}
		else {
			resIr = expr;
		}
	...
	case 27:
		...
		if(bytes != getPrecission(resIr->getType(), gen)) {
			resIr = doCast(gen.getUintPrecisionFix(), resIr, bytes);
		}
		break;
	case 33:
		if(bytes != getPrecission(exprTy, gen)) {
			resIr = doCast(gen.getRealPrecisionFix(), expr, bytes);
		}
		else {
			resIr = expr;
		}
	...
	case 66:
		if(bytes != getPrecission(exprTy, gen)) {
			resIr = doCast(gen.getWCharPrecisionFix(), expr, bytes);
		}
		else {
			resIr = expr;
		}
	...
	case 12:
		resIr = doCast(gen.getUnsignedToInt(), expr, bytes);
		break;
	case 13:
		resIr = doCast(gen.getRealToInt(), expr, bytes);
		break;
	case 14:
		resIr = doCast(gen.getCharToInt(), expr, bytes);
		break;
	case 15:
		resIr = doCast(gen.getBoolToInt(), expr, bytes);
		break;
	case 16:
		resIr = doCast(gen.getWCharToInt(), expr, bytes);
		break;
		
	case 21:
		resIr = doCast(gen.getSignedToUnsigned(), expr, bytes);
		break;
	case 23:
		resIr = doCast(gen.getRealToUnsigned(), expr, bytes);
		break;
	case 24:
		resIr = doCast(gen.getCharToUnsigned(), expr, bytes);
		break;
	case 25:
		resIr = doCast(gen.getBoolToUnsigned(), expr, bytes);
		break;
	case 26:
		resIr = doCast(gen.getWCharToUnsigned(), expr, bytes);
		break;
		
	case 31:
		resIr = doCast(gen.getSignedToReal(), expr, bytes);
		break;
	case 32:
		resIr = doCast(gen.getUnsignedToReal(), expr, bytes);
		break;
	case 34:
		resIr = doCast(gen.getCharToReal(), expr, bytes);
		break;
	case 35:
		resIr = doCast(gen.getBoolToReal(), expr, bytes);
		break;
		
	case 41:
		resIr = doCast(gen.getSignedToChar(), expr, 0);
		break;
	case 42:
		resIr = doCast(gen.getUnsignedToChar(), expr, 0);
		break;
	case 43:
		resIr = doCast(gen.getRealToChar(), expr, 0);
		break;
	case 45:
		resIr = doCast(gen.getBoolToChar(), expr, 0);
		break;
		
	case 51:
		resIr = doCast(gen.getSignedToBool(), expr, 0);
		break;
	case 52:
		resIr = doCast(gen.getUnsignedToBool(), expr, 0);
		break;
	case 53:
		resIr = doCast(gen.getRealToBool(), expr, 0);
		break;
	case 54:
		resIr = doCast(gen.getCharToBool(), expr, 0);
		break;
		
	case 57:
		resIr = doCast(enumExt.getEnumElementAsBool(), expr, 0);
		break;
		
	case 61:
		resIr = doCast(gen.getSignedToWChar(), expr, bytes);
		break;
	case 62:
		resIr = doCast(gen.getUnsignedToWChar(), expr, bytes);
		break;
	case 63:
		resIr = doCast(gen.getRealToWChar(), expr, bytes);
		break;
	case 64:
		resIr = doCast(gen.getCharToWChar(), expr, bytes);
		break;
	case 65:
		resIr = doCast(gen.getBoolToWChar(), expr, bytes);
		break;
		
		
	case 71:
		resIr = builder.callExpr(enumExt.getIntAsEnum(), expr, builder.getTypeLiteral(targetTy));
		break;
	case 72:
		resIr = builder.callExpr(enumExt.getUIntAsEnum(), expr, builder.getTypeLiteral(targetTy));
		break;
