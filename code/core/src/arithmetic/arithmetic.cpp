...
namespace arithmetic {
	...
	namespace {
		...
		Container combine(const Container& a, const Container& b) {
			...
			while(it1 != end1 && it2 != end2) {
				...
				if(ext(a.first) == ext(b.first)) {
					...
					if(exp != ExprTy(0)) { res.push_back(std::make_pair(a.first, exp)); }
					++it1;
					++it2;
			...
			std::transform(it2, end2, out, [](const Element& cur) {
				Combinator op;
				return std::make_pair(cur.first, op(0, cur.second));
			});
		...
		inline ExprTy findAssignedValue(const Container& list, const T& value) {
			...
			if(list.size() == 1u) {
				if(ext(list[0].first) == ext(value)) { return list[0].second; }
				return 0;
			}

			// search for product (binary search)
			auto end = list.end();
			auto pos = std::lower_bound(list.begin(), end, std::make_pair(value, ExprTy(0)), [](const Element& a, const Element& b) {
				Extractor ext;
				return ext(a.first) < ext(b.first);
			});

			// check whether it has been found
			if(pos != end && ext(pos->first) == ext(value)) { return pos->second; }
		...
		bool lessThan(const NodePtr& a, const NodePtr& b) {
			...
			if(a->getNodeCategory() == NC_Type && b->getNodeCategory() == NC_Type) { return toString(*a) < toString(*b); }
	...
	namespace {
		...
		bool isValueInternal(const ExpressionPtr& expr, bool topLevel = false) {
			...
			if(topLevel && !basic.isInt(expr->getType())) { return false; }

			// all variables are values
			if(expr->getNodeType() == core::NT_Variable) { return true; }
			...
			if(!topLevel && expr->getNodeType() == core::NT_Literal) { return true; }
			...
			// handle tuples
			if(ext.isRefComponentAccess(fun) || basic.isTupleMemberAccess(fun)) { return isValueInternal(args[0]); }

			// handle composites
			if(ext.isRefMemberAccess(fun) || basic.isCompositeMemberAccess(fun)) { return isValueInternal(args[0]); }
			...
			try {
				...
				if(isValueConstructor(fun)) {
					...
					for_each(args, [](const ExpressionPtr& cur) { toFormula(cur); });
					return true;
				}
	...
	Product::Product(const VariablePtr& var, int exponent) : factors(getSingle(var, exponent)){};

	Product::Product(const Value& value, int exponent) : factors(getSingle(value, exponent)){};

	Product::Product(const vector<Factor>&& factors) : factors(factors){};

	void Product::appendValues(ValueSet& set) const {
		for_each(factors, [&](const Factor& cur) { set.insert(cur.first); });
	}
	...
	bool Product::operator<(const Product& other) const {
		...
		if(other.factors.empty()) { return true; }

		// the rest is ordered lexicographically
		return lexicographical_compare(factors, other.factors,
		                               [](const Factor& a, const Factor& b) { return a.first < b.first || (a.first == b.first && a.second > b.second); });
	}

	Product Product::operator^(int exp) const {
		vector<Factor> ret;

		for_each(factors, [&](const Factor& cur) { ret.push_back(Factor(cur.first, cur.second * exp)); });
	...
	bool Product::isLinear() const {
		return factors.size() <= static_cast<std::size_t>(1) && all(factors, [](const Factor& cur) { return cur.second == 1; });
	}
	...
	bool Product::isPolynomial() const {
		return all(factors, [](const Factor& cur) { return cur.second > 0; });
	}
	...
	Formula::Formula(const Rational& value) : terms((value.isZero()) ? vector<Term>() : toVector(Term(Product(), value))){};

	Formula::Formula(const Product& product, const Rational& coefficient) : terms(toVector(std::make_pair(product, coefficient))) {
	...
	Formula::Formula(const Value& value, int exponent, const Rational& coefficient) : terms(toVector(Term(Product(value, exponent), coefficient))) {
		assert_ne(exponent, 0) << "Exponent must be != 0!";
		assert_false(coefficient.isZero()) << "Coefficient must be != 0!";
	};

	void Formula::appendValues(ValueSet& set) const {
		for_each(terms, [&](const Term& cur) { cur.first.appendValues(set); });
	}

	Formula Formula::replace(const ValueReplacementMap& replacements) const {
		// quick check for empty replacement map
		if(replacements.empty()) { return *this; }
		...
		for_each(terms, [&](const Term& cur) {
			...
			for_each(cur.first.getFactors(), [&](const Product::Factor& prod) {
				...
				if(pos != replacements.end()) { value = pos->second; }
	...
	std::ostream& Formula::printTo(std::ostream& out) const {
		...
		return out << join("", terms, [&](std::ostream& out, const Term& cur) -> std::ostream& {
			       ...
			       if(!isFirst && cur.second.isPositive()) { out << "+"; }

			       if(cur.first.isOne()) { return out << cur.second; }

			       if(cur.second.isOne()) { return out << cur.first; }

			       if(cur.second.isMinusOne()) { return out << "-" << cur.first; }
	...
	const Formula Formula::operator-() const {
		...
		for_each(terms, [&](const Term& cur) { negTerms.push_back(Term(cur.first, -cur.second)); });
	...
	Formula& Formula::operator*=(const Formula& other) {
		...
		for_each(terms, [&](const Term& a) {
			for_each(other.terms, [&](const Term& b) {
				...
				if(!newCoeff.isZero()) { res += (A * B) * newCoeff; }
			});
		});
	...
	Formula& Formula::operator/=(const Rational& divisor) {
		...
		for_each(terms, [&](Term& cur) { cur.second /= divisor; });
		return *this;
	}

	Formula& Formula::operator/=(const Product& divisor) {
		for_each(terms, [&](Term& cur) { cur.first /= divisor; });
		return *this;
	}
	...
	bool Formula::isLinear() const {
		return all(terms, [](const Term& cur) { return cur.first.isLinear(); });
	}

	bool Formula::isValue() const {
		return terms.size() == static_cast<std::size_t>(1) && terms[0].second.isOne() && terms[0].first.isValue();
	}
	...
	bool Formula::isPolynomial() const {
		return all(terms, [](const Term& cur) { return cur.first.isPolynomial(); });
	}
	...
	Inequality::Inequality(const Formula& f) : formula(f) {
		...
		for_each(formula.getTerms(), [&](const Formula::Term& cur) { denominator_lcm = lcm(denominator_lcm, cur.second.getDenominator()); });
		if(denominator_lcm != 1) { formula *= denominator_lcm; }

		// reduce coefficients
		int factor_gcd = formula.getTerms()[0].second.getNumerator();
		for_each(formula.getTerms(), [&](const Formula::Term& cur) { factor_gcd = gcd(factor_gcd, std::abs(cur.second.getNumerator())); });
	...
	namespace detail {
		  ...
		  public:
			int getVarIdFor(const Inequality& inequality) {
				for(std::size_t i = 0; i < atomList.size(); ++i) {
					if(atomList[i] == inequality) { return i; }
				}
			...
			const Inequality* getInequalityWithId(int id) {
				if(0 <= id && id < (int)atomList.size()) { return &(atomList[id]); }
				return NULL;
			}
		...
		class BDD : public utils::Printable {
		  ...
		  public:
			BDD(bool value) : manager(), isTrue(value), isFalse(!value), bdd(){};

			BDD(const BDDManagerPtr& manager, bool value) : manager(manager), isTrue(value), isFalse(!value), bdd(){};
			...
			static BDDPtr getLiteralBDD(const BDDManagerPtr& manager, const Inequality& atom) {
				if(atom.isValid()) { return getTrueBDD(manager); }
				if(atom.isUnsatisfiable()) { return getFalseBDD(manager); }
				return std::make_shared<BDD>(manager, manager->getVar(atom));
			}

			static BDDPtr getLiteralBDD(const Inequality& atom) {
				if(atom.isValid()) { return getTrueBDD(); }
				if(atom.isUnsatisfiable()) { return getFalseBDD(); }
			...
			const DNF& toDNF() const {
				...
				if(dnf.isEvaluated()) { return dnf.getValue(); }
		  ...
		  private:
			CuddBDD getWithinLocalManager(const BDD& bdd) const {
				...
				if(bdd.manager == manager) { return bdd.bdd; }
	...
	Constraint::Constraint() : bdd(detail::BDD::getFalseBDD()) {}

	Constraint::Constraint(const Inequality& atom) : bdd(detail::BDD::getLiteralBDD(atom)) {}

	Constraint::Constraint(const detail::BDDPtr& bdd) : bdd(bdd) {}
	...
	namespace {
		...
		class pieces_builder {
		  ...
		  public:
			void addPiece(const Constraint& constraint, const Formula& formula) {
				...
				if(constraint.isUnsatisfiable()) { return; }

				// test whether same formula has been computed before
				auto pos = std::find_if(pieces.begin(), pieces.end(), [&](const Piece& cur) { return cur.second == formula; });
				if(pos != pieces.end()) {
		...
		vector<Piece> combine(const vector<Piece>& as, const vector<Piece>& bs) {
			...
			for_each(as, [&](const Piece& a) {
				for_each(bs, [&](const Piece& b) {
					...
					builder.addPiece(a.first && b.first,    // the conjunction of the constraints
		...
		vector<Piece> expandPiece(const Piece& piece, const Formula& elseValue) {
			if(piece.first.isValid()) { return toVector(piece); }
			if(piece.first.isUnsatisfiable() || piece.second == elseValue) { return toVector(Piece(Constraint::getTrue(), elseValue)); }
		...
		struct ArithConstraintConverter : public utils::ConstraintVisitor<Formula, Constraint> {
			...
			ArithConstraintConverter(detail::BDDManagerPtr& manager) : manager(manager) {}
			...
			Constraint visitBinConstraint(const utils::BinConstraint<Formula>& bcc) {
				...
				// combine results
				if(bcc.isConjunction()) { return lhs && rhs; }

				if(bcc.isDisjunction()) { return lhs || rhs; }
		...
		vector<Piece> extractFrom(const utils::Piecewise<Formula>& other) {
			...
			if(other.empty()) { return toVector(Piece(Constraint::getTrue(), 0)); }
	...
	Piecewise::Piecewise(const Piece& piece) : pieces(expandPiece(piece, 0)) {}

	Piecewise::Piecewise(const utils::Piecewise<Formula>& other) : pieces(extractFrom(other)) {}

	Piecewise::Piecewise(const Constraint& constraint, const Piecewise& thenValue, const Piecewise& elseValue) {
		pieces_builder builder;

		// add then values ...
		for_each(thenValue.pieces, [&](const Piece& cur) { builder.addPiece(cur.first && constraint, cur.second); });

		// add else values ...
		auto notConstraint = !constraint;
		for_each(elseValue.pieces, [&](const Piece& cur) { builder.addPiece(cur.first && notConstraint, cur.second); });
	...
	const Piecewise Piecewise::operator-() const {
		...
		for_each(newPieces, [](Piece& cur) { cur.second = -cur.second; });
	...
	Piecewise& Piecewise::operator/=(const Formula::Term& divisor) {
		for_each(pieces, [&](Piece& cur) { cur.second = cur.second / divisor; });
		return *this;
	}
	...
	Piecewise Piecewise::replace(const ValueReplacementMap& replacements) const {
		...
		for_each(pieces, [&](const Piece& cur) { builder.addPiece(cur.first.replace(replacements), cur.second.replace(replacements)); });
	...
	std::ostream& Piecewise::printTo(std::ostream& out) const {
		...
		return out << join("; ", pieces, [](std::ostream& out, const Piecewise::Piece& cur) { out << cur.second << " -> if " << cur.first; });
	}
