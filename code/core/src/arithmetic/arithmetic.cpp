...
size_t Formula::getDegree() const {
	...
	for_each(terms, [&](const Term& cur) {
		degrees.insert(cur.first.getDegree());
	});
	if(degrees.empty()) {
		return 0;
	}
...
struct ArithConstraintConverter : public utils::ConstraintVisitor<Formula, Constraint> {
	...
	Constraint visitRawConstraint(const utils::RawConstraint<Formula>& rcc) {
		...
		case utils::ConstraintType::EQ:
			return le && ge;
			break;
		case utils::ConstraintType::NE:
			return !(le && ge);
			break;
		case utils::ConstraintType::LE:
			return le;
			break;
		case utils::ConstraintType::LT:
			return le && !(le && ge);
			break;
		case utils::ConstraintType::GE:
			return ge;
			break;
		case utils::ConstraintType::GT:
			return ge && !(le && ge);
			break;
		}
