		...
		class FormulaConverter : public IRVisitor<Formula> {
		  ...
		  protected:
			...
			Formula visitCallExpr(const CallExprPtr& call) {
				...
				if(call->getArguments().size() != static_cast<std::size_t>(2)) { throw NotAFormulaException(call); }
		...
		class ConstraintConverter : public IRVisitor<Constraint> {
		  ...
		  protected:
			...
			Constraint visitCallExpr(const CallExprPtr& call) {
				...
				if(call->getArguments().size() != 2u || fun->getNodeType() != core::NT_Literal || !lang.isCompOp(fun)) { throw NotAConstraintException(call); }
