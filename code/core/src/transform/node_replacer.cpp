	...
	class RecVariableMapReplacer : public CachedNodeMapping {
	  ...
	  private:
		...
		virtual const NodePtr resolveElement(const NodePtr& ptr) {
			...
			if(res->getNodeType() == NT_CallExpr) { res = handleCallExpr(res.as<CallExprPtr>()); }

			// special handling for declaration statements
			if(res->getNodeType() == NT_DeclarationStmt) { res = handleDeclStmt(res.as<DeclarationStmtPtr>()); }
		...
		bool typesMatch(ExpressionList a, ExpressionList b) {
			...
			for(unsigned int cnt = 0; cnt < a.size(); ++cnt) {
				if(a.at(cnt) != b.at(cnt)->getType()) { return false; }
			}
		...
		ExpressionPtr handleCallExpr(const CallExprPtr& call) {
			...
			if(manager.getLangExtension<lang::ReferenceExtension>().isRefAssign(fun)) {
				...
				if(lhs->getType().isa<GenericTypePtr>()) { return call; }
				...
				if(!isSubTypeOf(rhs->getType(), lhsTy)) {
					...
					if(CastExprPtr rCast = rhs.isa<CastExprPtr>()) { return builder.callExpr(fun, lhs, builder.castExpr(lhsTy, rCast->getSubExpression())); }
	  ...
	  private:
		...
		NodePtr handleDeclStmt(const DeclarationStmtPtr& decl) {
			...
			if(pos != replacements.end()) {
				...
				if(pos->second->getType() != newInit->getType()) { return typeHandler(builder.declarationStmt(pos->second.as<VariablePtr>(), newInit)); }
		...
		CallExprPtr handleCall(const CallExprPtr& call) {
			...
			ExpressionList newArgs =
			    ::transform(args, [&](const ExpressionPtr& cur) -> ExpressionPtr { return static_pointer_cast<const Expression>(this->resolveElement(cur)); });

			// fix call to known built-in functions
			if(lang::isBuiltIn(call->getFunctionExpr())) { return handleCallToBuiltIn(call, newArgs); }
			...
			if(args == newArgs && fun->getNodeType() != NT_Literal) { return call; }

			if(fun->getNodeType() == NT_Literal) { return handleCallToLiteral(call, call->getType(), newArgs).as<CallExprPtr>(); }
		...
		CallExprPtr handleCallToBuiltIn(const CallExprPtr& call, const ExpressionList& args) {
			...
			if(refExt.isRefMemberAccess(fun)) {
				return static_pointer_cast<const CallExpr>(builder.refMember(args.at(0), static_pointer_cast<const Literal>(args.at(1))->getValue()));
			}
			if(basic.isCompositeMemberAccess(fun)) {
				return static_pointer_cast<const CallExpr>(builder.accessMember(args.at(0), static_pointer_cast<const Literal>(args.at(1))->getValue()));
			}
			if(refExt.isRefComponentAccess(fun)) { return static_pointer_cast<const CallExpr>(builder.refComponent(args.at(0), args.at(1))); }
			if(basic.isTupleMemberAccess(fun)) { return static_pointer_cast<const CallExpr>(builder.accessComponent(args.at(0), args.at(1))); }

			if(refExt.isRefAssign(fun)) { return builder.assign(args.at(0), args.at(1)); }

			if(refExt.isRefNew(fun)) { return builder.refNew(args.at(0)); }

			if(refExt.isRefVar(fun)) { return builder.refVar(args.at(0)); }
		...
		CallExprPtr handleCallToLamba(const TypePtr& resType, const LambdaExprPtr& lambda, const ExpressionList& args) {
			...
			if(callTy) { return static_pointer_cast<const CallExpr>(builder.callExpr(callTy, lambda, args)->substitute(manager, *this)); }
...
namespace transform {
	...
	NodePtr fixTypes(NodeManager& mgr, NodePtr root, const ExpressionMap& replacements, bool limitScope, const TypeHandler& typeHandler) {
		// conduct actual substitutions
