...
public:
	...
	NodePtr simplifyCall(const NodePtr& ptr) {
		...
		if(!simplifyDerivedOps && core::lang::isDerived(func)) {
			return ptr;
		}
	...
	NodePtr simplifyIf(const NodePtr& ptr) {
		// it has to be an if ...
		if(ptr->getNodeType() != core::NT_IfStmt) {
			return ptr;
		}
	...
	NodePtr simplifyITE(const NodePtr& ptr) {
		...
		if(!analysis::isCallOf(ptr, manager.getLangBasic().getIfThenElse())) {
			return ptr;
		}
	...
	NodePtr simplifyWhile(const NodePtr& ptr) {
		...
		if(ptr->getNodeType() != core::NT_WhileStmt) {
			return ptr;
		}
	...
	NodePtr simplifyFor(const NodePtr& ptr) {
		...
		if(ptr->getNodeType() != core::NT_ForStmt) {
			return ptr;
		}
	...
	NodePtr simplifyCompound(const NodePtr& ptr) {
		...
		if(ptr->getNodeType() != core::NT_CompoundStmt) {
			return ptr;
		}
		...
		for(const StatementPtr& cur : stmt) {
			...
			if(cur->getNodeType() == NT_CompoundStmt) {
				...
				if(!any(inner.getStatements(), [](const StatementPtr& cur) {
				return cur->getNodeType() == NT_DeclarationStmt;
				})) {
	...
	virtual const NodePtr resolveElement(const NodePtr& ptr) {
		...
		if(!simplifyDerivedOps && core::lang::isDerived(ptr)) {
			return ptr;
		}
