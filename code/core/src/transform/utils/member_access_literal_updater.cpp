...
#define BASIC builder.getNodeManager().getLangBasic()
	...
	const NodePtr MemberAccessLiteralUpdater::resolveElement(const NodePtr& ptr) {
		...
		if(ptr->getNodeCategory() == NC_Type || ptr->getNodeCategory() == NC_IntTypeParam) { return ptr; }
		...
		if(const CallExprPtr& call = dynamic_pointer_cast<const CallExpr>(res)) {
			...
			if(refExt.isRefArrayElement(fun)) {
				...
				const GenericTypePtr& seTy =
				    (isRef ? analysis::getReferencedType(call->getArgument(0)->getType()) : call->getArgument(0)->getType()).as<GenericTypePtr>();
				const TypePtr& type = isRef ? builder.refType(seTy->getTypeParameter(0)) : seTy->getTypeParameter(0);

				if(call->getType() != type) { res = builder.callExpr(type, fun, call->getArguments()); }
			}
			...
			if(refExt.isRefComponentAccess(fun) || BASIC.isTupleMemberAccess(fun)) {
				...
				bool isRef = analysis::isRefType(call->getArgument(0)->getType());
				const TupleTypePtr tupleTy = dynamic_pointer_cast<const TupleType>(isRef ? analysis::getReferencedType(call->getArgument(0)->getType())
				...
				if(*call->getType() != *retTy || *call->getArgument(2)->getType() != *elemTyLit->getType()) {
					res = builder.callExpr(retTy, isRef ? refExt.getRefComponentAccess() : BASIC.getTupleMemberAccess(), call->getArgument(0),
					                       call->getArgument(1), elemTyLit);
				}
			}
		}
