...
namespace irp {
	...
	void matchAllPairsReverse(const TreePatternPtr& pattern, const core::NodeAddress& root, std::function<void(core::NodeAddress addr, AddressMatch match)> lambda, bool matchTypes) {
		auto&& matches = collectAllPairs(pattern, root, matchTypes);
