...
template<typename T> T convert(NodeAddress a) {
	return a.as<T>();
}
template<typename T> T convert(NodePtr n) {
	return T(n);
}
...
NodePtr replaceAllInternal(const TreePattern& pattern, const core::NodePtr& root, std::function<core::NodePtr(LambdaArgType match)> lambda, bool matchTypes,
                           bool passAddresses) {
