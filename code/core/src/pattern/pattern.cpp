...
struct Descendant : public TreePattern {
	...
	Descendant(const vector<TreePatternPtr>& patterns)
		: TreePattern(TreePattern::Descendant, ::all(patterns, [](const TreePatternPtr& cur) {
		return cur->isVariableFree;
	})),
...
bool contains(MatchContext<T>& context, const typename T::value_type& tree, const impl::TreePatternPtr& pattern,
	...
	// prune types
	if(!pattern->mayBeType && isTypeOrValueOrParam(tree)) {
		return false;
	}
...
MATCH(Recursion) {
	...
	std::function<bool(MatchContext<T>&)> accept = [](MatchContext<T>& context) {
		return true;
	};
	...
	if(preBound) {
		context.bindRecVar(pattern.name, oldInfo);
	}
...
MATCH(Node) {
	if(pattern.type != -1 && pattern.type != tree->getNodeType()) {
		return false;
	}
...
MATCH(Disjunction) {
	...
	auto backup = context.backup();
	if(match(pattern.pattern1, context, tree, delayedCheck)) {
		return true;
	}
	// restore context
	backup.restore(context);
...
inline bool matchNode(const pattern::impl::tree::Node& pattern, MatchContext<tree_target>& context, const TreePtr& tree,
                      const std::function<bool(MatchContext<tree_target>&)>& delayedCheck) {
	if(pattern.id != -1 && pattern.id != tree->getId()) {
		return false;
	}
...
bool match(const impl::TreePattern& pattern, MatchContext<T>& context, const typename T::value_type& tree,
	...
	if(DEBUG) {
		std::cout << "Matching " << pattern << " against " << tree << " with context " << context << " ... \n";
	}
	assert_decl(auto path = context.getCurrentPath());
	
	// quick check for wildcards (should not even be cached)
	if(pattern.type == impl::TreePattern::Wildcard) {
		return delayedCheck(context);
	}
	...
	if(DEBUG) {
		std::cout << "Matching " << pattern << " against " << tree << " with context " << context << " ... - matched: " << res << "\n";
	}
...
bool match(const impl::ListPattern& pattern, MatchContext<T>& context, const iterator& begin, const iterator& end,
	...
	if(DEBUG) {
		std::cout << "Matching " << pattern << " against " << join(", ", begin, end,
		          print<deref<typename T::value_type>>()) << " with context " << context << " ... \n";
	}
	assert_decl(auto path = context.getCurrentPath());
	...
	if(pattern.minLength > length || length > pattern.maxLength) {
		if(DEBUG) {
			std::cout << "Matching " << pattern << " against " << join(", ", begin, end,
			          print<deref<typename T::value_type>>()) << " with context " << context << " ... skipped due to length limit. \n";
		}
...
//			}
	...
	if(DEBUG) {
		std::cout << "Matching " << pattern << " against " << join(", ", begin, end,
		          print<deref<typename T::value_type>>()) << " with context " << context << " ...  match: " << res << "\n";
	}
