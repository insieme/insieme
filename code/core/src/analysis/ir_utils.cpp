...
private:
	...
	bool visitNamedCompositeType(const NamedCompositeTypePtr& type, TypeVariableSet& boundVars) {
		return any(type->getElements(), [&](const NamedTypePtr& element) {
			return this->visit(element->getType(), boundVars);
		});
	}
	
	bool visitTupleType(const TupleTypePtr& tuple, TypeVariableSet& boundVars) {
		return any(tuple->getElements(), [&](const TypePtr& type) {
			return this->visit(type, boundVars);
		});
	}
	...
	bool visitRecType(const RecTypePtr& recType, TypeVariableSet& boundVars) {
		...
		return any(recType->getDefinition(), [&](const RecTypeBindingPtr& binding) {
			return this->visit(binding->getType(), local);
		});
	}
	
	bool visitType(const TypePtr& type, TypeVariableSet& boundVars) {
		return any(type->getChildList(), [&](const NodePtr& node) {
...
bool isReadOnly(const StatementPtr& stmt, const VariablePtr& var) {
	...
	visitDepthFirstPrunable(NodeAddress(stmt), [&](const NodeAddress& cur) {
		...
		if(cur.getParentNode()->getNodeType() == NT_CompoundStmt) {
			return true;
		}
...
bool compareTypes(const TypePtr& a, const TypePtr& b) {
	...
	//TupleType
	if(a.isa<TupleTypePtr>() && b.isa<TupleTypePtr>()) {
		return a==b;
	}
	//StructType
