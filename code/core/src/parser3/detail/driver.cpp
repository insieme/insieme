...
NodePtr DeclarationContext::find(const std::string& name) const {
	...
	for(auto it = scope_stack.rbegin(); it != scope_stack.rend(); ++it) {
		if((mit = it->find(name)) != it->end()) {
			return mit->second;
		}
	}
	if((mit = global_scope.find(name)) != global_scope.end()) {
		return mit->second;
	}
	return nullptr;
}
...
ExpressionPtr inspire_driver::genBinaryExpression(const location& l, const std::string& op, ExpressionPtr left, ExpressionPtr right) {
	...
	// comparators
	if(op == "==") {
		return builder.eq(a,b);
	}
	if(op == "!=") {
		return builder.ne(a,b);
	}
	if(op == "<") {
		return builder.lt(a,b);
	}
	if(op == ">") {
		return builder.gt(a,b);
	}
	if(op == "<=") {
		return builder.le(a,b);
	}
	if(op == ">=") {
		return builder.ge(a,b);
	}
	
	// bitwise
	if(op == "&") {
		return builder.bitwiseAnd(a,b);
	}
	if(op == "|") {
		return builder.bitwiseOr(a,b);
	}
	if(op == "^") {
		return builder.bitwiseXor(a,b);
	}
	
	// logic
	if(op == "||") {
		return builder.logicOr(a,b);
	}
	if(op == "&&") {
		return builder.logicAnd(a,b);
	}
...
ExpressionPtr inspire_driver::genCall(const location& l, const ExpressionPtr& callable, ExpressionList args) {
	...
	if(!ftype.isa<FunctionTypePtr>()) {
		error(l, "attempt to call non function expression");
	}
	...
	::transform(args, back_inserter(argumentTypes), [](const ExpressionPtr& cur) {
		return cur->getType();
	});
...
bool contains_type_variables(const TypePtr& t, const TypeList& variables) {
	...
	visitDepthFirstOnce(t, [&](const TypePtr& t) {
		if(t.isa<TypeVariablePtr>() && (std::find(variables.begin(), variables.end(), t) != variables.end())) {
			contains = true;
		}
	});
	return contains;
