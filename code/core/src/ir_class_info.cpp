...
namespace core {
	...
	void ClassMetaInfo::setConstructors(const vector<ExpressionPtr>& constructors) {
		...
		assert_true(all(constructors, [&](const ExpressionPtr& cur) {
			return cur->getType().isa<FunctionTypePtr>() && cur->getType().as<FunctionTypePtr>()->isConstructor();
		}));
	...
	void ClassMetaInfo::addMemberFunction(const MemberFunction& function) {
		...
		if(hasMemberFunction(function.getName(), function.getImplementation()->getType().as<FunctionTypePtr>(), function.isConst())) {
			assert_eq(
			    *analysis::normalize(getMemberFunction(function.getName(), function.getImplementation()->getType().as<FunctionTypePtr>(), function.isConst())
			                             ->getImplementation()),
	...
	bool ClassMetaInfo::checkObjectType(const ExpressionPtr& lambda) const {
		...
		if(!typeA) {
			return true; // everything is allowed if object type is not fixed yet
		}
	...
	std::ostream& ClassMetaInfo::printTo(std::ostream& out) const {
		...
		if(!constructors.empty()) {
			out << " -- Constructors --\n"
			    << join("\n", constructors, [](std::ostream& out, const ExpressionPtr& cur) { out << PrettyPrinter(cur, PrettyPrinter::NO_LET_BINDINGS); })
			    << "\n\n";
		}
		...
		if(!memberFunctions.empty()) { out << " -- Member Functions --\n" << join("\n", memberFunctions) << "\n"; }
		out << ">\n";
	...
	bool ClassMetaInfo::migrate(const NodeAnnotationPtr& ptr, const NodePtr& before, const NodePtr& after) const {
		...
		for(auto cur : constructors) {
			if(auto lit = cur.isa<LiteralPtr>()) {
				...
				newInfo.addConstructor(core::transform::replaceNode(mgr, LiteralAddress(lit)->getType().as<FunctionTypeAddress>()->getParameterType(0),
		...
		if(destructor) {
			if(auto lit = destructor.isa<LiteralPtr>()) {
				// update function type
				newInfo.setDestructor(core::transform::replaceNode(mgr, LiteralAddress(lit)->getType().as<FunctionTypeAddress>()->getParameterType(0),
				                                                   builder.refType(newClassType))
				                          .as<LiteralPtr>());
			} else {
				// handle as all other implementations
...
//			}
	...
	void ClassMetaInfo::cloneTo(const NodePtr& target) const {
		...
		for(auto& cur : newInfo.constructors) {
			cur = newMgr.get(cur);
		}
		...
		for(auto& cur : newInfo.memberFunctions) {
			cur.setImplementation(newMgr.get(cur.getImplementation()));
		}
	...
	const NodeList& ClassMetaInfo::getChildNodes() const {
		...
		for(auto cur : memberFunctions) {
			res.push_back(cur.getImplementation());
		}
...
void setMetaInfo(const TypePtr& type, const ClassMetaInfo& info) {
	assert_true(analysis::isObjectType(type)) << "Meta-Information may only be attached to object types!";
	assert_true(!info.getClassType() || *info.getClassType() == *type) << "Target Type: " << *type << "\n"
...
ClassMetaInfo merge(const ClassMetaInfo& a, const ClassMetaInfo& b) {
	...
	assert_eq(*a.getClassType(), *b.getClassType()) << "Cannot merge\n" << a << "and\n" << b;
	...
	for(auto cur : b.getConstructors()) {
		...
		if(!res.containsConstructor(cur)) { res.addConstructor(cur); }
	}
	...
	for(auto cur : b.getMemberFunctions()) {
		if(!containsMember(cur)) { res.addMemberFunction(cur); }
	}

	// update destructor
	if(res.hasDestructor() && b.hasDestructor()) {
		assert_eq(*analysis::normalize(res.getDestructor()), *analysis::normalize(b.getDestructor())) << "Unable to merge distinct destructors!";
