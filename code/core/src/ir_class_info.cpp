...
void ClassMetaInfo::setConstructors(const vector<ExpressionPtr>& constructors) {
	assert_true(all(constructors, [&](const ExpressionPtr& cur) {
		return cur->getNodeType() == NT_Literal || cur->getNodeType() == NT_LambdaExpr;
	}));
...
void ClassMetaInfo::setMemberFunctions(const vector<MemberFunction>& functions) {
	// check new functions
	assert_true(all(functions, [&](const MemberFunction& cur) {
		return cur.getImplementation()->getType().as<FunctionTypePtr>()->isMemberFunction();
	}));
	assert_true(all(functions, [&](const MemberFunction& cur) {
		return checkObjectType(cur.getImplementation());
	}));
...
void ClassMetaInfo::addMemberFunction(const MemberFunction& function) {
	...
	if(hasMemberFunction(function.getName(), function.getImplementation()->getType().as<FunctionTypePtr>(), function.isConst())) {
		assert_eq(*analysis::normalize(getMemberFunction(function.getName(), function.getImplementation()->getType().as<FunctionTypePtr>(),
		                               function.isConst())->getImplementation()),
...
TypePtr ClassMetaInfo::getClassType() const {
	...
	// try destructor
	if(destructor) {
		return destructor->getType().as<FunctionTypePtr>()->getObjectType();
	}
	...
	if(!memberFunctions.empty()) {
		return memberFunctions.front().getImplementation()->getType().as<FunctionTypePtr>()->getObjectType();
	}
...
void ClassMetaInfo::cloneTo(const NodePtr& target) const {
	...
	// migrate destructor
	if(newInfo.destructor) {
		newInfo.destructor = newMgr.get(newInfo.destructor);
	}
