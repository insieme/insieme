...
CodeLocation CodeLocation::shift(const NodeAddress& outer, const utils::AnnotationKeyPtr& key) const {
	...
	assert(any(outer.getAnnotation(key)->getChildNodes(), [&](const NodePtr& cur) {
		return cur == address.getRootNode();
	}));
...
public:
	...
	CombinedIRCheck(const CheckList& checks = CheckList())
		: IRCheck(any(checks, [](const CheckPtr& cur) {
		return cur->isVisitingTypes();
	})), checks(checks) {};
