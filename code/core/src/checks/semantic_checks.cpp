	...
	OptionalMessageList ValidInitExprMemLocationCheck::visitInitExpr(const InitExprAddress& initExpr) {
		...
		if(!memLocExpr.isa<VariablePtr>() && !memLocExpr.isa<LiteralPtr>() && !refExt.isCallOfRefTemp(memLocExpr)
		   && !refExt.isCallOfRefMemberAccess(memLocExpr)) {
			add(res, Message(initExpr, EC_SEMANTIC_INVALID_INIT_MEMLOC,
				             format("InitExpr must initialize memory of a variable, literal, member, or temporary generated by ref_temp."), Message::ERROR));
		}
