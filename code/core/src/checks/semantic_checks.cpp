...
namespace checks {
	...
	OptionalMessageList ValidInitExprMemLocationCheck::visitInitExpr(const InitExprAddress& initExpr) {
		...
		// memloc must be either a variable, literal, or a call to ref_temp
		if(!memLocExpr.isa<VariablePtr>() && !memLocExpr.isa<LiteralPtr>() && !refExt.isCallOfRefTemp(memLocExpr)) {
			add(res, Message(initExpr, EC_SEMANTIC_INVALID_INIT_MEMLOC, format("InitExpr must initialize memory of variable or generated by ref_temp."),
				             Message::ERROR));
		}
