...
core::ExpressionPtr handleMemAlloc(const core::IRBuilder& builder,
		const core::TypePtr& type, const core::ExpressionPtr& subExpr) {
	...
	if ( core::CallExprPtr&& callExpr = core::dynamic_pointer_cast<const core::CallExpr>(subExpr)) {
		...
		if ( core::LiteralPtr&& lit = core::dynamic_pointer_cast<const core::Literal>(callExpr->getFunctionExpr())) {
			...
			if (lit->getStringValue() == "malloc"
				...
				assert(
						core::analysis::getReferencedType(type)->getNodeType() == core::NT_ArrayType);

				const core::RefTypePtr& refType = core::static_pointer_cast<
						const core::RefType>(type);
			...
			if (!ctx.isResolvingRecFuncBody) {
				...
				if (fit != ctx.lambdaExprCache.end()) {
					...
					for (it = temporaries.begin(); it < temporaries.end();
						...
						funcTy = addThisArgToFunctionType(builder,
...
core::ExpressionPtr VisitCXXMemberCallExpr(
	...
	retExpr = convFact.builder.callExpr(funcTy->getReturnType(),
			lambdaExpr, packedArgs);
...
core::ExpressionPtr VisitCXXOperatorCallExpr(
	...
	if (const CXXMethodDecl* cxxMethodDecl = dyn_cast<CXXMethodDecl>(callExpr->getDirectCallee())) {
		...
		assert(
				classTypePtr && "no class declaration to type pointer mapping");
		funcTy = addThisArgToFunctionType(builder, classTypePtr,
				funcTy);
	}
	...
	for (it = temporaries.begin(); it < temporaries.end(); it++) {
		...
		funcTy = addThisArgToFunctionType(builder, builder.deref(var).getType(),
				funcTy);
...
core::ExpressionPtr VisitMemberExpr(clang::MemberExpr* membExpr) {
	...
	assert(
			(structTy->getNodeType() == core::NT_StructType || structTy->getNodeType() == core::NT_UnionType || structTy->getNodeType() == core::NT_RecType) && "Using a member access operation on a non struct/union type");
...
core::ExpressionPtr VisitExtVectorElementExpr(
	...
	pos = "3";
		...
		llvm::StringRef numStr = accessor.substr(1,
				accessor.size() - 1);
		assert(
				insieme::utils::numeric_cast<unsigned int>(numStr.data()) >= 0 && "Vector accessing string is not a number");
		pos = numStr;
	} else if (accessor.size() <= 4) { // opencl vector permutation
		vector<core::ExpressionPtr> args;

		for (auto I = accessor.begin(), E = accessor.end(); I != E;
				++I) {
			args.push_back(
					convFact.builder.intLit(
							*I == 'w' ? 3 : (*I) - 'x')); //convert x, y, z, w to 0, 1, 2, 3
		}
		return (retIr = convFact.builder.callExpr(
						gen.getVectorPermute(), convFact.tryDeref(base),
						convFact.builder.vectorExpr(args)));
	} else {
		assert(
				accessor.size() <= 4 && "ExtVectorElementExpr has unknown format");
	}
...
core::NodePtr ConversionFactory::convertFunctionDecl(
	...
	if (isEntryPoint && ctx.globalVar) {
		...
		stmts[0] = builder.declarationStmt(ctx.globalVar,
				builder.refNew(ctx.globalStruct.second));
		std::copy(compStmt->getStatements().begin(),
				compStmt->getStatements().end(), stmts.begin() + 1);
