...
namespace {
	...
	struct InitializationCollector : public core::IRVisitor<bool, core::Address> {
		...
		InitializationCollector(const core::ExpressionPtr& ind) : core::IRVisitor<bool, core::Address>(), inductionExpr(ind), isDecl(false) {
...
namespace analysis {
	...
	LoopAnalyzer::LoopAnalyzer(const clang::ForStmt* forStmt, Converter& convFact) : convFact(convFact), loopToBoundary(false), restoreValue(false) {
		...
		if(core::types::isRefArray(inductionVar->getType()) || core::types::isRefArray(endValue->getType()) || core::types::isRefArray(initValue->getType())) {
			...
			if(invertComparisonOp) {
				...
				if(mgr.getLangBasic().isUnsignedInt(currentType)) {
					TypePtr newType =
					    builder.genericType("int", TypeList(), toVector(currentType.as<insieme::core::GenericTypePtr>().getIntTypeParameter()->getElement(0)));
			...
			if(loopToBoundary) { endValue = convFact.getIRBuilder().add(endValue, convFact.getIRBuilder().literal("1", inductionVar->getType())); }
	...
	void LoopAnalyzer::findInductionVariable(const clang::ForStmt* forStmt) {
		...
		if(const BinaryOperator* binOp = dyn_cast<const BinaryOperator>(cond)) {
			...
			if(core::CallExprPtr call = originalInductionExpr.isa<core::CallExprPtr>()) {
				if(convFact.getIRBuilder().getLangBasic().isScalarCast(call.getFunctionExpr())) { originalInductionExpr = call[0]; }
			}
	...
	void LoopAnalyzer::handleIncrExpr(const clang::ForStmt* forStmt) {
		...
		if(const UnaryOperator* unOp = dyn_cast<const UnaryOperator>(forStmt->getInc())) {
			...
			case UO_PreInc:
			case UO_PostInc: stepExpr = incrExpr; return;
			case UO_PreDec:
			case UO_PostDec: stepExpr = convFact.getIRBuilder().invertSign(incrExpr); return;
			default: LoopNormalizationError("UnaryOperator different from post/pre inc/dec (++/--) not supported in loop increment expression");
			}
		}
		...
		if(const BinaryOperator* binOp = dyn_cast<const BinaryOperator>(forStmt->getInc())) {
			...
			case BO_AddAssign: stepExpr = tmpStep; return;
			case BO_SubAssign: stepExpr = convFact.getIRBuilder().invertSign(tmpStep); return;
			default: LoopNormalizationError("unable to produce a for loop with " + binOp->getOpcodeStr().str() + " condition");
			}
		}
	...
	void LoopAnalyzer::handleCondExpr(const clang::ForStmt* forStmt) {
		...
		if(!cond) { throw LoopNormalizationError("no condition -> no for loop"); }
		...
		if(const BinaryOperator* binOp = dyn_cast<const BinaryOperator>(cond)) {
			...
			default: throw LoopNormalizationError("BinOp (" + binOp->getOpcodeStr().str() + ") in ConditionExpression not supported");
			}
