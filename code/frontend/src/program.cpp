...
// headers reqd for cleanup transform
...
public:
	...
	void visitCallExpr(const CallExprPtr& call) {
		...
		if(basic.isArithOp(fun) || basic.isArrayOp(fun)) sideEffectFree = true;
		if((litFun = dynamic_pointer_cast<const Literal>(fun))) {
			const string& name = litFun->getStringValue();
			if(name == "ceil" || name == "floor") sideEffectFree = true;
		}
		if(sideEffectFree) {
			// visit arguments
 ...
 */
core::ProgramPtr applyCleanup(core::ProgramPtr& prog, core::NodeManager& mgr) {
