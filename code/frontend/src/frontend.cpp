...
ConversionSetup::ConversionSetup(const vector<path>& includeDirs)
	: includeDirs(includeDirs),
	  systemHeaderSearchPath(::transform(insieme::utils::compiler::getDefaultCppIncludePaths(), [](const string& cur) {
	return path(cur);
})),
standard(Auto),
definitions(),
...
bool ConversionJob::isCxx() const {
	if(getStandard() == Standard::Cxx03 || getStandard() == Cxx98 || getStandard() == Standard::Cxx11) {
		return true;
	}
	if(getStandard() == Standard::C99) {
		return false;
	}
