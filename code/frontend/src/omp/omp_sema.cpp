...
protected:
	...
	virtual const NodePtr resolveElement(const NodePtr& node) {
		...
		if(newNode != node) {
			transform::utils::migrateAnnotations(node, newNode);
		}
			...
			if(CompoundStmtPtr newCompound = dynamic_pointer_cast<const CompoundStmt>(newNode)) {
				...
				for(auto i = sl.begin(); i != sl.end(); ++i) {
					...
					if(innerCompound && toFlatten.contains(innerCompound)) {
						...
						for_each(innerCompound->getStatements(), [&](const StatementPtr s) {
							newSl.push_back(s);
						});
					}
					else {
	...
	NodePtr handleFunctions(const NodePtr& newNode) {
		if(CallExprPtr callExp = dynamic_pointer_cast<const CallExpr>(newNode)) {
			...
			if(basic.isScalarToArray(fun)) {
				...
				if(analysis::isRefOf(arg, basic.getLock())) {
					return arg;
				}
				return newNode;
			}
				...
				// OMP Locks --------------------------------------------
					...
					if(analysis::isCallOf(arg, basic.getScalarToArray())) {
						arg = analysis::getArgument(arg, 0);
					}
	...
	NodePtr handleTypes(const NodePtr& newNode) {
		if(TypePtr type = newNode.isa<core::TypePtr>()) {
			...
			if(ArrayTypePtr arr = dynamic_pointer_cast<ArrayTypePtr>(type)) {
	...
	NodePtr handleThreadprivate(const NodePtr& node, bool masterCopy = false) {
		...
		if(masterCopy) {
			indexExpr = build.literal(basic.getUInt8(), "0");
		}
...
//				break;
	...
	StatementPtr implementDataClauses(const StatementPtr& stmtNode, const DatasharingClause* clause, StatementList& outsideDecls,
		...
		if(taskP) {
			...
			std::copy_if(freeVarsAndFuns.begin(), freeVarsAndFuns.end(), back_inserter(freeVars), [&](const VariablePtr& v) {
				...
				if(taskP->hasShared()) {
					ret = ret && !contains(taskP->getShared(), v);
				}
				if(taskP->hasFirstPrivate()) {
					ret = ret && !contains(taskP->getFirstPrivate(), v);
				}
				if(taskP->hasPrivate()) {
					ret = ret && !contains(taskP->getPrivate(), v);
				}
				if(taskP->hasFirstLocal()) {
					ret = ret && !contains(taskP->getFirstLocal(), v);
				}
				if(taskP->hasLocal()) {
					ret = ret && !contains(taskP->getLocal(), v);
				}
				if(taskP->hasReduction()) {
					ret = ret && !contains(taskP->getReduction().getVars(), v);
				}
		...
		if(clause->hasReduction()) {
			allp.insert(allp.end(), clause->getReduction().getVars().begin(), clause->getReduction().getVars().end());
		}
		...
		if(!publicToPrivateAddressMap.empty()) {
			subStmt = transform::pushInto(nodeMan, publicToPrivateAddressMap).as<StatementPtr>();
		}
		
		// specific handling if clause is a omp for
		if(forP) {
			if(forP->hasOrdered()) {
				subStmt = processOrderedFor(subStmt);
			}
			// Handling lastlocal
		...
		if(clause->hasReduction()) {
			replacements.push_back(implementReductions(clause, publicToPrivateMap));
		}
		// specific handling if clause is a omp for (insert barrier if not nowait)
		if(forP && !forP->hasNoWait()) {
			replacements.push_back(build.barrier());
		}
	...
	StatementPtr implementParamClause(const StatementPtr& stmtNode, const SharedOMPPPtr& reg) {
		...
		if(param.hasRange()) {
			...
			auto pick 	= (!param.hasQualityRange()) ? build.pickInRange(build.intLit(paramCounter++), core::types::castScalar(basic.getUInt8(), param.getEnumSize()))
			              : build.pickInRange(build.intLit(paramCounter++), core::types::castScalar(basic.getUInt8(), param.getEnumSize()), param.getQualityRangeLBound(),
	...
	NodePtr handleParallel(const StatementPtr& stmtNode, const ParallelPtr& par) {
		...
		if(par->hasNumThreads()) {
			range = build.getThreadNumRange(par->getNumThreads(), par->getNumThreads());
		}
	...
	StatementPtr processOrderedParallel(const StatementPtr& origStmt) {
		...
		visitDepthFirst(StatementAddress(origStmt), [&](const LiteralAddress& lit) { // could prune this to improve performance
			if(lit.getAddressedNode() == orderedCountLit) {
				pushMap.insert(std::make_pair(lit, orderedCountVar));
			}
		});
	...
	StatementPtr processOrderedFor(const StatementPtr& origStmt) {
		...
		visitDepthFirst(CompoundStmtAddress(newBody), [&](const LiteralAddress& lit) { // could prune this to improve performance
			if(lit.getAddressedNode() == orderedIncLit) {
				pushMap.insert(std::make_pair(lit, stepVar));
			}
			if(lit.getAddressedNode() == orderedItLit) {
				pushMap.insert(std::make_pair(lit, iterator));
			}
		});
	...
	NodePtr handleAtomic(const StatementPtr& stmtNode, const AtomicPtr& atomicP) {
		...
		if(!call) {
			cerr << printer::PrettyPrinter(stmtNode) << std::endl;
		}
