...
namespace {
	...
	bool isCppConstructor(const core::ExpressionPtr& expr) {
		// constructor
		if(core::analysis::isConstructorCall(expr)) { return true; }
	...
	core::ExpressionPtr convertInitForGlobal(insieme::frontend::conversion::Converter& converter, const clang::VarDecl* var, core::TypePtr elementType) {
		...
		if(var->hasInit() && !var->isStaticLocal()) {
			...
			if(outDecl->hasInit() && !outDecl->getAnyInitializer()->getType().getTypePtr()->isDependentType()
		...
		if(initValue) { initValue = converter.getInitExpr(elementType.as<core::RefTypePtr>()->getElementType(), initValue); }
...
namespace conversion {
	...
	Converter::Converter(core::NodeManager& mgr, const TranslationUnit& tu, const ConversionSetup& setup)
	    : staticVarCount(0), translationUnit(tu), convSetup(setup), pragmaMap(translationUnit.pragmas_begin(), translationUnit.pragmas_end()),
	      irTranslationUnit(mgr), used(false), mgr(mgr), builder(mgr), feIR(mgr, getCompiler().isCXX()), lastTrackableLocation(),
	...
	core::TypePtr Converter::tryDeref(const core::TypePtr& type) const {
		if(core::RefTypePtr&& refTy = core::dynamic_pointer_cast<const core::RefType>(type)) { return refTy->getElementType(); }
		return type;
	}
	...
	core::ExpressionPtr Converter::lookUpVariable(const clang::ValueDecl* valDecl) {
		...
		if(!result) {
			...
			if(varTy.isVolatileQualified()) { irType = builder.volatileType(irType); }
			...
			if(!(varTy.isConstQualified() ||                // is a constant
			     ...
			     (isa<const clang::ParmVarDecl>(valDecl) && // is the declaration of a parameter
			      ((irType->getNodeType() != core::NT_VectorType && irType->getNodeType() != core::NT_ArrayType) || isOclVector || isGCCVector)))) {
				...
				if(core::types::isRefArray(irType) && varTy.isConstQualified() && !llvm::isa<clang::ParmVarDecl>(valDecl)) { irType = builder.refType(irType); }
			}
			...
			if(varDecl->isStaticLocal() && !irType.isa<core::RefTypePtr>()) { irType = builder.refType(irType); }
			...
			if(varDecl && varDecl->hasGlobalStorage() && !varDecl->isStaticLocal()) {
				...
				if(!irType.isa<core::RefTypePtr>()) { irType = builder.refType(irType); }
			...
			if(annotations::c::hasIncludeAttached(irType)) { VLOG(2) << " header " << annotations::c::getAttachedInclude(irType); }
		}
	...
	core::ExpressionPtr Converter::defaultInitVal(const core::TypePtr& valueType) const {
		...
		if(mgr.getLangBasic().isReal(type)) {
			...
			if(mgr.getLangBasic().isReal4(type)) { return builder.literal("0.0f", type); }
			if(mgr.getLangBasic().isReal8(type)) { return builder.literal("0.0", type); }
			if(mgr.getLangBasic().isReal16(type)) { return builder.literal("0.0", type); }
		}
		...
		if(mgr.getLangBasic().isString(type)) { return builder.literal("", type); }
		...
		if(type->getNodeType() == core::NT_RecType) { curType = type.as<core::RecTypePtr>()->unroll(); }

		// Handle structs initialization
		if(curType.isa<core::StructTypePtr>()) { return builder.getZero(type); }

		// Handle unions initialization
		if(curType.isa<core::UnionTypePtr>()) { return builder.getZero(type); }
		...
		if(core::VectorTypePtr&& vecTy = core::dynamic_pointer_cast<const core::VectorType>(curType)) {
			...
			return builder.callExpr(vecTy, mgr.getLangBasic().getVectorInitUniform(), initVal, builder.getIntTypeParamLiteral(vecTy->getSize()));
		}

		// handle any-ref initialization
		if(mgr.getLangBasic().isAnyRef(type)) { return mgr.getLangBasic().getRefNull(); }

		// handle enum values
		auto& enumExt = mgr.getLangExtension<core::lang::EnumExtension>();
		if(enumExt.isEnumType(type)) { return builder.literal(type, "0"); }
	...
	core::StatementPtr Converter::convertVarDecl(const clang::VarDecl* varDecl) {
		...
		VLOG(1) << "\n****************************************************************************************\n"
		        ...
		        << "-> at location: (" << utils::location(varDecl->getLocation(), getSourceManager()) << "): ";
		if(VLOG_IS_ON(2)) {
		...
		if(const VarDecl* definition = varDecl->getDefinition()) {
			...
			if(definition->isStaticLocal()) {
				...
				if(definition->getInit()) {
					initIr = convertInitExpr(definition->getType().getTypePtr(), definition->getInit(), var->getType().as<core::RefTypePtr>().getElementType(),
					                         false);

					auto call = initIr.isa<core::CallExprPtr>();
					if(call && call->getFunctionExpr()->getType().as<core::FunctionTypePtr>()->isConstructor()) { initIr = builder.deref(initIr); }
					...
					if(call && call->getFunctionExpr()->getType().as<core::FunctionTypePtr>()->isConstructor() && call->getArguments().size() == 1
				...
				if(!core::analysis::isAnyCppRef(var->getType()) && !isCppConstructor(initExpr) && !isConstant) { initExpr = builder.refVar(initExpr); }

				// constants need to be derefed
				if(isConstant && isCppConstructor(initExpr)) { initExpr = builder.tryDeref(initExpr); }
	...
	//////////////////////////////////////////////////////////////////
	///
	core::ExpressionPtr Converter::convertInitExpr(const clang::Type* clangType, const clang::Expr* expr, const core::TypePtr& type, const bool zeroInit) {
		...
		if(!expr) {
			...
			if(type.isa<core::RefTypePtr>()) { return (zeroInit) ? builder.getZero(type) : builder.undefined(type); }
			...
			if(kind == core::NT_ArrayType && clangType && llvm::isa<clang::VariableArrayType>(clangType)) {
				...
				return retIr = builder.callExpr(type, mgr.getLangBasic().getArrayCreate1D(), builder.getTypeLiteral(arrType->getElementType()),
				                                builder.castExpr(mgr.getLangBasic().getUInt8(), size));
			}
		...
		if(core::analysis::isCallOf(retIr, builder.getLangBasic().getRefDeref()) && isCppConstructor(retIr.as<core::CallExprPtr>()[0])) {
	...
	core::ExpressionPtr Converter::getInitExpr(const core::TypePtr& targetType, const core::ExpressionPtr& init) {
		...
		if(core::encoder::isListType(init->getType())) {
			...
			if(core::lang::isSIMDVector(elementType)) {
				...
				return builder.callExpr(elementType, initOp,
			...
			if(elementType.isa<core::VectorTypePtr>() || elementType.isa<core::ArrayTypePtr>()) {
				...
				if(elementType.isa<core::VectorTypePtr>()
				   && retIr->getType().as<core::VectorTypePtr>()->getSize() != elementType.isa<core::VectorTypePtr>()->getSize()) {
					return builder.callExpr(builder.getLangBasic().getVectorInitPartial(),
					                        core::encoder::toIR<ExpressionList, core::encoder::DirectExprListConverter>(targetType->getNodeManager(), elements),
			...
			if(core::StructTypePtr&& structTy = elementType.isa<core::StructTypePtr>()) {
				...
				for(size_t i = 0; i < inits.size(); ++i) {
					...
					members.push_back(builder.namedValue(curr->getName(), getInitExpr(curr->getType(), inits[i])));
				}
				if(members.empty()) {
					...
					retIr = core::transform::replaceNode(mgr, core::NodeAddress(retIr).as<core::StructExprAddress>()->getType(), targetType)
			...
			if(core::GenericTypePtr&& gen = elementType.isa<core::GenericTypePtr>()) {
				...
				if(inits.empty()) { return builder.callExpr(mgr.getLangBasic().getUndefined(), builder.getTypeLiteral(gen)); }

				if(core::encoder::isListType(inits[0]->getType())) {
					vector<core::ExpressionPtr> innerList =
					    core::encoder::toValue<vector<core::ExpressionPtr>, core::encoder::DirectExprListConverter>(inits[0]);
		...
		if(core::UnionTypePtr unionTy = elementType.isa<core::UnionTypePtr>()) {
			...
			if(init.as<core::CallExprPtr>()->getFunctionExpr().isa<core::LiteralPtr>()) {
				...
				for(unsigned i = 0; i < unionTy->getEntries().size(); ++i)
					if(*unionTy->getEntries()[i]->getName() == *name) { entityType = unionTy->getEntries()[0]->getType(); }
		...
		if(core::analysis::isConstCppRef(elementType) && !init->getType().isa<core::RefTypePtr>()) {
			return (retIr = builder.callExpr(mgr.getLangExtension<core::lang::IRppExtensions>().getRefIRToConstCpp(), builder.refVar(init)));
			// builder.callExpr (mgr.getLangExtension<core::lang::IRppExtensions>().getMaterialize(), init)));
		}

		// init const ref with value, extend lifetime  ( const T&& x = f() where f returns by value )
		if(core::analysis::isConstRValCppRef(elementType) && !init->getType().isa<core::RefTypePtr>()) {
			return (retIr = builder.callExpr(mgr.getLangExtension<core::lang::IRppExtensions>().getRefIRToConstRValCpp(), builder.refVar(init)));
			// builder.callExpr (mgr.getLangExtension<core::lang::IRppExtensions>().getMaterialize(), init)));
		}
		...
		if(builder.getLangBasic().isAny(elementType)) { return (retIr = init); }
		...
		if(elementType.isa<core::VectorTypePtr>()) {
			...
			if(core::types::isRefVector(init->getType())) { initVal = builder.deref(initVal); }
			// it can be a partial initialization
			if(core::types::isSubTypeOf(initVal->getType(), elementType)) { return (retIr = initVal); }

			return (retIr = core::types::smartCast(initVal, elementType));
		}

		if(core::analysis::isVolatileType(elementType)) {
			if(!core::analysis::isVolatileType(init->getType())) { return (retIr = builder.makeVolatile(init)); }
			return (retIr = init);
		}

		// the case of enum type initializations
		if(mgr.getLangExtension<core::lang::EnumExtension>().isEnumType(init->getType())) { return (retIr = core::types::castScalar(elementType, init)); }
	...
	core::FunctionTypePtr Converter::convertFunctionType(const clang::FunctionDecl* funcDecl) {
		...
		switch(funcKind) {
		case core::FK_MEMBER_FUNCTION: returnType = funcType.getReturnType(); break;
		case core::FK_CONSTRUCTOR:
		case core::FK_DESTRUCTOR: returnType = thisType; break;
		default: assert_fail() << "invalid state!"; break;
		}
	...
	namespace {
		...
		core::StatementPtr prepentInitializerList(const clang::CXXConstructorDecl* ctorDecl, const core::TypePtr& classType, const core::StatementPtr& body,
		                                          Converter& converter) {
			...
			assert((ctorDecl->getParent()->getName().empty() || (!ctorDecl->getParent()->getName().empty() && classType.isa<core::GenericTypePtr>()))
			       && "for convenion, this literal must keep the generic type");
			...
			for(auto it = ctorDecl->init_begin(); it != ctorDecl->init_end(); ++it) {
				...
				if((*it)->isBaseInitializer()) {
					...
					toInit = builder.callExpr(builder.refType(membTy), builder.getLangBasic().getCompositeRefElem(), genThis,
					...
					if(isCtor) {
						...
						// parameter is NOT cpp_ref but left hand side is -> wrap into cppref
							...
							if(!core::types::isRefArray(expr->getType())
							   && !(core::types::isRefRef(toInit.getType()) && builder.getLangBasic().isAnyRef(expr->getType()))) {
					...
					for(; ind_it != end; ++ind_it) {
						...
						toInit = builder.callExpr(builder.refType(fieldTy), builder.getLangBasic().getCompositeRefElem(), toInit,
						                          builder.getIdentifierLiteral(ident), builder.getTypeLiteral(fieldTy));
					}
	...
	void Converter::convertFunctionDeclImpl(const clang::FunctionDecl* funcDecl) {
		...
		core::LambdaExprPtr lambda;
		{
			...
			std::for_each(funcDecl->param_begin(), funcDecl->param_end(),
			              [&](ParmVarDecl* currParam) { params.push_back(core::static_pointer_cast<const core::Variable>(this->lookUpVariable(currParam))); });
		...
		if(funcTy->isMember()) {
			...
			if(funcTy->isConstructor()) {
				...
				if(!classInfo.containsConstructor(symbol)) { classInfo.addConstructor(symbol); }
				...
				if(!classInfo.hasMemberFunction(functionname, funcTy, llvm::cast<clang::CXXMethodDecl>(funcDecl)->isConst())) {
					classInfo.addMemberFunction(functionname, symbol, llvm::cast<clang::CXXMethodDecl>(funcDecl)->isVirtual(),
	...
	core::ExpressionPtr Converter::convertFunctionDecl(const clang::FunctionDecl* funcDecl, bool symbolic) {
		...
		for(auto extension : this->getConversionSetup().getExtensions()) {
			...
			if(ret) { expr = ret.as<core::ExpressionPtr>(); }
		}
