...
bool isCppConstructor(const core::ExpressionPtr& expr) {
	...
	if(core::analysis::isCallOf(expr, mgr.getLangExtension<core::lang::IRppExtensions>().getVectorCtor2D())) {
		return true;
	}
...
tu::IRTranslationUnit Converter::convert() {
	...
	//frontend done
	if(getConversionSetup().hasOption(ConversionSetup::ProgressBar)) {
		std::cout << std::endl;
	}
...
// check for overloaded operator "function" (normal function has kind OO_None)
	...
	if(core::annotations::hasAttachedName(node)) {
		VLOG(2) << "attachedName: " << core::annotations::getAttachedName(node);
	}
...
Converter::convertInitExpr(const clang::Type* clangType, const clang::Expr* expr, const core::TypePtr& type, const bool zeroInit)  {
	...
	if(lookupTypeDetails(type) ==  lookupTypeDetails(retIr->getType())) {
		return retIr;
	}
