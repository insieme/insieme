...
std::ostream& IRTranslationUnit::printTo(std::ostream& out) const {
	static auto print = [](const core::NodePtr& node) {
		return core::printer::printInOneLine(node);
	};
	return out << "TU(\n\tTypes:\n\t\t"
	<< join("\n\t\t", types, [&](std::ostream& out, const std::pair<core::GenericTypePtr, core::TypePtr>& cur) {
		out << *cur.first << " => " << *cur.second;
	})
	        << ",\n\tGlobals:\n\t\t"
	<< join("\n\t\t", globals, [&](std::ostream& out, const std::pair<core::LiteralPtr, core::ExpressionPtr>& cur) {
		...
		if(cur.second) {
	        ...
	        << ",\n\tInitializer:\n\t\t"
	<< join("\n\t\t", initializer, [&](std::ostream& out, const core::StatementPtr& cur) {
		out << print(cur);
	})
	        << ",\n\tFunctions:\n\t\t"
	<< join("\n\t\t", functions, [&](std::ostream& out, const std::pair<core::LiteralPtr, core::ExpressionPtr>& cur) {
		out << *cur.first << " : " << *cur.first->getType() << " => " << print(cur.second);
	})
	        << ",\n\tEntry Points:\t{"
	<< join(", ", entryPoints, [&](std::ostream& out, const core::LiteralPtr& cur) {
		out << *cur;
	})
	        << "},\n\tMetaInfos:\t{"
...
IRTranslationUnit merge(core::NodeManager& mgr, const vector<IRTranslationUnit>& units) {
	...
	res.setCXX(any(units, [](const IRTranslationUnit& cur) {
		return cur.isCXX();
	}));
	return res;
}
...
private:
	...
	bool containsSymbols(const NodePtr& node) const {
		...
		if(pos != containsSymbolsCache.end()) {
			return pos->second;
		}
		
		// compute result recursively
		bool res = isSymbol(node) || any(node->getChildList(), [&](const NodePtr& cur) {
	...
	const NodeSet& getContainedSymbols(const NodePtr& node) const {
		...
		if(!containsSymbols(node)) {
			return empty;
		}
		
		// first check cache
		auto pos = containedSymbols.find(node);
		if(pos != containedSymbols.end()) {
			return pos->second;
		}
	...
	void resolveComponents(const RecComponentGraph& graph) {
		...
		for(const auto& cur : components) {
			...
			for(const auto& s : vars) {
				...
				if(s.isa<TypePtr>()) {
					core::removeMetaInfo(s.as<TypePtr>());
				}
...
core::LambdaExprPtr addGlobalsInitialization(const IRTranslationUnit& unit, const core::LambdaExprPtr& mainFunc, Resolver& resolver) {
	...
	for(auto cur : unit.getGlobals()) {
		...
		if(!globalTy.isa<RefTypePtr>() || !globalTy.as<RefTypePtr>()->getElementType().isa<VectorTypePtr>()) {
			continue;
		}
	...
	for(auto cur : unit.getGlobals()) {
		...
		if(!contains(usedLiterals, newLit)) {
			continue;
		}
	...
	for(auto cur : usedLiterals) {
		...
		if(!type.isa<RefTypePtr>() || !ext.isStaticType(type.as<RefTypePtr>()->getElementType())) {
			continue;
		}
	...
	for(auto cur : usedLiterals) {
		...
		!any(unit.getGlobals(), [&](const IRTranslationUnit::Global& global) {
			return *resolver.apply(global.first) == *cur;
		})
		                                   );
	}
