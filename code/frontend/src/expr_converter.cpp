...
namespace exprutils {
	...
	core::ExpressionPtr getCArrayElemRef(const core::IRBuilder& builder, const core::ExpressionPtr& expr) {
		...
		if(exprTy->getNodeType() == core::NT_RefType) {
			...
			if(subTy->getNodeType() == core::NT_VectorType || subTy->getNodeType() == core::NT_ArrayType) {
				...
				return builder.callExpr(builder.refType(elemTy), (subTy->getNodeType() == core::NT_VectorType ? builder.getLangBasic().getVectorRefElem()
	...
	////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	//
	core::ExpressionPtr getMemberAccessExpr(frontend::conversion::Converter& convFact, const core::IRBuilder& builder, core::ExpressionPtr base,
		...
		if(baseTy.isa<core::GenericTypePtr>()) {
			...
			if(!membType && llvm::isa<const clang::VarDecl>(membExpr->getMemberDecl())
			   && llvm::dyn_cast<const clang::VarDecl>(membExpr->getMemberDecl())->isStaticDataMember()) {
				ident = builder.stringValue(frontend::utils::buildNameForVariable(llvm::dyn_cast<const clang::VarDecl>(membExpr->getMemberDecl())));
		...
		if(base->getType()->getNodeType() == core::NT_RefType) { returnType = builder.refType(returnType); }
...
namespace conversion {
	...
	core::ExpressionPtr Converter::ExprConverter::fixType(const core::ExpressionPtr& expr, const core::TypePtr& targetType) {
		...
		// into a var
		if(core::analysis::isAnyCppRef(targetType) && type.isa<core::FunctionTypePtr>()) { res = builder.refVar(res); }
		...
		if(core::analysis::isCppRef(targetType)) {
			...
			if(!res->getType().isa<core::RefTypePtr>()) { res = builder.refVar(res); }
	...
	core::ExpressionPtr Converter::ExprConverter::wrapVariable(const clang::Expr* expr) {
		...
		if(ref && llvm::isa<clang::ParmVarDecl>(ref->getDecl())) {
			const core::VariablePtr& parmVar = core::static_pointer_cast<const core::Variable>(convFact.convertExpr(ref));

			auto fit = convFact.wrapRefMap.find(parmVar);
			if(fit == convFact.wrapRefMap.end()) {
				fit = convFact.wrapRefMap.insert(std::make_pair(parmVar, builder.variable(builder.refType(parmVar->getType())))).first;
			}
			return fit->second;
		}
	...
	core::ExpressionPtr Converter::ExprConverter::asLValue(const core::ExpressionPtr& value) {
		...
		if(core::analysis::isAnyCppRef(irType)) { return builder.toIRRef(value); }
		...
		if(const core::RefTypePtr& refTy = irType.isa<core::RefTypePtr>()) {
			...
			if(core::analysis::isCppRef(refTy->getElementType())) { return builder.toIRRef(builder.deref(value)); }
		...
		if(value->getNodeType() != core::NT_CallExpr || irType.isa<core::RefTypePtr>()) { return value; }
		...
		if(const core::CallExprPtr& call = value.isa<core::CallExprPtr>()) {
			...
			if(core::analysis::isCallOf(call, gen.getRefDeref())) { return call->getArgument(0); }
			...
			if(core::analysis::isCallOf(call, gen.getArraySubscript1D())) {
				...
				if(*inner != *arg) { return builder.callExpr(builder.refType(irType), gen.getArrayRefElem1D(), inner, call->getArgument(1)); }
			}
			...
			if(core::analysis::isCallOf(call, gen.getVectorSubscript())) {
				const core::ExpressionPtr arg = call->getArgument(0);
				const core::ExpressionPtr inner = asLValue(arg);
				if(*inner != *arg) { return builder.callExpr(builder.refType(irType), gen.getVectorRefElem(), inner, call->getArgument(1)); }
			}
			...
			if(core::analysis::isCallOf(call, gen.getCompositeMemberAccess())) {
				...
				const core::ExpressionPtr inner = asLValue(arg);
				// if (*inner != *arg) {
				return builder.callExpr(builder.refType(irType), gen.getCompositeRefElem(), inner, call->getArgument(1), call->getArgument(2));
				//}
			}

			if(core::analysis::isCallOf(call, gen.getRefVectorToRefArray())) { return call; }
		}
	...
	core::ExpressionPtr Converter::ExprConverter::asRValue(const core::ExpressionPtr& value) {
		...
		if(core::analysis::isRefType(value->getType()) && !(core::types::isRefVector(type) || core::types::isRefArray(type))) { return builder.deref(value); }
		return value;
	}
	...
	core::ExpressionPtr Converter::ExprConverter::VisitCharacterLiteral(const clang::CharacterLiteral* charLit) {
		...
		case clang::CharacterLiteral::Ascii: elemType = gen.getChar(); break;
	...
	core::ExpressionPtr Converter::ExprConverter::VisitStringLiteral(const clang::StringLiteral* stringLit) {
		...
		case clang::StringLiteral::UTF8: elemType = gen.getChar(); break;
		...
		auto vecType = builder.refType(builder.vectorType(elemType, core::ConcreteIntTypeParam::get(builder.getNodeManager(), vectorLenght)));
	...
	core::ExpressionPtr Converter::ExprConverter::VisitBinaryOperator(const clang::BinaryOperator* binOp) {
		...
		if(core::analysis::isVolatileType(rhs->getType())) { rhs = builder.callExpr(builder.getLangBasic().getVolatileRead(), rhs); }
		...
		auto doPointerArithmetic = [&]() -> core::ExpressionPtr {
			...
			if(gen.isUnsignedInt(rhs->getType())) { rhs = builder.castExpr(gen.getInt8(), rhs); }
			if(gen.isInt16(rhs->getType())) { rhs = builder.castExpr(gen.getInt4(), rhs); }
	...
	core::ExpressionPtr Converter::ExprConverter::VisitUnaryOperator(const clang::UnaryOperator* unOp) {
		...
		case clang::UO_Not:
			...
			return (retIr = builder.callExpr(retIr->getType(), gen.getOperator(retIr->getType(), core::lang::BasicGenerator::Not), retIr));
		// !a
		case clang::UO_LNot:
			if(!gen.isBool(subExpr->getType())) { subExpr = core::types::smartCast(subExpr, gen.getBool()); }
	...
	core::ExpressionPtr Converter::ExprConverter::VisitConditionalOperator(const clang::ConditionalOperator* condOp) {
		...
		return (retIr = builder.callExpr(retTy, gen.getIfThenElse(),
		                                 condExpr,                                                                                  // Condition
	...
	core::ExpressionPtr Converter::ExprConverter::VisitArraySubscriptExpr(const clang::ArraySubscriptExpr* arraySubExpr) {
		...
		if(!gen.isUInt4(idx->getType())) { idx = core::types::castScalar(gen.getUInt4(), idx); }
