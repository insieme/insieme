...
core::ExpressionPtr Converter::ExprConverter::VisitCallExpr(const clang::CallExpr* callExpr) {
	...
	if(typeList.size() == 0 && args.size() > 0) {
		typeList = ::transform(args, [](const core::ExpressionPtr& exprPtr) {
			return exprPtr->getType();
		});
...
//			rhs = core::types::smartCast(rhs, exprTy);
			...
			switch(binOp->getOpcode()) {
			case clang::BO_Add:
				simdOp = ext.getSIMDAdd();
				break;
			case clang::BO_Mul:
				simdOp = ext.getSIMDMul();
				break;
			case clang::BO_Div:
				simdOp = ext.getSIMDDiv();
				break;
			case clang::BO_Rem:
				simdOp = ext.getSIMDMod();
				break;
			case clang::BO_And:
				simdOp = ext.getSIMDAnd();
				break;
			case clang::BO_Or:
				simdOp = ext.getSIMDOr();
				break;
			case clang::BO_Xor:
				simdOp = ext.getSIMDXor();
				break;
			case clang::BO_Shl:
				simdOp = ext.getSIMDLShift();
				break;
			case clang::BO_Shr:
				simdOp = ext.getSIMDRShift();
				break;
			case clang::BO_EQ:
				simdOp = ext.getSIMDEq();
				break;
			case clang::BO_NE:
				simdOp = ext.getSIMDNe();
				break;
			case clang::BO_LT:
				simdOp = ext.getSIMDLt();
				break;
			case clang::BO_LE:
				simdOp = ext.getSIMDLe();
				break;
			case clang::BO_GT:
				simdOp = ext.getSIMDGt();
				break;
			case clang::BO_GE:
				simdOp = ext.getSIMDGe();
				break;
...
core::ExpressionPtr Converter::ExprConverter::VisitUnaryOperator(const clang::UnaryOperator *unOp) {
	...
	[&](core::ExpressionPtr subExpr, core::lang::BasicGenerator::Operator op) -> core::ExpressionPtr {
		...
		switch(op) {
		case core::lang::BasicGenerator::PreInc:
			return builder.preInc(subExpr);
		case core::lang::BasicGenerator::PostInc:
			return builder.postInc(subExpr);
		case core::lang::BasicGenerator::PreDec:
			return builder.preDec(subExpr);
		case core::lang::BasicGenerator::PostDec:
			return builder.postDec(subExpr);
		default :
...
core::ExpressionPtr Converter::ExprConverter::VisitConditionalOperator(const clang::ConditionalOperator* condOp) {
	...
	auto fixingThrowExprType = [&](core::ExpressionPtr toFix, const core::TypePtr& retTy) {
		...
		core::NodeAddress addrTy4 = throwExprAddr.getDefinition().getBindingOf(
		                                throwExprAddr.getVariable()).getVariable().getType().as<core::FunctionTypeAddress>().getReturnType();
...
core::ExpressionPtr Converter::ExprConverter::VisitAtomicExpr(const clang::AtomicExpr* atom) {
	...
	case clang::AtomicExpr::AtomicOp::AO__c11_atomic_init:
		assert_not_implemented();
	case clang::AtomicExpr::AtomicOp::AO__c11_atomic_load:
		assert_not_implemented();
	case clang::AtomicExpr::AtomicOp::AO__c11_atomic_store:
		assert_not_implemented();
	case clang::AtomicExpr::AtomicOp::AO__c11_atomic_exchange:
		assert_not_implemented();
	case clang::AtomicExpr::AtomicOp::AO__c11_atomic_compare_exchange_strong:
		assert_not_implemented();
	case clang::AtomicExpr::AtomicOp::AO__c11_atomic_compare_exchange_weak:
		assert_not_implemented();
	...
	case clang::AtomicExpr::AtomicOp::AO__atomic_load:
		assert_not_implemented();
	case clang::AtomicExpr::AtomicOp::AO__atomic_load_n:
		assert_not_implemented();
	case clang::AtomicExpr::AtomicOp::AO__atomic_store:
		assert_not_implemented();
	case clang::AtomicExpr::AtomicOp::AO__atomic_store_n:
		assert_not_implemented();
	case clang::AtomicExpr::AtomicOp::AO__atomic_exchange:
		assert_not_implemented();
	case clang::AtomicExpr::AtomicOp::AO__atomic_exchange_n:
		assert_not_implemented();
		
	case clang::AtomicExpr::AtomicOp::AO__atomic_compare_exchange:
		assert_not_implemented();
	case clang::AtomicExpr::AtomicOp::AO__atomic_compare_exchange_n:
		assert_not_implemented();
	...
	case clang::AtomicExpr::AtomicOp::AO__atomic_fetch_nand:
		assert_not_implemented();
	...
	case clang::AtomicExpr::AtomicOp::AO__atomic_nand_fetch:
		assert_not_implemented();
	}
