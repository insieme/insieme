...
namespace utils {
	...
	core::ExpressionPtr convertMaterializingExpr(conversion::Converter& converter, const core::ExpressionPtr& retIr) {
		...
		if(const auto& subCall = retIr.isa<core::CallExprPtr>()) {
			...
			if(refExt.isCallOfRefDeref(retIr)) {
				return subCall->getArgument(0);
			}
			// otherwise, materialize call if not built in
			if(!core::lang::isBuiltIn(subCall->getFunctionExpr())) {
				return builder.callExpr(builder.refType(retIr->getType()), subCall->getFunctionExpr(), subCall->getArgumentDeclarations());
			}
		}

		// otherwise we create a new temporary here
