...
HeaderTagger::HeaderTagger(const vector<fs::path>& stdLibDirs, const vector<fs::path>& interceptedHeaderDirs, const vector<fs::path>& userIncludeDirs,
	...
	stdLibDirs(::transform(stdLibDirs, [](const fs::path& cur) {
	return fs::canonical(cur);
})),
interceptedHeaderDirs(::transform(interceptedHeaderDirs, [](const fs::path& cur) {
	return fs::canonical(cur);
})),
userIncludeDirs(::transform(userIncludeDirs, [](const fs::path& cur) {
	return fs::canonical(cur);
})),
sm(srcMgr) {
	VLOG(2) << "stdLibDirs: \n\t" << this->stdLibDirs;
...
boost::optional<fs::path> HeaderTagger::toInterceptedLibHeader(const fs::path& path) const {
	...
	if(interceptedHeaderDirs.empty()) {
		return fail;
	}
...
boost::optional<std::string> isOCLHeader(const fs::path& path) {
	...
	if(name == "cl_platform.h") {
		return std::string("CL/cl_platform.h");
	}
	if(name == "cl.h") {
		return std::string("CL/cl.h");
	}
...
void HeaderTagger::addHeaderForDecl(const core::NodePtr& node, const clang::Decl* decl, bool attachUserDefined) const {
	...
	if(const clang::FunctionDecl* funDecl = llvm::dyn_cast<clang::FunctionDecl>(decl)) {
		...
		if(funDecl->isExternC() &&
		        !(isStdLibHeader(fileName) || isIntrinsicHeader(fileName))) {
			return;
		}
	}
