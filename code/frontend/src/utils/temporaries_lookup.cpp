...
namespace utils {
	...
	namespace {
		...
		class temporariesVisitor : public clang::ConstStmtVisitor<temporariesVisitor, bool> {
		  ...
		  public:
			temporariesVisitor(std::vector<const clang::CXXTemporary*>& list) : tempList(list) {}
			...
			bool Visit(const clang::Stmt* S) {
				...
				if(llvm::isa<clang::CXXBindTemporaryExpr>(S)) { tempList.push_back(llvm::cast<clang::CXXBindTemporaryExpr>(S)->getTemporary()); }
