...
// enum(A: s)
auto param_enum			= Tok<clang::tok::kw_enum>() >> l_paren >> var["enum_list"] >> colon >> tok::expr["enum_size"] >> !
                          (tok::semi >> param_quality_range) >> r_paren;
...
omp::ParamPtr handleParamClause(const MatchObject& mmap) {
	...
	return std::make_shared<omp::Param>(varExpr, std::make_shared<std::vector<core::ExpressionPtr>>(range),
	                                    std::make_shared<std::vector<core::ExpressionPtr>>(quality_range), core::ExpressionPtr(), core::ExpressionPtr());
}
...
omp::ApproximatePtr handleApproximateClause(const MatchObject& mmap) {
	if(!mmap.stringValueExists("approximate")) {
		return omp::ApproximatePtr();
	}
...
insieme::frontend::tu::IRTranslationUnit OmpFrontendExtension::IRVisit(insieme::frontend::tu::IRTranslationUnit& tu) {
	...
	for(auto& pair : tu.getGlobals()) {
		if(std::find(thread_privates.begin(), thread_privates.end(), pair.first) != thread_privates.end()) {
			...
			lit->addAnnotation(std::make_shared<omp::BaseAnnotation>(
			                       omp::BaseAnnotation::AnnotationList({std::make_shared<omp::ThreadPrivate>()}))
			                  );
	...
	for(auto& pair : tu.getFunctions()) {
		...
		auto fixer = [&](const core::NodePtr& node)-> core::NodePtr{
			...
			if(std::find(thread_privates.begin(), thread_privates.end(), node) != thread_privates.end()) {
				...
				node->addAnnotation(std::make_shared<omp::BaseAnnotation>(
				omp::BaseAnnotation::AnnotationList({std::make_shared<omp::ThreadPrivate>()}))
				                   );
			}
			return node;
