...
NodeBookmark WhileToForExtension::extractStepFromAssignment(core::Address<const core::Node> a) {
	...
	if(step.ok()) {
		...
		if(*stmt!=*op1) {
			step.err("step size expression is not in the format x = x +- n");
		}
		
		// our second argument should be a literal
		if(op2ptr.getNodeType()!=core::NT_Literal) {
			step.err("second argument to self-assignment should be a literal");
		}
	}
...
NodeBookmark WhileToForExtension::extractInitialValForVar(core::NodeAddress loop, core::VariablePtr var) {
	...
	core::visitDepthFirst(loop.getRootAddress(), [&](const core::VariableAddress &x) {
		...
		if(x.getAddressedNode()==var && !core::isChildOf(loop, x) &&
		        x.getParentAddress().getChildAddresses().size()>=2) {
			allvars.push_back(x);
		}
	});
	...
	if(allvars.size()!=1) {
		initial.err("multiple variable occurrences outside the loop");
	}
