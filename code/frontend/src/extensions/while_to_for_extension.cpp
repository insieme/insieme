...
namespace extensions {
	...
	void WhileToForExtension::printNodes(const insieme::core::Address<const insieme::core::Node> n, std::string prefix = "        @\t", int max = -1,
	                                     unsigned int depth = 0) {
		if(max < 0) { max = maxDepth(n); }
		int depth0 = n.getDepth() - depth;
		std::cout << prefix << n << std::setw(2 * (max - depth + 1)) << "+" << std::setw(1 + 2 * (depth0 + depth - 1)) << "" << *n << std::endl;
		for(auto c : n.getChildAddresses())
	...
	NodeBookmark WhileToForExtension::extractStepFromAssignment(core::Address<const core::Node> a) {
		...
		auto assignpat =
		    irp::assignment(pattern::var("lhs"), pattern::node(pattern::any                                 // any type will do (could be integer)
		...
		if(m && m.get().isVarBound("lhs") && m.get().isVarBound("addsub") && m.get().isVarBound("ops")) {
			std::vector<core::Address<const core::Node>> addsubvec = m.get()["addsub"].getFlattened(), opsvec = m.get()["ops"].getFlattened();
	...
	NodeBookmark WhileToForExtension::extractInitialValForVar(core::NodeAddress loop, core::VariablePtr var) {
		...
		core::visitDepthFirst(loop.getRootAddress(), [&](const core::VariableAddress& x) {
			...
			if(x.getAddressedNode() == var && !core::isChildOf(loop, x) && x.getParentAddress().getChildAddresses().size() >= 2) { allvars.push_back(x); }
		});
		...
		for(core::VariableAddress x : allvars) {
			...
			if(x.getParentNode()->getNodeType() == core::NT_DeclarationStmt && literal.getAddressedNode()->getNodeType() == core::NT_Literal) {
		...
		core::visitDepthFirst(cond, [&](const core::VariableAddress& x) {
			...
			if(x.getAddressedNode() == var && x.getDepth() > 2 && x.getParentNode()->getNodeType() == core::NT_CallExpr
			   ...
			   && mgr.getLangExtension<core::lang::ReferenceExtension>().isRefDeref((refderef = x.getParentNode().as<core::CallExprPtr>()).getFunctionExpr())) {
				...
				if(comperator.getChildAddresses().size() != 4 || (arg = comperator.getAddressOfChild(3)).getNodeType() != core::NT_Literal) {
				...
				if(target.ok()) {
					...
					/**/ if(basic.isSignedIntLe(comperator.getFunctionExpr())) {
						target.value += 1;
					} else if(basic.isSignedIntGe(comperator.getFunctionExpr())) {
						target.value -= 1;
					} else if(basic.isSignedIntLt(comperator.getFunctionExpr()) || basic.isSignedIntGt(comperator.getFunctionExpr())
					          || basic.isSignedIntNe(comperator.getFunctionExpr()))
						;
					else {
						target.err("unsupported comparison operator"); // unhandled for now
					}
					...
					if(cond != comperator) {
						target.err("more than one condition in while loop; currently unsupported");
					} else {
						target.nodes.push_back(comperator);
					}
				}
			}
		});
	...
	core::ProgramPtr WhileToForExtension::replaceWhileByFor(core::NodeAddress whileaddr, NodeBookmark initial, NodeBookmark target, NodeBookmark step) {
		...
		for(core::NodeAddress n : step.nodes) {
			stepstmt.push_back(n.isa<core::StatementAddress>());
		}
		...
		core::LiteralPtr start = builder.intLit(initial.value), end = builder.intLit(target.value), incr = builder.intLit(step.value);
	...
	insieme::core::ProgramPtr WhileToForExtension::IRVisit(insieme::core::ProgramPtr& prog) {
		auto whilepat = irp::whileStmt(pattern::var("condition", pattern::all(pattern::var("cvar", irp::variable()))), pattern::var("body"));

		// match all while statements while making sure that we have access to their superior nodes -> ...Pairs
		irp::matchAllPairsReverse(whilepat, core::NodeAddress(prog), [&](core::NodeAddress whileaddr, pattern::AddressMatch match) {
			//			std::cout << "Program now:" << std::endl << pp(prog);
			whileaddr = whileaddr.switchRoot(prog);
			auto condition = whileaddr.getAddressOfChild(0), body = whileaddr.getAddressOfChild(1);
			...
			for(core::VariablePtr var : cvarSet) {
				NodeBookmark initial = extractInitialValForVar(whileaddr, var), target = extractTargetValForVar(condition, var),
		...
		#if 0
				if(initial.ok()) {
					std::cout << "initial value is " << initial.value << std::endl;
				}
				else {
					std::cout << "no initial value found: " << initial.msg << std::endl;
				}
				if(target.ok()) {
					std::cout << "target value is " << target.value << std::endl;
				}
				else {
					std::cout << "target size cannot be determined: " << target.msg << std::endl;
				}
				if(step.ok()) {
					std::cout << "step size is " << step.value << std::endl;
				}
				else {
					std::cout << "step size cannot be determined: " << step.msg << std::endl;
				}
				std::cout << std::endl;
		#endif
				}
