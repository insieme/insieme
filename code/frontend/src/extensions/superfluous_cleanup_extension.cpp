...
namespace cleanup {
	...
	namespace {
		...
		struct UnneccesaryAssignmentVisitor : IRVisitor<bool, Address> {
			...
			bool visitVariable(const VariableAddress& n) {
				...
				if(parent.isa<DeclarationStmtPtr>()) {
					...
					if(parentAddr.getDepth() > 1 && parentAddr.getParentNode().isa<ForStmtPtr>()) { return false; }
					// we also need to make sure they aren't aliases to something
					if(!isSaveVarInitialization(parent.as<DeclarationStmtPtr>())) { return false; }
					declaredVars.insert(std::make_pair(nVar, parentAddr));
					return false;
				}
				if(declaredVars.find(nVar) == declaredVars.end()) { return false; }
				// check if used as non-deref (and not in an assignment)
				if(!core::analysis::isCallOf(parent, basic.getRefAssign()) && !core::analysis::isCallOf(parent, basic.getRefDeref())) {
		...
		LambdaExprPtr removeUnneccesaryAssignmentsIteration(LambdaExprPtr lambda) {
			...
			for(auto& pair : uv.declaredVars) {
				...
				if(core::analysis::isCallOf(pair.second.getAddressedNode(), basic.getRefAssign())) {
					...
					if(!isUsedPreviouslyInEnclosingLoop(pair.second, pair.first)) {
						...
						if(analysis::isSideEffectFree(ass.getArgument(1))) {
							...
							if(!basic.isInt(pair.first->getType())
							   && !(analysis::isRefType(pair.first->getType()) && basic.isInt(getReferencedType(pair.first->getType()))))
				...
				if(pair.second.isa<DeclarationStmtAddress>()) {
					...
					if(declAddr.getVariable().getAddressedNode() == pair.first) { toRemove.push_back(declAddr); }
				}
			}
			if(!toRemove.empty()) { lambda = transform::remove(nodeMan, toRemove).as<LambdaExprPtr>(); }
		...
		struct SuperfluousControlFlowVisitor : IRVisitor<bool, Address> {
			...
			bool visitForStmt(const ForStmtAddress& adr) {
				...
				if(n->getBody()->getStatements().empty()) {
					if(isSideEffectFree(n->getStart()) && isSideEffectFree(n->getStep()) && isSideEffectFree(n->getEnd())) { toDelete.push_back(adr); }
					return true;
				}
				return false;
			}
			...
			bool visitWhileStmt(const WhileStmtAddress& adr) {
				...
				if(n->getBody()->getStatements().empty()) {
					if(isSideEffectFree(n->getCondition())) { toDelete.push_back(adr); }
					return true;
				}
				return false;
			}
		...
		LambdaExprPtr removeSuperfluousControlFlowIteration(LambdaExprPtr lambda) {
			...
			if(!sfcfv.toDelete.empty()) { lambda = transform::remove(lambda->getNodeManager(), sfcfv.toDelete).as<LambdaExprPtr>(); }
