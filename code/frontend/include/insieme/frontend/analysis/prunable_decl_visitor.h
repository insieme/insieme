...
class PrunableDeclVisitor {
	...
	void dispatchDecl(const clang::Decl* decl) {
		...
		case clang::Decl::Record: {
			if(llvm::cast<clang::TagDecl>(decl)->isDependentType() && !visitTemplates) {
				break;
			}
		...
		case clang::Decl::CXXConversion: {
			if(llvm::cast<clang::DeclContext>(decl)->isDependentContext()) {
				break;
			}
		}
		...
		case clang::Decl::TypeAlias: {
			if(llvm::isa<clang::TemplateTypeParmType>(llvm::cast<clang::TypedefNameDecl>(decl)->getUnderlyingType().getTypePtr())) {
				break;
			}
		...
		case clang::Decl::ClassTemplate: {
			...
			if(visitTemplates) {
				static_cast<BASE*>(this)->VisitClassTemplate(classTmplDecl);
			}
		...
		case clang::Decl::FunctionTemplate: {
			...
			if(visitTemplates) {
				static_cast<BASE*>(this)->VisitFunctionTemplate(funcTmplDecl);
			}
			// FIXME: it seems that clang 3.2 does not like const iterators, change whenever clang upgrade
