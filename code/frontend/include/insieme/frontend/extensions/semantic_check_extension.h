...
public:
	...
	virtual insieme::frontend::tu::IRTranslationUnit IRVisit(insieme::frontend::tu::IRTranslationUnit& tu) {
		...
		for(auto cur : tu.getFunctions()) {
			...
			std::cout << "function " << ++i << " of " << tu.getFunctions().size()
		...
		/*		    n++;
					...
					if (!msg.empty()){
		                for(auto s : msg.getErrors()) {
		                    //file3 << s.getMessage() << "\n" << s.getAddress() << "\n";
		                    for (std::unordered_map<insieme::core::checks::Message, std::pair<const clang::Expr*, insieme::core::ExpressionPtr>>::iterator it=exprErrors.begin(); it!=exprErrors.end(); ++it) {
		                    ...
		                    for (std::unordered_map<insieme::core::checks::Message, std::pair<const clang::Type*, insieme::core::TypePtr>>::iterator it=typeErrors.begin(); it!=typeErrors.end(); ++it) {
		                        if(it->first.getMessage().compare(s.getMessage())==0) {
		                            std::cout << "#########################" << std::endl;
