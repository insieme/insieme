...
namespace extensions {
	...
	class Cpp11Extension : public insieme::frontend::extensions::FrontendExtension {
		...
		virtual stmtutils::StmtWrapper Visit(const clang::Stmt* stmt, insieme::frontend::conversion::Converter& convFact) {
			if(const clang::CXXForRangeStmt* fr = llvm::dyn_cast<clang::CXXForRangeStmt>(stmt)) { return VisitCXXForRangeStmt(fr, convFact); }
		...
		virtual insieme::core::ExpressionPtr Visit(const clang::Expr* expr, insieme::frontend::conversion::Converter& convFact) {
			if(const clang::CXXMemberCallExpr* memberExpr = llvm::dyn_cast<clang::CXXMemberCallExpr>(expr)) {
				if(const clang::CXXConversionDecl* cd = llvm::dyn_cast<clang::CXXConversionDecl>(memberExpr->getMethodDecl())) {
					...
					if(cd->getParent()->isLambda()) {
						...
						if(fit != simpleLambdaReplacementMap.end()) { return convFact.getIRBuilder().refVar(fit->second); }
					}
				}
			}
			if(const clang::LambdaExpr* lambda = llvm::dyn_cast<clang::LambdaExpr>(expr)) { return VisitLambdaExpr(lambda, convFact); }
			...
			if(const clang::SizeOfPackExpr* sope = llvm::dyn_cast<clang::SizeOfPackExpr>(expr)) { return VisitSizeOfPackExpr(sope, convFact); }
		...
		virtual insieme::core::TypePtr Visit(const clang::QualType& type, insieme::frontend::conversion::Converter& convFact) {
			if(const clang::AutoType* autoTy = llvm::dyn_cast<clang::AutoType>(type.getTypePtr())) { return VisitAutoType(autoTy, convFact); }
			if(const clang::DecltypeType* declTy = llvm::dyn_cast<clang::DecltypeType>(type.getTypePtr())) { return VisitDecltypeType(declTy, convFact); }
