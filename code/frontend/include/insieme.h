...
public:
	static ParserProxy& get(clang::Parser* parser=NULL) {
		static ParserProxy* proxy = NULL;
		if(!proxy) {
			assert(parser && "ParserProxy cannot be initialized with a NULL parser");
			proxy = new ParserProxy(parser);
		}
		return *proxy;
	}
...
public:
	...
	OwningStmtResult ActOnCompoundStmt(clang::SourceLocation 	L,
									clang::SourceLocation 	R,
									MultiStmtArg		Elts,
									bool 			isStmtExpr);
								   
	OwningStmtResult ActOnIfStmt(clang::SourceLocation 	IfLoc,
								FullExprArg 		CondVal,
								DeclPtrTy 			CondVar,
								StmtArg 			ThenVal,
	...
	OwningStmtResult ActOnForStmt(clang::SourceLocation 	ForLoc,
	...
	template <class T>
	void ActOnPragma(std::string const&	name,
					 MatchMap const& 	mmap, 
...
template <class T>
void InsiemeSema::ActOnPragma(std::string const&	name,
							  MatchMap const& 		mmap, 
							  clang::SourceLocation 	startLoc,
							  clang::SourceLocation 	endLoc)
{
	addPragma( new T(startLoc, endLoc, name, mmap) );
}

#include "pragma_handler.h"
namespace insieme{

class InlinePragma: public Pragma{
    unsigned mLevel;
    
public:
    InlinePragma(clang::SourceLocation const& startLoc,
...
//		DEBUG_RUN(
//			llvm::outs() << "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~\n" <<
//							"|-> Pragma: " << getType() << " : " << mLevel << " " << toString() << "\n"
//		);
	}
