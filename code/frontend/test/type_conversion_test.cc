...
#define CREATE_TYPE(ClangType) convFactory.convertType(ClangType)

#define CREATE_PTR(ClangType) convFactory.convertType(ASTctx.getPointerType(ClangType))

#define CREATE_REF(ClangType) convFactory.convertType(ASTctx.getLValueReferenceType(ClangType))
...
TEST(TypeConversion, CombinedTypes) {
	...
	clang::RecordDecl* classDecl =
	    clang::CXXRecordDecl::Create(ASTctx, clang::TagTypeKind::TTK_Class, ASTctx.getTranslationUnitDecl(), loc, loc, &ASTctx.Idents.get("BaseClass"));
	clang::BuiltinType fieldType(clang::BuiltinType::UShort);
	clang::FieldDecl* fieldDecl = clang::FieldDecl::Create(ASTctx, classDecl, loc, loc, &ASTctx.Idents.get("fieldA"), fieldType.getCanonicalTypeInternal(), 0,
	                                                       0, false, clang::InClassInitStyle::ICIS_NoInit);
	...
	CHECK_TYPE(ASTctx.getRecordType(classDecl), "BaseClass", "struct BaseClass <fieldA:uint<2>>");
	CHECK_POINTER(ASTctx.getRecordType(classDecl), "ref<array<BaseClass,1>>", "ref<array<struct BaseClass <fieldA:uint<2>>,1>>");
	CHECK_REFERENCE(ASTctx.getRecordType(classDecl), "struct<_cpp_ref:ref<BaseClass>>", "struct<_cpp_ref:ref<struct BaseClass <fieldA:uint<2>>>>");

	CHECK_TYPE(ASTctx.getRecordType(classDecl).withConst(), "BaseClass", "struct BaseClass <fieldA:uint<2>>");
	CHECK_POINTER(ASTctx.getRecordType(classDecl).withConst(), "src<array<BaseClass,1>>", "src<array<struct BaseClass <fieldA:uint<2>>,1>>");
	...
	// function with objects
	{
		...
		CHECK_TYPE(funcTy, "((BaseClass,struct<_cpp_ref:ref<BaseClass>>,ref<array<BaseClass,1>>,struct<_cpp_ref:ref<ref<array<BaseClass,1>>>>)->BaseClass)",
		           "((struct BaseClass <fieldA:uint<2>>,struct<_cpp_ref:ref<struct BaseClass <fieldA:uint<2>>>>,ref<array<struct BaseClass "
		           "<fieldA:uint<2>>,1>>,struct<_cpp_ref:ref<ref<array<struct BaseClass <fieldA:uint<2>>,1>>>>)->struct BaseClass <fieldA:uint<2>>)");
		CHECK_POINTER(funcTy, "((BaseClass,struct<_cpp_ref:ref<BaseClass>>,ref<array<BaseClass,1>>,struct<_cpp_ref:ref<ref<array<BaseClass,1>>>>)->BaseClass)",
		              "((struct BaseClass <fieldA:uint<2>>,struct<_cpp_ref:ref<struct BaseClass <fieldA:uint<2>>>>,ref<array<struct BaseClass "
		              "<fieldA:uint<2>>,1>>,struct<_cpp_ref:ref<ref<array<struct BaseClass <fieldA:uint<2>>,1>>>>)->struct BaseClass <fieldA:uint<2>>)");
		CHECK_REFERENCE(funcTy, "struct<_cpp_ref:ref<((BaseClass,struct<_cpp_ref:ref<BaseClass>>,ref<array<BaseClass,1>>,struct<_cpp_ref:ref<ref<array<"
		                        "BaseClass,1>>>>)->BaseClass)>>",
		                "struct<_cpp_ref:ref<((struct BaseClass <fieldA:uint<2>>,struct<_cpp_ref:ref<struct BaseClass <fieldA:uint<2>>>>,ref<array<struct "
		                "BaseClass <fieldA:uint<2>>,1>>,struct<_cpp_ref:ref<ref<array<struct BaseClass <fieldA:uint<2>>,1>>>>)->struct BaseClass "
		                "<fieldA:uint<2>>)>>");
	}
}
...
TEST(TypeConversion, HandleRecursiveStructType) {
	...
	clang::RecordDecl* classDecl =
	    clang::CXXRecordDecl::Create(ASTctx, clang::TagTypeKind::TTK_Class, ASTctx.getTranslationUnitDecl(), loc, loc, &ASTctx.Idents.get("RecClass"));

	clang::FieldDecl* fieldOne = clang::FieldDecl::Create(ASTctx, classDecl, loc, loc, &ASTctx.Idents.get("one"), charTy.getCanonicalTypeInternal(), 0, 0,
	                                                      false, clang::InClassInitStyle::ICIS_NoInit);
	fieldOne->setAccess(clang::AccessSpecifier::AS_public);

	auto classTy = ASTctx.getRecordType(classDecl);
	clang::FieldDecl* fieldPtr = clang::FieldDecl::Create(ASTctx, classDecl, loc, loc, &ASTctx.Idents.get("rec_ptr"), ASTctx.getPointerType(classTy), 0, 0,
	                                                      false, clang::InClassInitStyle::ICIS_NoInit);
	fieldPtr->setAccess(clang::AccessSpecifier::AS_public);

	clang::FieldDecl* fieldRef = clang::FieldDecl::Create(ASTctx, classDecl, loc, loc, &ASTctx.Idents.get("ref"), ASTctx.getLValueReferenceType(classTy), 0, 0,
	                                                      false, clang::InClassInitStyle::ICIS_NoInit);
	fieldRef->setAccess(clang::AccessSpecifier::AS_public);

	clang::FieldDecl* fieldConstRef =
	    clang::FieldDecl::Create(ASTctx, classDecl, loc, loc, &ASTctx.Idents.get("const_ref"), ASTctx.getLValueReferenceType(classTy.withConst()), 0, 0, false,
	...
	CHECK_TYPE(classTy, "RecClass", "rec 'RecClass.{'RecClass=struct RecClass "
	                                "<one:int<1>,rec_ptr:ref<array<'RecClass,1>>,ref:struct<_cpp_ref:ref<'RecClass>>,const_ref:struct<_const_cpp_ref:src<'"
	                                "RecClass>>>}");
	CHECK_POINTER(classTy, "ref<array<RecClass,1>>", "ref<array<rec 'RecClass.{'RecClass=struct RecClass "
	                                                 "<one:int<1>,rec_ptr:ref<array<'RecClass,1>>,ref:struct<_cpp_ref:ref<'RecClass>>,const_ref:struct<_const_"
	                                                 "cpp_ref:src<'RecClass>>>},1>>");
	CHECK_REFERENCE(classTy, "struct<_cpp_ref:ref<RecClass>>", "struct<_cpp_ref:ref<rec 'RecClass.{'RecClass=struct RecClass "
	                                                           "<one:int<1>,rec_ptr:ref<array<'RecClass,1>>,ref:struct<_cpp_ref:ref<'RecClass>>,const_ref:"
	                                                           "struct<_const_cpp_ref:src<'RecClass>>>}>>");
}
...
TEST(TypeConversion, FileTest) {
	...
	for(auto it = begin; it != end; ++it) {
		...
		if(pragma->isStatement()) {
			...
			EXPECT_EQ(ia::ExpectedIRAnnotation::getValue(stmt),
			          '\"' + toString(printer::PrettyPrinter(stmt, printer::PrettyPrinter::PRINT_SINGLE_LINE)) + '\"');
		} else {
...
TEST(TypeConversion, FunctionPtr) {
	...
	// check for some enum features
	EXPECT_EQ("PROGRAM { \n// Entry Points: \n\trec v0.{v0=fun() {ref<(()->int<4>)> v1 = rec v0.{v0=fun('a v1) {ref<'a> v2 = ref_alloc(rec v0.{v0=fun('a v1) "
	          "{return type<'a>;}}(v1), memloc_stack); ref_assign(v2, v1); return v2;}}(rec v0.{v0=fun() {return 1;}});}}\n",
	          toString(*program));
}
