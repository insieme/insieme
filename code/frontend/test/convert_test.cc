...
//		EXPECT_PRED2(containsSubString, res, "let fun004 = recFun v38 {");		// odd header
		...
		EXPECT_PRED2(containsSubString, res, "frontend_test__inputs_conversion_test_cpp113221, 3.0);");
...
//		std::cout << irtu << "\n";
		...
		EXPECT_EQ("recFun v0 {v1 = fun(uint<4> v2) -> int<4> {return (v2==0u)?0:v0( var(v2)-1u);};v0 = fun(uint<4> v5) -> int<4> {return (v5==0u)?1:v1( var(v5)-1u);};}", print(even));
