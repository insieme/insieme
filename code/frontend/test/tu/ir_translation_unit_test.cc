...
namespace tu {
	...
	TEST(TranslationUnit, MetaClassInfoMerge) {
		...
		infoA.addConstructor(builderA.parseExpr("let A = struct { int<4> a; }; "
		                                        "lambda ctor A::() {}")
		...
		infoB.addConstructor(builderB.parseExpr("let A = struct { int<4> a; }; "
		                                        "lambda ctor A::() {}")
		                         .as<core::LambdaExprPtr>());
		infoB.addConstructor(builderB.parseExpr("let A = struct { int<4> a; }; "
	...
	TEST(TranslationUnit, TypeMerge) {
		...
		core::IRBuilder builderB(mgrB);
		{
			...
			auto member = builderB.parseExpr("let class = struct A { int<4> x; };"
			                                 "lambda class :: ()->unit { return; }")
			                  .as<core::LambdaExprPtr>();
