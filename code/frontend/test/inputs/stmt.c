...
#pragma test expected "decl ref<int<4>> v0 = ( var(0))"
	...
	#pragma test expected "decl ref<uint<4>> v0 = ( var(undefined(type<uint<4>>)))"
	unsigned int c;

	#pragma test expected "decl ref<real<4>> v0 = ( var(undefined(type<real<4>>)))"
	float d;

	#pragma test expected "decl ref<real<4>> v0 = ( var(1.0E+0))"
	float e = 1.0f;
	// ...
}
...
void binary_op_test() {
	...
	#pragma test expected "(( *v100)+( *v101))"
	a + b;

	#pragma test expected "(v100 := (( *v100)+( *v101)))"
	a += b;

	#pragma test expected "(( *v100)-( *v101))"
	a - b;

	#pragma test expected "(( *v100)-1u)"
	c - 1;

	#pragma test expected "(v100 := (( *v100)-( *v101)))"
	a -= b;

	#pragma test expected "fun(int<4> v1, int<4> v2) -> int<4> { (v1+1); return (v2-1);}(( *v100), ( *v101))"
	(a + 1, b - 1);
}
...
#pragma test expected "decl ref<struct Person <weight:int<4>,age:int<4>>> v0 = ( var(undefined(type<struct Person <weight:int<4>,age:int<4>>>)))"
	struct Person p;
...
void if_stmt_test() {
	...
	#pragma test expected "if(int_to_bool(( *v100))) { (v100 := (( *v100)+1));} else { (v100 := (( *v100)-1));}"
	if(cond) {
		cond += 1;
	} else {
		cond -= 1;
	...
	#pragma test expected "if((( *v100)==0)) { (v100 := (( *v100)+( *v100)));}"
	if(cond == 0) { cond += cond; }
	...
	#pragma test expected "if(((( *v100)>0) && bind(){fun(int<4> v1) -> bool { return (v1!=1);}(( *v100))})) { }"
	if(cond > 0 && cond != 1) { ; }
}
...
void for_stmt_test() {
	...
	// standard for loop
	#pragma test expected "for(decl int<4> v0 = 0 .. 100 : 1) { { };}"
	for(int i = 0; i < 100; i++) {
		;
	}

	// for loop using a variable declared outside
	#pragma test expected "{ for(decl int<4> v0 = 0 .. 100 : 1) { (v100 := v0); }; { (v101 := 100); };}"
	for(it = 0; it < 100; ++it) {
		a = it;
	}
	...
	#pragma test expected "{ for(decl int<4> v0 = ( *v100) .. 100 : 1) { { }; }; { (v100 := 100); };}"
	for(; it < 100; it += 1) {
		;
	}

	#pragma test expected "{ decl ref<int<4>> v0 = ( var(1)); decl ref<int<4>> v1 = ( var(2)); for(decl int<4> v2 = 0 .. 100 : 1) { (v100 := v2); };}"
...
void switch_stmt_test() {
	...
	#pragma test expected "{ decl int<4> v0 = CAST<int<4>>(( *v100)); switch(v0) { case 1: { break; } default: { } };}"
	switch(a) {
	case 1: break;
	}
	...
	#pragma test expected "{ decl int<4> v0 = CAST<int<4>>(( *v100)); switch(v0) { case 0: { break; gen_post_inc(v100); } default: { gen_post_inc(v100); } };}"
	switch(a) {
	case 0: break;
	default: a++;
	}
...
void while_stmt_test() {
	int it = 0;
	#pragma test expected "while((( *v100)!=0)) { (v100 := (( *v100)-1));}"
	while(it != 0) {
		it -= 1;
	}
...
#pragma test expected "recFun v0 { v1 = fun(int<4> v2) -> int<4> { return v0((v2+1)); }; v0 = fun(int<4> v3) -> int<4> { return v1((v3-1)); };}"
int f(int x) {
	return g(x - 1);
}

#pragma test expected "recFun v0 { v0 = fun(int<4> v1) -> int<4> { return v2((v1+1)); }; v2 = fun(int<4> v3) -> int<4> { return v0((v3-1)); };}"
int g(int x) {
	return f(x + 1);
}
// recFun v1 {
...
#pragma test expected "recFun v0 { v1 = fun(int<4> v2) -> int<4> { return v0((v2+1)); }; v0 = fun(int<4> v3) -> int<4> { return v1((v3-1)); };}(10)"
	f(10);
}
...
#pragma test expected "decl ref<vector<int<4>,5>> v0 = ( var(undefined(type<vector<int<4>,5>>)))"
	...
	#pragma test expected "( *(ref_vector_to_ref_array(v100)&[0u]))"
	a[0];

	#pragma test expected "((ref_vector_to_ref_array(v100)&[0u]) := 1)"
	a[0] = 1;

	#pragma test expected "decl ref<vector<vector<int<4>,3>,2>> v0 = ( var(undefined(type<vector<vector<int<4>,3>,2>>)))"
	int b[2][3];

	#pragma test expected "( *(ref_vector_to_ref_array((ref_vector_to_ref_array(v100)&[0u]))&[0u]))"
	b[0][0];

	#pragma test expected "((ref_vector_to_ref_array((ref_vector_to_ref_array(v100)&[1u]))&[1u]) := 0)"
	b[1][1] = 0;

	#pragma test expected "decl ref<vector<int<4>,10>> v0 = ( var(vector_init_partial(([0]), 10)))"
	int vec[10] = {0};

	#pragma test expected "decl ref<ref<array<int<4>,1>>> v0 = ( var(ref_vector_to_ref_array(( var(vector_init_partial(([0,5,10]), 10))))))"
...
void init_expr() {
	...
	#pragma test expected "( *(ref_vector_to_ref_array(( var([1, 2, 3])))&[1u]))"
	((int[3]){1, 2, 3})[1];

	struct Person p;
	#pragma test expected "(v100 := struct{weight:=10, age:=20})"
