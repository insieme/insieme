...
LambdaExprPtr getEntryPoint(const core::ProgramPtr& prog, const std::string& entry) {
	for(auto it = prog->getEntryPoints().begin(); it != prog->getEntryPoints().end(); it++) {
		...
		if(core::annotations::hasAttachedName(fun) && core::annotations::getAttachedName(fun).compare(entry) == 0) { return analysis::normalize(fun); }
	}
...
TEST(OMPx, SimpleRegion) {
	...
	LOG(INFO) << "Converting input program '" << std::string(FRONTEND_TEST_DIR) << "inputs/omp+_region.c"
	          << "' to IR...";
	...
	/*
	    ...
	    */
	EXPECT_EQ("AP(rec v0.{v0=fun() {ref<int<4>> v1 = rec v0.{v0=fun('a v1) {ref<'a> v2 = ref_alloc(rec v0.{v0=fun('a v1) {return type<'a>;}}(v1), "
	          "memloc_stack); ref_assign(v2, v1); return v2;}}(0); ref<int<4>> v2 = rec v0.{v0=fun('a v1) {ref<'a> v2 = ref_alloc(rec v0.{v0=fun('a v1) "
	          "{return type<'a>;}}(v1), memloc_stack); ref_assign(v2, v1); return v2;}}(0); {parallel(job (bind(){rec v0.{v0=fun() {ref<int<4>> v1 = rec "
	          "v0.{v0=fun('a v1) {ref<'a> v2 = ref_alloc(rec v0.{v0=fun('a v1) {return type<'a>;}}(v1), memloc_stack); ref_assign(v2, v1); return "
	          "v2;}}(3);}}()}));}; return 0;}})",
	          toString(progEntry));
}

void dumpObjectiveMetaInfo(insieme::annotations::ompp_objective_info info) {
	LOG(INFO) << "[" << info.region_id << "] { " << info.energy_weight << ", (" << info.energy_min << ", " << info.energy_max << "), " << info.power_weight
	          << ", (" << info.power_min << ", " << info.power_max << "), " << info.time_weight << ", (" << info.time_min << ", " << info.time_max << ") }";
}
...
TEST(OMPx, Param) {
	...
	EXPECT_EQ("AP(rec v0.{v0=fun() {ref<int<4>> v1 = rec v0.{v0=fun('a v1) {ref<'a> v2 = ref_alloc(rec v0.{v0=fun('a v1) {return type<'a>;}}(v1), "
	          "memloc_stack); ref_assign(v2, v1); return v2;}}(0); ref<int<4>> v2 = rec v0.{v0=fun('a v1) {ref<'a> v2 = ref_alloc(rec v0.{v0=fun('a v1) "
	          "{return type<'a>;}}(v1), memloc_stack); ref_assign(v2, v1); return v2;}}(0); ref<vector<int<4>,3>> v3 = rec v0.{v0=fun('a v1) {ref<'a> v2 = "
	          "ref_alloc(rec v0.{v0=fun('a v1) {return type<'a>;}}(v1), memloc_stack); ref_assign(v2, v1); return v2;}}(undefined(type<vector<int<4>,3>>)); "
	          "{parallel(job (bind(){rec v0.{v0=fun(ref<int<4>> v1) {ref_assign(v1, int_add(int_mul(pickInRange(0, int_div(int_sub(10, 0), 2), 0, 0, 0), 2), "
	          "0)); ref<int<4>> v2 = rec v0.{v0=fun('a v1) {ref<'a> v2 = ref_alloc(rec v0.{v0=fun('a v1) {return type<'a>;}}(v1), memloc_stack); "
	          "ref_assign(v2, v1); return v2;}}(ref_deref(v1));}}(v1)}));}; {merge(parallel(job (bind(){rec v0.{v0=fun(ref<int<4>> v1, ref<vector<int<4>,3>> "
	          "v2) {ref_assign(v1, ref_deref(rec v0.{v0=fun(ref<vector<'elem,#l>> v1, uint<8> v2) {return ref_narrow(v1, dp_element(dp_root, v2), "
	          "type<'elem>);}}(v2, pickInRange(1, 3ul, 0, 0, 0)))); {ref<int<4>> v3 = rec v0.{v0=fun('a v1) {ref<'a> v2 = ref_alloc(rec v0.{v0=fun('a v1) "
	          "{return type<'a>;}}(v1), memloc_stack); ref_assign(v2, v1); return v2;}}(ref_deref(v1));}; mergeAll();}}(v1, v3)})));}; return 0;}})",
	          toString(progEntry));
}
...
TEST(OMPx, FirstLocal) {
	...
	/*
	    ...
	    */
	EXPECT_EQ("AP(rec v0.{v0=fun() {ref<int<4>> v1 = rec v0.{v0=fun('a v1) {ref<'a> v2 = ref_alloc(rec v0.{v0=fun('a v1) {return type<'a>;}}(v1), "
	          "memloc_stack); ref_assign(v2, v1); return v2;}}(5); {int<4> v2 = ref_deref(v1); parallel(job (bind(){rec v0.{v0=fun(int<4> v1) {ref<int<4>> v2 "
	          "= rec v0.{v0=fun('a v1) {ref<'a> v2 = ref_alloc(rec v0.{v0=fun('a v1) {return type<'a>;}}(v1), memloc_local); ref_assign(v2, v1); return "
	          "v2;}}(v1); {ref<int<4>> v3 = rec v0.{v0=fun('a v1) {ref<'a> v2 = ref_alloc(rec v0.{v0=fun('a v1) {return type<'a>;}}(v1), memloc_stack); "
	          "ref_assign(v2, v1); return v2;}}(int_add(ref_deref(v2), 3));}; ref_delete(v2);}}(v2)}));}; return 0;}})",
	          toString(progEntry));
}
