...
$options = {:insieme_builtins_dir => nil, :command => nil, :std_include_dir => nil, :force => false, :quiet => false, :detect => false}
...
parser = OptionParser.new do|opts|
	...
	opts.on('-d', '--detect-input', 'Automatically detect standard header include directory by asking preprocessor') do |detect|
		$options[:detect] = detect;
	end

	opts.on('-i', '--input dir', 'Standard header include directory') do |std_inc_dir|
		$options[:std_include_dir] = std_inc_dir;
	end

	opts.on('-o', '--output dir', 'Insieme builtin header directory') do |insieme_builtins_dir|
...
raise OptionParser::MissingArgument if $options[:insieme_builtins_dir] == nil || ($options[:std_include_dir] == nil && $options[:detect] == nil)
...
if($options[:std_include_dir] == nil && $options[:detect])
	compiler = "g++"
	compiler = ENV['INSIEME_CXX_BACKEND_COMPILER'] if(ENV['INSIEME_CXX_BACKEND_COMPILER'] != nil)
	preprocessor_output = `echo | #{compiler} -v -xc -E - 2>&1`
	match = preprocessor_output.match(/#include <\.\.\.> search starts here:\n\s+([^\n]+)/)
	if(match)
		$options[:std_include_dir] = match[1]
	else
		puts "Unable to parse preprocessor output"
		exit 1
	end
end
