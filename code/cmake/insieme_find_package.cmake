...
macro(insieme_find_package lib_name )
    ...
    if( DEFINED ENV{${lib_name}_VERSION} )
        # overwrite if user specifies otherwise 
        set(${lib_name}_VERSION $ENV{lib_name}_VERSION)
    endif()
    
    if( NOT DEFINED ${lib_name}_ROOT)
        #user did not provide a CMAKE variable
        if( DEFINED ENV{${lib_name}_ROOT} )
            #user did specify a custom ${lib_name}_ROOT as Environment variable
			set(${lib_name}_ROOT $ENV{${lib_name}_ROOT} CACHE PATH "${lib_name} installation directory")
        endif()
    endif()
    ...
    if( NOT DEFINED ${lib_name}_ROOT)
        ...
        if(EXISTS "${THIRD_PARTY_LIBS_HOME}/${lib_name_lower_case}-latest" AND IS_DIRECTORY "${THIRD_PARTY_LIBS_HOME}/${lib_name_lower_case}-latest")
            #set lib_name_ROOT to found path
			set(${lib_name}_ROOT "${THIRD_PARTY_LIBS_HOME}/${lib_name_lower_case}-latest" CACHE PATH "${lib_name_lower_case} installation directory")
        elseif()
            ...
            if(${lib_name}_FORCE_INSTALL)
                ...
                #set ${lib_name}_ROOT
				set(${lib_name}_ROOT "${THIRD_PARTY_LIBS_HOME}/${lib_name_lower_case}-latest" CACHE PATH "${lib_name} installation directory") 
            endif()
        endif()
    endif()
    
    find_package(${lib_name} ${${lib_name}_VERSION} COMPONENTS ${lib_components})
    ...
    if( NOT ${lib_name}_FOUND )
        ...
        find_package(${lib_name} ${${lib_name}_VERSION} REQUIRED COMPONENTS ${lib_components})
    endif()
endmacro()
